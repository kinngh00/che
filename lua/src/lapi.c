...
*/
struct CallS {  /* data to `ce_h5km51' */
  StkId func;
  int nresults;
};


static void ce_h5km51 (lua_State *L, void *ud) {
...
LUA_API int lua_pcall (lua_State *L, int nargs, int nresults, int errfunc) {
  ...
  c.nresults = nresults;
  status = luaD_pcall(L, ce_h5km51, &c, savestack(L, c.func), func);
  adjustresults(L, nresults);
  lua_unlock(L);
  return status;
}
...
*/
struct CCallS {  /* data to `ce_wpp2d7' */
  lua_CFunction func;
  void *ud;
};


static void ce_wpp2d7 (lua_State *L, void *ud) {
...
LUA_API int lua_cpcall (lua_State *L, lua_CFunction func, void *ud) {
  ...
  c.func = func;
  c.ud = ud;
  status = luaD_pcall(L, ce_wpp2d7, &c, savestack(L, L->top), 0);
  lua_unlock(L);
  return status;
}
