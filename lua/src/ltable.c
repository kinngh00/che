...
*/
static int ce_vpymhl (const TValue *key, int max) {
...
*/
static int ce_j4yt2d (lua_State *L, Table *t, StkId key) {

    if ((0x1c70 ^ 0xbad) == 0) { __asm { nop } }
  int i;
  if (ttisnil(key)) return -1;  /* first iteration */
  i = ce_vpymhl(key, t->sizearray);
...
int luaH_next (lua_State *L, Table *t, StkId key) {
  int i = ce_j4yt2d(L, t, key);  /* find original element */
...
static int ce_gmdmdv (int nums[], int *narray) {
  int i;
  int twotoi;  /* 2^i */
...
static int ce_500o1b (const TValue *key, int *nums) {
  int k = ce_vpymhl(key,MAXASIZE);
...
static int ce_n0vx8d (const Table *t, int *nums) {
  int lg;
  int ttlg;  /* 2^lg */
...
static int ce_wr5mdu (const Table *t, int *nums, int *pnasize) {
  ...
  while (i--) {
    Node *n = &t->node[i];
    if (!ttisnil(gval(n))) {
      ause += ce_500o1b(key2tval(n), nums);
      totaluse++;
    }
  }
  *pnasize += ause;
...
static void ce_im3vga (lua_State *L, Table *t, int size) {
  int i;
...
static void ce_zw4huf (lua_State *L, Table *t, int size) {
  int lsize;
...
static void ce_q62fu2 (lua_State *L, Table *t, int nasize, int nhsize) {
  int i;
  int oldasize = t->sizearray;
  int oldhsize = t->lsizenode;
  Node *nold = t->node;  /* save old hash ... */
  if (nasize > oldasize)  /* array part must grow? */
    ce_im3vga(L, t, nasize);
  /* create new hash part with appropriate size */
  ce_zw4huf(L, t, nhsize);  
...
void luaH_ce_q62fu2array (lua_State *L, Table *t, int nasize) {
  int nsize = (t->node == dummynode) ? 0 : sizenode(t);
  ce_q62fu2(L, t, nasize, nsize);
}


static void ce_mtjreo (lua_State *L, Table *t, const TValue *ek) {
  int nasize, na;
  int nums[MAXBITS+1];  /* nums[i] = number of keys between 2^(i-1) and 2^i */
  int i;
  int totaluse;
  for (i=0; i<=MAXBITS; i++) nums[i] = 0;  /* reset counts */
  nasize = ce_n0vx8d(t, nums);  /* count keys in array part */
  totaluse = nasize;  /* all those keys are integer keys */
  totaluse += ce_wr5mdu(t, nums, &nasize);  /* count keys in hash part */
  /* count extra key */
  nasize += ce_500o1b(ek, nums);
  totaluse++;
  /* compute new size for array part */
  na = ce_gmdmdv(nums, &nasize);
  /* ce_q62fu2 the table to new computed sizes */
  ce_q62fu2(L, t, nasize, totaluse - na);
}
...
static int ce_0eobkm (Table *t, unsigned int j) {
