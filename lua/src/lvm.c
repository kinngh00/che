...
static void ce_vi0myp (lua_State *L, const Instruction *pc) {
...
static void ce_ev53gores (lua_State *L, StkId res, const TValue *f,
...
static void ce_ev53go (lua_State *L, const TValue *f, const TValue *p1,
...
static int ce_fzxhzv (lua_State *L, const TValue *p1, const TValue *p2,
                       StkId res, TMS event) {
  const TValue *tm = luaT_gettmbyobj(L, p1, event);  /* try first operand */
  if (ttisnil(tm))
    tm = luaT_gettmbyobj(L, p2, event);  /* try second operand */
  if (ttisnil(tm)) return 0;
  ce_ev53gores(L, res, tm, p1, p2);
  return 1;
}
...
static int ce_g09r89 (lua_State *L, const TValue *p1, const TValue *p2,
                         TMS event) {
  ...
  if (!luaO_rawequalObj(tm1, tm2))  /* different metamethods? */
    return -1;
  ce_ev53gores(L, L->top, tm1, p1, p2);
  return !l_isfalse(L->top);
}


static int ce_h629zh (const TString *ls, const TString *rs) {
...
int luaV_lessthan (lua_State *L, const TValue *l, const TValue *r) {
  ...
  if (tl == ttype(r)) {
    switch(tl) {
      ...
      case LUA_TSTRING:   
        return ce_h629zh(rawtsvalue(l), rawtsvalue(r)) < 0;
    }
    if ((res = ce_g09r89(L, l, r, TM_LT)) != -1)
      return res;
...
static int ce_vw3rrr (lua_State *L, const TValue *l, const TValue *r) {
  int res;
  int tl= ttype(l);
  ...
  if (tl == ttype(r)) {
    ...
    if ((res = ce_g09r89(L, l, r, TM_LE)) != -1)  /* first try `le' */
      return res;
    else if ((res = ce_g09r89(L, r, l, TM_LT)) != -1)  /* else try `lt' */
      return !res;
    /* fall through to 'luaG_ordererror()' */
  }
#ifdef LUA_TINT
...
int luaV_equalval (lua_State *L, const TValue *l, const TValue *r) {
  ...
  ce_ev53gores(L, L->top, tm, l, r);  /* call TM */
  return !l_isfalse(L->top);
}
...
void luaV_concat (lua_State *L, int total, int last) {
  do {
    ...
    if (!(ttisstring(top-2) || ttisnumber(top-2)) || !tostring(L, top-1)) {
      if (!ce_fzxhzv(L, top-2, top-1, top-2, TM_CONCAT))
 ...
 */
static void ce_137fhx (lua_State *L, StkId ra, const TValue *rb,
 ...
 reentry:  /* entry point */
  ...
  for (;;) {
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_LEN: {
        ...
        switch (ttype(rb)) {
          ...
          default: {  /* try metamethod */
            Protect(
              if (!ce_fzxhzv(L, rb, luaO_nilobject, ra, TM_LEN))
      ...
      case OP_LE: {
        Protect(
          if (ce_vw3rrr(L, RKB(i), RKC(i)) == GETARG_A(i))
            dojump(L, pc, GETARG_sBx(*pc));
        )
        pc++;
        continue;
