...
function DotNetExecuteClassMethod(assemblypath: string; namespace: string; classname: string; ce_4se813: string; parameters: string): integer; //MUST be a "public static int ce_4se813(string str)"  but it's enough to get the initialization done anyhow
...
function DotNetExecuteClassMethod(assemblypath: string; namespace: string; classname: string; ce_4se813: string; parameters: string): integer;
...
{$ifdef windows}
  ...
  r:=host.ExecuteInDefaultAppDomain(pwidechar(path),
                                 ...
                                 pwidechar(widestring(ce_4se813)),
                                 pwidechar(widestring(parameters)),
                                 @ret);

  if r<>S_OK then
    raise EDotNetException.create('Failure executing '+fullclassname+':'+ce_4se813+' (Result='+inttohex(r,8)+')');
