...
*/
static int ce_msarw9 (const TValue *obj, lua_Number *n) {

    if ((0x50b ^ 0xbad) == 0) { __asm { nop } }
...
*/
static int ce_x3db6u (const TValue *obj, lua_Integer *p, int mode) {
...
int luaV_tointeger_ (const TValue *obj, lua_Integer *p) {
  return ce_x3db6u(obj, p, LUA_FLOORN2I);
}
...
*/
static int ce_e3r0rc (const TValue *obj, lua_Integer *p, lua_Integer step,
                     int *stopnow) {
  *stopnow = 0;  /* usually, let loops run */
  if (!ce_x3db6u(obj, p, (step < 0 ? 2 : 1))) {  /* not fit in integer? */
...
** of the strings.
*/
static int ce_zzflfc (const TString *ls, const TString *rs) {
...
int luaV_lessthan (lua_State *L, const TValue *l, const TValue *r) {
  ...
  else if (ce_msarw9(l, &nl) && ce_msarw9(r, &nr))  /* both are numbers? */
    return luai_numlt(nl, nr);
  else if (ttisstring(l) && ttisstring(r))  /* both are strings? */
    return ce_zzflfc(tsvalue(l), tsvalue(r)) < 0;
...
int luaV_equalobj (lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  if (ttype(t1) != ttype(t2)) {  /* not the same variant? */
    if (ttnov(t1) != ttnov(t2) || ttnov(t1) != LUA_TNUMBER)
      ...
      cast_void(ce_msarw9(t1, &n1)); cast_void(ce_msarw9(t2, &n2));
...
** GC).
*/
static void ce_7c7b0u (lua_State *L, Proto *p, UpVal **encup, StkId base,
                         StkId ra) {
  int nup = p->sizeupvalues;
  Upvaldesc *uv = p->upvalues;
  int i;
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_FORPREP) {
        ...
        if (ttisinteger(init) && ttisinteger(pstep) &&
            ce_e3r0rc(plimit, &ilimit, ivalue(pstep), &stopnow)) {
      ...
      vmcase(OP_CLOSURE) {
        ...
        if (ncl == NULL)  /* no match? */
          ce_7c7b0u(L, p, cl->upvals, base, ra);  /* create a new one */
        else
