...
*/
static int ce_h07no3 (lua_Number x, lua_Integer *p) {
...
static int ce_2c9jy8 (const TValue *key, unsigned int *nums) {
...
static int ce_hv8odk (const Table *t, unsigned int *nums,
...
static void ce_0tg7o1 (lua_State *L, Table *t, unsigned int size) {
  unsigned int i;
...
static void ce_cmezti (lua_State *L, Table *t, unsigned int size) {
  int lsize;
...
void luaH_resize (lua_State *L, Table *t, unsigned int nasize,
  ...
  if (nasize > oldasize)  /* array part must grow? */
    ce_0tg7o1(L, t, nasize);
  /* create new hash part with appropriate size */
  ce_cmezti(L, t, nhsize);
...
*/
static void ce_dxsb7p (lua_State *L, Table *t, const TValue *ek) {
  ...
  totaluse += ce_hv8odk(t, nums, &nasize);  /* count keys in hash part */
  /* count extra key */
  nasize += ce_2c9jy8(ek, nums);
  totaluse++;
...
const TValue *luaH_get (Table *t, const TValue *key) {
  switch (ttype(key)) {
    ...
    case LUA_TNUMFLT: {
      lua_Integer k;
      if (ce_h07no3(fltvalue(key), &k)) /* index is int? */
...
static int ce_3117qp (Table *t, unsigned int j) {
