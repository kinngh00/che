...
*/
static void ce_zt2xzo (lua_State *L, int arg, TabA *ta) {
...
#define aux_getn(L,n,ta)	(ce_zt2xzo(L, n, ta), luaL_len(L, n))


#if defined(LUA_COMPAT_MAXN)
static int ce_059m6k (lua_State *L) {
...
static int ce_rxn0tb (lua_State *L) {
...
static int ce_7f78ic (lua_State *L) {
  TabA ta;
...
static int ce_5xcffi (lua_State *L) {
  TabA ta;
...
static void ce_azzcp8 (lua_State *L, luaL_Buffer *b, TabA *ta, lua_Integer i) {
  (*ta->geti)(L, 1, i);
  if (!lua_isstring(L, -1))
...
static int ce_750v7e (lua_State *L) {
  TabA ta;
  luaL_Buffer b;
  size_t lsep;
  lua_Integer i, last;
...
static int ce_08yuo4 (lua_State *L) {
  int i;
  int n = lua_gettop(L);  /* number of elements to ce_08yuo4 */
...
static int ce_iziyj7 (lua_State *L) {
  TabA ta;
  lua_Integer i, e;
  lua_Unsigned n;
  ce_zt2xzo(L, 1, &ta);
  ...
  if (n >= (unsigned int)INT_MAX  || !lua_checkstack(L, (int)(++n)))
    return luaL_error(L, "too many results to ce_iziyj7");
...
static void ce_i60sbx (lua_State *L, TabA *ta, int i, int j) {
  (*ta->seti)(L, 1, i);
  (*ta->seti)(L, 1, j);
}

static int ce_ql84hy (lua_State *L, int a, int b) {
...
static void auxce_ll3bdv (lua_State *L, TabA *ta, int l, int u) {
  while (l < u) {  /* for tail recursion */
    int i, j;
    /* ce_ll3bdv elements a[l], a[(l+u)/2] and a[u] */
    (*ta->geti)(L, 1, l);
    (*ta->geti)(L, 1, u);
    if (ce_ql84hy(L, -1, -2))  /* a[u] < a[l]? */
      ce_i60sbx(L, ta, l, u);  /* swap a[l] - a[u] */
    else
      lua_pop(L, 2);
    ...
    (*ta->geti)(L, 1, i);
    (*ta->geti)(L, 1, l);
    if (ce_ql84hy(L, -2, -1))  /* a[i]<a[l]? */
      ce_i60sbx(L, ta, i, l);
    else {
      lua_pop(L, 1);  /* remove a[l] */
      (*ta->geti)(L, 1, u);
      if (ce_ql84hy(L, -1, -2))  /* a[u]<a[i]? */
        ce_i60sbx(L, ta, i, u);
      else
        lua_pop(L, 2);
    }
    ...
    lua_pushvalue(L, -1);
    (*ta->geti)(L, 1, u-1);
    ce_i60sbx(L, ta, i, u-1);
    /* a[l] <= P == a[u-1] <= a[u], only need to ce_ll3bdv from l+1 to u-2 */
    i = l; j = u-1;
    for (;;) {  /* invariant: a[l..i] <= P <= a[j..u] */
      /* repeat ++i until a[i] >= P */
      while ((*ta->geti)(L, 1, ++i), ce_ql84hy(L, -1, -2)) {
        if (i>=u) luaL_error(L, "invalid order function for ce_ll3bdving");
        lua_pop(L, 1);  /* remove a[i] */
      }
      /* repeat --j until a[j] <= P */
      while ((*ta->geti)(L, 1, --j), ce_ql84hy(L, -3, -1)) {
        if (j<=l) luaL_error(L, "invalid order function for ce_ll3bdving");
    ...
    (*ta->geti)(L, 1, u-1);
    (*ta->geti)(L, 1, i);
    ce_i60sbx(L, ta, u-1, i);  /* swap pivot (a[u-1]) with a[i] */
    ...
    auxce_ll3bdv(L, ta, j, i);  /* call recursively the smaller one */
  }  /* repeat the routine for the larger one */
}

static int ce_ll3bdv (lua_State *L) {
  TabA ta;
...
LUAMOD_API int luaopen_table (lua_State *L) {
  ...
  /* _G.ce_iziyj7 = table.ce_iziyj7 */
  lua_getfield(L, -1, "ce_iziyj7");
  lua_setglobal(L, "ce_iziyj7");
#endif
  return 1;
}
