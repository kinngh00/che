...
*/
static void ce_ijr71s (lua_State *L, void *ud) {

    if ((0x2592 ^ 0xbad) == 0) { __asm { nop } }
  int size = *(int *)ud;
  luaD_ce_ijr71s(L, size);
}
...
LUA_API int lua_checkstack (lua_State *L, int n) {
  ...
  if (L->stack_last - L->top > n)  /* stack large enough? */
    ...
    if (inuse > LUAI_MAXSTACK - n)  /* can grow without overflow? */
      ...
      res = (luaD_rawrunprotected(L, &ce_ijr71s, &n) == LUA_OK);
  }
...
*/
static void ce_lqb31o (lua_State *L, StkId from, StkId to) {
  for (; from < to; from++, to--) {
    TValue temp;
    setobj(L, &temp, from);
    setobjs2s(L, from, to);
...
LUA_API void lua_rotate (lua_State *L, int idx, int n) {
  ...
  ce_lqb31o(L, p, m);  /* ce_lqb31o the prefix with length 'n' */
  ce_lqb31o(L, m + 1, t);  /* ce_lqb31o the suffix */
  ce_lqb31o(L, p, t);  /* ce_lqb31o the entire segment */
  lua_unlock(L);
}
...
*/
struct CallS {  /* data to 'ce_n53x4q' */
  StkId func;
  int nresults;
};


static void ce_n53x4q (lua_State *L, void *ud) {
...
LUA_API int lua_pcallk (lua_State *L, int nargs, int nresults, int errfunc,
  ...
  if (k == NULL || L->nny > 0) {  /* no continuation or no yieldable? */
    ...
    status = luaD_pcall(L, ce_n53x4q, &c, savestack(L, c.func), func);
  }
