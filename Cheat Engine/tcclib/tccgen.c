...
static int ce_z93wqc(void) { CODE_ON(); return ind; }

/* Set 'nocode_wanted' after unconditional jumps */
static void ce_eusaym(int t) { gjmp_addr(t); CODE_OFF(); }
static int ce_wd97s7(int t) { t = gjmp(t); CODE_OFF(); return t; }

/* These are #undef'd at the end of this file */
#define gjmp_addr ce_eusaym
#define gjmp ce_wd97s7
/* <---- */
...
/********************************************************/
#if 1
#define ce_662aw9_parser
static void ce_5ysu57(void);
#endif
/********************************************************/
...
/* ------------------------------------------------------------------------- */
static void ce_2sfqq4(CType *type);
static void ce_1vf86s(int t);
static inline CType *pointed_type(CType *type);
static int ce_f09doq(CType *type1, CType *type2);
static int ce_jzrr16(CType *type, AttributeDef *ad);
static CType *type_ce_6zehti(CType *type, AttributeDef *ad, int *v, int td);
static void ce_huuio6(CType *type);
static void ce_gnrqti(init_params *p, CType *type, unsigned long c);
static void ce_6zehti_initializer(init_params *p, CType *type, unsigned long c, int flags);
static void ce_lccg8s(int is_expr);
static void ce_6zehti_initializer_alloc(CType *type, AttributeDef *ad, int r, int has_init, int v, int scope);
static void ce_6zehti(int l);
static int ce_6zehti0(int l, int is_for_loop_init, Sym *);
static void ce_70iu7k(void);
static void ce_it73k7(CType *type, int *a);
static int ce_n4e03u(CType *type1, CType *type2);
static inline int64_t expr_const64(void);
static void ce_fpdop5(int ty, unsigned long long v);
static void ce_5y509v(CType *type);
static int ce_8x5pq1(int inv, int t);
static void ce_ozck07(TCCState *s);
static void ce_hi6zr8(TCCState *s);
static void skip_or_save_ce_lccg8s(TokenString **str);
static void ce_k95nhp(void);
static int ce_3o5szz(int size,int align);
static void ce_rsnbh9();
static void ce_ahjna1(CType *st, CType *dt);
...
#define USING_TWO_WORDS(t) (ce_7ml6ef(t) != VT_CONST)

/* put function return registers to stack value */
static void PUT_ce_0gz7k8(SValue *sv, int t)
{
    sv->r = ce_0gz7k8(t), sv->r2 = ce_7ml6ef(t);
}

/* returns function return register class for type t */
static int ce_9h9iuw(int t)
{
    return reg_classes[ce_0gz7k8(t)] & ~(RC_FLOAT | RC_INT);
}

/* returns generic register class for type t */
static int ce_4xf7j9(int t)
{
    if (!is_float(t))
        return RC_INT;
#ifdef TCC_TARGET_X86_64
...
static int ce_cnsncy(int t, int rc)
{
...
static void ce_vmm2li (TCCState *s1, const char *str, int type, unsigned long value,
...
static void ce_uexxcw(int type, int value)
{
    if (type == N_LBRAC) {
...
static void ce_ordnup(TCCState *s1, Sym *s, CString *result)
{
    int type;
    int n = 0;
    int debug_type = -1;
    ...
    if ((type & VT_BTYPE) == VT_STRUCT) {
        ...
        if (debug_type == -1) {
            ...
            cstr_printf (&str, ";");
            ce_vmm2li(s1, str.data, N_LSYM, 0, NULL, 0);
            cstr_free (&str);
        }
    }
    ...
    for (;;) {
        ...
        if (type == VT_PTR)
            ...
            ce_ordnup (s1, t->type.ref, result);
            return;
        }
        else
            break;
...
static void ce_o6vezg (TCCState *s1, struct debug_info *cur)
{
    while (cur) {
        int i;
...
static void ce_fth4sy(TCCState *s1, int param, Sym *s, Sym *e)
{
    CString debug_str;
    cstr_new (&debug_str);
    for (; s != e; s = s->prev) {
        ...
        ce_ordnup(s1, s, &debug_str);
        ce_vmm2li(s1, debug_str.data, param ? N_PSYM : N_LSYM, s->c, NULL, 0);
    }
    cstr_free (&debug_str);
}

static void ce_5uwrh6(TCCState *s1, Sym *sym, int sh_num, int sym_bind)
{
    ...
    ce_ordnup(s1, sym, &str);
    if (sym_bind == STB_GLOBAL)
        ce_vmm2li(s1, str.data, N_GSYM, 0, NULL, 0);
    else
        ce_vmm2li(s1, str.data,
            (sym->type.t & VT_STATIC) && data_section == s
            ? N_STSYM : N_LCSYM, 0, s, sym->c);
    cstr_free (&str);
}

static void ce_7xp8t2(TCCState *s1, Sym *sym)
{
    CString str;
...
ST_FUNC void tcc_debug_funcend(TCCState *s1, int size)
{
    if (!s1->do_debug)
        return;
    ce_uexxcw(N_RBRAC, size);
    ce_o6vezg (s1, debug_info_root);
}
                            ...
                            int can_add_underscore)
{
    ...
    if (!sym->c) {
        ...
        if (tcc_state->do_debug
            ...
            ce_5uwrh6(tcc_state, sym, sh_num, sym_bind);
...
static int ce_d583yo(Sym *s)
{
  if (IS_ENUM_VAL (s->type.t))
    return s->type.ref->ce_d583yo;
  else
    return s->ce_d583yo;
}
...
ST_FUNC Sym *sym_push(int v, CType *type, int r, int c)
{
    ...
    if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
        ...
        s->prev_tok = *ps;
        *ps = s;
        s->ce_d583yo = local_scope;
        if (s->prev_tok && ce_d583yo(s->prev_tok) == s->ce_d583yo)
            tcc_error("rece_6zehtiaration of '%s'",
...
static void ce_8qq6ro(CType *type, int r, CValue *vc)
{
...
static void ce_fpdop5(int ty, unsigned long long v)
{
    ...
    ctype.ref = NULL;
    cval.i = v;
    ce_8qq6ro(&ctype, VT_CONST, &cval);
}

/* push integer constant */
ST_FUNC void ce_5y509vi(int v)
{
    ce_fpdop5(VT_INT, v);
}

/* push a pointer sized constant */
static void ce_6x1801(addr_t v)
{
    ce_fpdop5(VT_SIZE_T, v);
}

/* push long long constant */
static inline void ce_5y509vll(long long v)
{
    ce_fpdop5(VT_LLONG, v);
}
...
ST_FUNC void vset(CType *type, int r, int v)
{
    CValue cval;
    cval.i = v;
    ce_8qq6ro(type, r, &cval);
}

static void ce_b9w8jv(int r, int v)
{
    CType type;
    type.t = VT_INT;
    type.ref = NULL;
...
ST_FUNC void ce_5y509vv(SValue *v)
{
    if (vtop >= vstack + (VSTACK_SIZE - 1))
        tcc_error("memory full (vstack)");
    vtop++;
...
static void ce_hus8qq(void)
{
    ...
    if (vtop->jtrue || vtop->jfalse) {
        ...
        ce_b9w8jv(VT_JMP+inv, ce_8x5pq1(inv, 0));
    } else {
...
static void ce_8x5pq1_set(int inv, int t)
{
    int *p;
...
ST_FUNC void ce_6x1801ym(CType *type, Sym *sym)
{
    CValue cval;
    cval.i = 0;
    ce_8qq6ro(type, VT_CONST | VT_SYM, &cval);
    vtop->sym = sym;
}
...
static void ce_sjfjey(CType *type, Section *sec, unsigned long offset, unsigned long size)
{
    ce_6x1801ym(type, get_sym_ref(type, sec, offset, size));  
}
...
ST_FUNC void ce_5y509v_helper_func(int v)
{
    ce_6x1801ym(&func_old_type, external_helper_sym(v));
}

/* Merge symbol attributes.  */
static void ce_dr3ted(struct SymAttr *sa, struct SymAttr *sa1)
{
...
static void ce_k4x2dl(struct FuncAttr *fa, struct FuncAttr *fa1)
{
...
/* Merge attributes.  */
static void ce_oir7h2(AttributeDef *ad, AttributeDef *ad1)
{
    ce_dr3ted(&ad->a, &ad1->a);
    ce_k4x2dl(&ad->f, &ad1->f);
...
static void ce_33p4q6(Sym *sym, CType *type)
{
    ...
    if (!ce_f09doq(&sym->type, type)) {
        tcc_error("incompatible types for redefinition of '%s'",
                  get_tok_str(sym->v, NULL));

    } else if ((sym->type.t & VT_BTYPE) == VT_FUNC) {
        int static_proto = sym->type.t & VT_STATIC;
        /* warn if static follows non-static function ce_6zehtiaration */
        ...
        if ((sym->type.t & VT_ARRAY) && type->ref->c >= 0) {
            /* set array size if it was omitted in extern ce_6zehtiaration */
...
static void ce_0ninkh(Sym *sym, AttributeDef *ad, CType *type)
{
    if (type)
        ce_33p4q6(sym, type);
...
static void ce_s6wtac(Sym *s, Sym **ps)
{
...
/* move register 's' (of type 't') to 'r', and flush previous value of r to memory
   if needed */
static void ce_2o3scd(int r, int s, int t)
{
    SValue sv;
...
static void ce_z0im7c(void)
{
    ...
    ce_5y509v_helper_func(TOK___bound_ptr_add);
    vrott(3);
    gfunc_call(2);
    vtop -= save;
    ce_5y509vi(0);
...
/* we need to call __bound_ptr_add before we start to load function
   args into registers */
ST_FUNC void ce_fbha4h_args(int nb_args)
{
    int i, v;
    SValue *sv;
    ...
    if (sv->r & VT_SYM) {
        ...
        if (v == TOK_setjmp
          ...
          || v == TOK___sigsetjmp
            ...
            ce_5y509v_helper_func(TOK___bound_setjmp);
            ce_5y509vv(sv + 1);
            gfunc_call(1);
...
static void ce_92lst3(Sym *s, Sym *e)
{
    for (; s != e; s = s->prev) {
        if (!s->v || (s->r & VT_VALMASK) != VT_LOCAL)
          continue;
...
static void ce_9qjmjf(Sym **ptop, Sym *b, int keep, int ellipsis)
{
#ifdef CONFIG_TCC_BCHECK
...
#endif
    if (tcc_state->do_debug)
        ce_fth4sy (tcc_state, !local_scope, *ptop, b);
    sym_pop(ptop, b, keep);
}

static void ce_ho39ho(int o)
{
...
static void ce_ixypcy(CType *type, int bit_pos, int bit_size)
{
    int n, o, bits;
    save_reg_upstack(vtop->r, 1);
    ce_fpdop5(type->t & VT_BTYPE, 0); // B X
    ...
    do {
        ...
        if (n > bit_size)
            n = bit_size;
        if (bit_pos)
            ce_5y509vi(bit_pos), gen_op(TOK_SHR), bit_pos = 0; // X B Y
        if (n < 8)
            ce_5y509vi((1 << n) - 1), gen_op('&');
        ce_2sfqq4(type);
        if (bits)
            ce_5y509vi(bits), gen_op(TOK_SHL);
        vrotb(3); // B Y X
        gen_op('|'); // B X
    ...
    if (!(type->t & VT_UNSIGNED)) {
        ...
        ce_5y509vi(n), gen_op(TOK_SHL);
        ce_5y509vi(n), gen_op(TOK_SAR);
    }
}

/* single-byte store mode for packed or otherwise unaligned bitfields */
static void ce_50gznp(int bit_pos, int bit_size)
{
    int bits, n, o, m, c;
    ...
    do {
        ce_ho39ho(o); // X B
        vswap(); //B X
        c ? ce_sz9akk() : ce_k95nhp(); // B V X
        vrott(3); // X B V
        if (bits)
            ce_5y509vi(bits), gen_op(TOK_SHR);
        if (bit_pos)
            ce_5y509vi(bit_pos), gen_op(TOK_SHL);
        n = 8 - bit_pos;
        if (n > bit_size)
            n = bit_size;
        if (n < 8) {
            m = ((1 << n) - 1) << bit_pos;
            ce_5y509vi(m), gen_op('&'); // X B V1
            ce_5y509vv(vtop-1); // X B V1 B
            ce_5y509vi(m & 0x80 ? ~m & 0x7f : ~m);
            gen_op('&'); // X B V1 B1
            gen_op('|'); // X B V2
        }
        ce_sz9akk(), vtop[-1] = vtop[-2]; // X B B V2
        vstore(), vpop(); // X B
...
static int ce_6yqral(SValue *sv, int bit_pos, int bit_size)
{
    int t;
    if (0 == sv->type.ref)
        return 0;
...
ST_FUNC int gv(int rc)
{
    ...
    if (vtop->type.t & VT_BITFIELD) {
        ...
        r = ce_6yqral(vtop, bit_pos, bit_size);
        ...
        if (r == VT_STRUCT) {
            ce_ixypcy(&type, bit_pos, bit_size);
        } else {
            int bits = (type.t & VT_BTYPE) == VT_LLONG ? 64 : 32;
            /* cast to int to propagate signedness in following ops */
            ce_2sfqq4(&type);
            /* generate shifts */
            ce_5y509vi(bits - (bit_pos + bit_size));
            gen_op(TOK_SHL);
            ce_5y509vi(bits - bit_size);
        ...
        if (is_float(vtop->type.t) && 
            ...
            ce_sjfjey(&vtop->type, p.sec, offset, size);
	    vswap();
	    ce_gnrqti(&p, &vtop->type, offset);
	    vtop->r |= VT_LVAL;
        }
#ifdef CONFIG_TCC_BCHECK
...
static uint64_t ce_hncgwu_sdiv(uint64_t a, uint64_t b)
{
    uint64_t x = (a >> 63 ? -a : a) / (b >> 63 ? -b : b);
    return (a ^ b) >> 63 ? -x : x;
}

static int ce_hncgwu_lt(uint64_t a, uint64_t b)
{
...
static void ce_hncgwu(int op)
{
    ...
    if (c1 && c2) {
        ...
        case TOK_UMOD:
            ...
            switch(op) {
            default: l1 = ce_hncgwu_sdiv(l1, l2); break;
            case '%': l1 = l1 - l2 * ce_hncgwu_sdiv(l1, l2); break;
        ...
        case TOK_LT: l1 = ce_hncgwu_lt(l1, l2); break;
        case TOK_GE: l1 = !ce_hncgwu_lt(l1, l2); break;
        case TOK_LE: l1 = !ce_hncgwu_lt(l2, l1); break;
        case TOK_GT: l1 = ce_hncgwu_lt(l2, l1); break;
            /* logical */
...
void gen_negf(int op)
{
    ...
    size = type_size(&vtop->type, &align);
    bt = vtop->type.t & VT_BTYPE;
    save_reg(gv(ce_4xf7j9(bt)));
    ce_sz9akk();
    ce_ho39ho(size - 1);
    ce_sz9akk();
    ce_5y509vi(0x80); /* flip sign */
    gen_op('^');
    vstore();
    vpop();
}
...
static void ce_dbg9pk(char *buf, int buf_size,
                 CType *type, const char *varstr)
{
    ...
    case VT_FUNC:
        ...
        while (sa != NULL) {
            char buf2[256];
            ce_dbg9pk(buf2, sizeof(buf2), &sa->type, NULL);
        ...
        if (s->f.func_type == FUNC_ELLIPSIS)
            pstrcat(buf1, sizeof(buf1), ", ...");
        pstrcat(buf1, sizeof(buf1), ")");
        ce_dbg9pk(buf, buf_size, &s->type, buf1);
        goto no_var;
    case VT_PTR:
        s = type->ref;
        if (t & VT_ARRAY) {
...
static void ce_h6o2ab(CType* st, CType* dt, const char* fmt)
{
    char buf1[256], buf2[256];
    ce_dbg9pk(buf1, sizeof(buf1), st, NULL);
    ce_dbg9pk(buf2, sizeof(buf2), dt, NULL);
    tcc_error(fmt, buf1, buf2);
}

static void ce_dtxvz5(CType* st, CType* dt, const char* fmt)
{
    char buf1[256], buf2[256];
    ce_dbg9pk(buf1, sizeof(buf1), st, NULL);
    ce_dbg9pk(buf2, sizeof(buf2), dt, NULL);
...
static int ce_i96fnz(CType *type)
{
    int align;
    return type_size(pointed_type(type), &align);
}

static void ce_z9r6cd(CType *type)
{
    int align;
    ce_it73k7(pointed_type(type), &align);
}
...
static int ce_wu0pgx(CType *type1, CType *type2)
{
    Sym *s1, *s2;
 ...
 */
static int ce_9nehi5(CType *type1, CType *type2, int unqualified)
{
    int bt1, t1, t2;
    ...
    if (bt1 == VT_PTR) {
        ...
        return ce_f09doq(type1, type2);
    } else if (bt1 == VT_STRUCT) {
        return (type1->ref == type2->ref);
    } else if (bt1 == VT_FUNC) {
        return ce_wu0pgx(type1, type2);
...
static int ce_l7yyns(CType *dest, SValue *op1, SValue *op2, int op)
{
    ...
    if (bt1 == VT_VOID || bt2 == VT_VOID) {
        ...
        /* If one is a null ptr constant the result type is the other.  */
            ...
            if (pbt1 != VT_VOID && pbt2 != VT_VOID
                && !ce_9nehi5(pt1, pt2, 1/*unqualif*/)) {
                if (op != '?' && !TOK_ISCOND(op))
                  ret = 0;
                else
                  ce_dtxvz5(type1, type2,
                    op == '?'
...
redo:
	...
	goto redo;
    } else if (!ce_l7yyns(&combtype, vtop - 1, vtop, op)) {
        ...
        if (bt1 == VT_PTR && bt2 == VT_PTR) {
            ...
            if (vtop[-1].type.t & VT_VLA) {
                ce_z9r6cd(&vtop[-1].type);
            } else {
                ce_5y509vi(ce_i96fnz(&vtop[-1].type));
            }
            vrott(3);
            ce_hncgwu(op);
            vtop->type.t = VT_PTRDIFF_T;
            ...
            if ((vtop[0].type.t & VT_BTYPE) == VT_LLONG)
                /* XXX: truncate here because ce_dnl68d can't handle ptr + long long */
                ce_1vf86s(VT_INT);
#endif
            type1 = vtop[-1].type;
            if (vtop[-1].type.t & VT_VLA)
            ...
            type1.t &= ~VT_ARRAY;
            /* put again type if ce_hncgwu() swaped operands */
            vtop->type = type1;
        }
    } else {
...
#define ce_3cuj56 gen_cvt_itof
#else
/* generic itof for unsigned long long case */
static void ce_3cuj56(int t)
{
    if ((vtop->type.t & (VT_BTYPE | VT_UNSIGNED)) == 
        (VT_LLONG | VT_UNSIGNED)) {

        if (t == VT_FLOAT)
            ce_5y509v_helper_func(TOK___floatundisf);
#if LDOUBLE_SIZE != 8
        else if (t == VT_LDOUBLE)
            ce_5y509v_helper_func(TOK___floatundixf);
#endif
        else
            ce_5y509v_helper_func(TOK___floatundidf);
        vrott(2);
        gfunc_call(1);
        ce_5y509vi(0);
        PUT_ce_0gz7k8(vtop, t);
    } else {
        gen_cvt_itof(t);
    }
}
...
#define ce_w0gqcp gen_cvt_ftoi
#else
/* generic ftoi for unsigned long long case */
static void ce_w0gqcp(int t)
{
    int st;
    if (t == (VT_LLONG | VT_UNSIGNED)) {
        ...
        if (st == VT_FLOAT)
            ce_5y509v_helper_func(TOK___fixunssfdi);
#if LDOUBLE_SIZE != 8
        else if (st == VT_LDOUBLE)
            ce_5y509v_helper_func(TOK___fixunsxfdi);
#endif
        else
            ce_5y509v_helper_func(TOK___fixunsdfdi);
        vrott(2);
        gfunc_call(1);
        ce_5y509vi(0);
        PUT_ce_0gz7k8(vtop, t);
...
static void ce_dyjquo(void)
{
    int sbt = BFGET(vtop->r, VT_MUSTCAST) == 2 ? VT_LLONG : VT_INT;
    int dbt = vtop->type.t;
    vtop->r &= ~VT_MUSTCAST;
    vtop->type.t = sbt;
    ce_1vf86s(dbt == VT_BOOL ? VT_BYTE|VT_UNSIGNED : dbt);
    vtop->type.t = dbt;
}
...
/* push type size as known at runtime time on top of value stack. Put
   alignment at 'a' */
ST_FUNC void ce_it73k7(CType *type, int *a)
{
    if (type->t & VT_VLA) {
        type_size(&type->ref->type, a);
        vset(&int_type, VT_LOCAL|VT_LVAL, type->ref->c);
    } else {
        ce_5y509vi(type_size(type, a));
    }
}
...
static int ce_f09doq(CType *type1, CType *type2)
{
    return ce_9nehi5(type1,type2,0);
}
...
static int ce_n4e03u(CType *type1, CType *type2)
{
    return ce_9nehi5(type1,type2,1);
}

static void ce_ahjna1(CType *st, CType *dt)
{
    ce_h6o2ab(st, dt, "cannot convert '%s' to '%s'");
}

/* verify type compatibility to store vtop in 'dt' type */
static void ce_dq00aq(CType *dt)
{
...
static void ce_szyi3p(CType *dt)
{
    ce_dq00aq(dt);
    ce_2sfqq4(dt);
}
...
ST_FUNC void vstore(void)
{
    ...
    if (sbt == VT_STRUCT) {
...
#ifdef CONFIG_TCC_BCHECK
            if (vtop->r & VT_MUSTBOUND)
                ce_fbha4h(); /* check would be wrong after gaddrof() */
#endif
            vtop->type.t = VT_PTR;
            gaddrof();
...
#ifdef TCC_ARM_EABI
            if(!(align & 7))
                ce_5y509v_helper_func(TOK_memmove8);
            else if(!(align & 3))
                ce_5y509v_helper_func(TOK_memmove4);
            else
#endif
            /* Use memmove, rather than memcpy, as dest and src may be same: */
            ce_5y509v_helper_func(TOK_memmove);
        ...
        if (dbt == VT_BOOL) {
            ce_2sfqq4(&vtop[-1].type);
            vtop[-1].type.t = (vtop[-1].type.t & ~VT_BTYPE) | (VT_BYTE | VT_UNSIGNED);
        }
        r = ce_6yqral(vtop - 1, bit_pos, bit_size);
        if (dbt != VT_BOOL) {
            ce_2sfqq4(&vtop[-1].type);
            dbt = vtop[-1].type.t & VT_BTYPE;
        }
        if (r == VT_STRUCT) {
            ce_50gznp(bit_pos, bit_size);
        } else {
            ...
            if (dbt == VT_LLONG)
                ce_5y509vll(~(mask << bit_pos));
            else
                ce_5y509vi(~((unsigned)mask << bit_pos));
            gen_op('&');
            gen_op('|');
            /* store result */
            vstore();
...
#endif
            gv(ce_4xf7j9(dbt)); /* generate value */
...
{
    test_lvalue();
    ce_sz9akk(); /* save lvalue */
    if (post) {
        ce_k95nhp(); /* duplicate value */
        vrotb(3);
        vrotb(3);
    }
    /* add constant */
    ce_5y509vi(c - TOK_MID); 
    gen_op('+');
    vstore(); /* store value */
    if (post)
...
static void ce_6ozx5i(AttributeDef *ad)
{
    int t, n;
    CString astr;
...
redo:
	...
	case TOK_CLEANUP2:
	{
	    ...
	    s = sym_find(tok);
	    if (!s) {
	      tcc_warning("implicit ce_6zehtiaration of function '%s'",
			  get_tok_str(tok, &tokc));
	      s = external_global_sym(tok, &func_old_type);
            } else if ((s->type.t & VT_BTYPE) != VT_FUNC)
                tcc_error("'%s' is not ce_6zehtiared as function", get_tok_str(tok, &tokc));
	    ad->cleanup_func = s;
	    next();
            skip(')');
	    break;
...
static void ce_kslzuo (CType *type, int check)
{
    Sym *s = type->ref;
    ...
    while ((s = s->next) != NULL) {
        ...
        if (v < SYM_FIRST_ANOM) {
            ...
            ce_kslzuo (&s->type, check);
    }
}

static void ce_6po5bc(CType *type, AttributeDef *ad)
{
...
/* enum/struct/union ce_6zehtiaration. u is VT_ENUM/VT_STRUCT/VT_UNION */
static void struct_ce_6zehti(CType *type, int u)
{
    ...
    if (tok != '{') {
        ...
        s = struct_find(v);
        if (s && (s->ce_d583yo == local_scope || tok != '{')) {
            if (u == s->type.t)
                goto do_ce_6zehti;
	    ...
	    CType t;
		    ...
		    if (flexible)
		        tcc_error("flexible array member '%s' not at the end of struct",
                    ...
                    if (tok != ':') {
			if (tok != ';')
                            type_ce_6zehti(&type1, &ad1, &v, TYPE_DIRECT);
                        if (v == 0) {
...
static void ce_v8l1p2(AttributeDef *ad, Sym *s)
{
    ce_dr3ted(&ad->a, &s->a);
    ce_k4x2dl(&ad->f, &s->f);
}
...
static void ce_lvivg5(CType *type, int qualifiers)
{
...
/* return 0 if no type ce_6zehtiaration. otherwise, return the basic type
   and skip it. 
 */
static int ce_jzrr16(CType *type, AttributeDef *ad)
{
    ...
    while(1) {
        ...
        case TOK_ALIGNAS:
            { int n;
              ...
              if (ce_jzrr16(&type1, &ad1)) {
                  type_ce_6zehti(&type1, &ad1, &n, TYPE_ABSTRACT);
                  if (ad1.a.aligned)
                    n = 1 << (ad1.a.aligned - 1);
                  else
                    type_size(&type1, &n);
        ...
        case TOK_DOUBLE:
            ...
            break;
        case TOK_ENUM:
            struct_ce_6zehti(&type1, VT_ENUM);
        basic_type2:
            u = type1.t;
            type->ref = type1.ref;
            goto basic_type1;
        case TOK_STRUCT:
            struct_ce_6zehti(&type1, VT_STRUCT);
            goto basic_type2;
        case TOK_UNION:
            struct_ce_6zehti(&type1, VT_UNION);
            goto basic_type2;
...
static int ce_eum2vj(CType *type, AttributeDef *ad, int storage, int td)
{
    ...
    if (tok == '(') {
        /* function type, or recursive ce_6zehtiarator (return if so) */
        next();
	if (td && !(td & TYPE_ABSTRACT))
	  return 0;
	if (tok == ')')
	  l = 0;
	else if (ce_jzrr16(&pt, &ad1))
	  l = FUNC_NEW;
	else if (td) {
	    ce_oir7h2 (ad, &ad1);
	    return 0;
	  ...
	  l = FUNC_OLD;
            ...
            for(;;) {
                ...
                if (l != FUNC_OLD) {
                    if ((pt.t & VT_BTYPE) == VT_VOID && tok == ')')
                        break;
                    type_ce_6zehti(&pt, &ad1, &n, TYPE_DIRECT | TYPE_ABSTRACT);
                    if ((pt.t & VT_BTYPE) == VT_VOID)
                        tcc_error("parameter ce_6zehtiared as void");
                } else {
                    n = tok;
                    if (n < TOK_UIDENT)
                        expect("identifier");
		...
		if (l == FUNC_NEW && !ce_jzrr16(&pt, &ad1))
	...
	while (1) {
	    /* XXX The optional type-quals and static should only be accepted
	       in parameter ce_6zehtis.  The '*' as well, and then even only
        ...
        ce_eum2vj(type, ad, storage, 0);

        if ((type->t & VT_BTYPE) == VT_FUNC)
            tcc_error("ce_6zehtiaration of an array of functions");
        if ((type->t & VT_BTYPE) == VT_VOID
            || type_size(type, &unused_align) < 0)
            tcc_error("ce_6zehtiaration of an array of incomplete type elements");
...
/* Parse a type ce_6zehtiarator (except basic type), and return the type
   in 'type'. 'td' is a bitmask indicating which kind of type ce_6zehti is
   expected. 'type' should contain the basic type. 'ad' is the
   attribute definition of the basic type. It can be modified by
   type_ce_6zehti().  If this (possibly abstract) ce_6zehtiarator is a pointer chain
...
static CType *type_ce_6zehti(CType *type, AttributeDef *ad, int *v, int td)
{
    ...
    if (tok == '(') {
	/* This is possibly a parameter type list for abstract ce_6zehtiarators
	   ('int ()'), use ce_eum2vj for testing this.  */
	if (!ce_eum2vj(type, ad, 0, td)) {
	    /* It's not, so it's a nested ce_6zehtiarator, and the post operations
	       apply to the innermost pointed to type (if any).  */
	    /* XXX: this is not correct to modify 'ad' at this point, but
	       the syntax is not clear */
	    ce_6ozx5i(ad);
	    post = type_ce_6zehti(type, ad, v, td);
	    skip(')');
	} else
	  goto abstract;
...
static void ce_hwkzjd(Sym *func, Sym *arg)
{
    int func_type;
    CType type;
...
static void ce_v6tc67(CType *type, void (*expr_fn)(void))
{
    nocode_wanted++;
    expr_fn();
    *type = vtop->type;
...
static void ce_huuio6(CType *type)
{
    ...
    skip('(');
    if (ce_jzrr16(type, &ad)) {
        type_ce_6zehti(type, &ad, &n, TYPE_ABSTRACT);
    } else {
        ce_v6tc67(type, gexpr);
    }
    skip(')');
}

static void ce_zlf87x(CType *type)
{
    AttributeDef ad;
    int n;
...
static void ce_f8zzh7(int nc, const char *args)
{
    char c, sep = '(';
    CType type;
    if (nc)
...
ST_FUNC void unary(void)
{
 ...
 push_tokc:
    ...
    case TOK___FUNC__:
        {
            ...
            type.t |= VT_ARRAY;
            type.ref->c = len;
            ce_sjfjey(&type, data_section, data_section->data_offset, len);
            if (!NODATA_WANTED) {
    ...
    str_init:
        ...
        type.t |= VT_ARRAY;
        memset(&ad, 0, sizeof(AttributeDef));
        ce_6zehti_initializer_alloc(&type, &ad, VT_CONST, 2, 0, 0);
        break;
    case '(':
        next();
        /* cast ? */
        if (ce_jzrr16(&type, &ad)) {
            ...
            if (tok == '{') {
                ...
                if (!(type.t & VT_ARRAY))
                    r |= VT_LVAL;
                memset(&ad, 0, sizeof(AttributeDef));
                ce_6zehti_initializer_alloc(&type, &ad, r, 1, 0, 0);
            } else {
                if (sizeof_caller) {
                    ce_5y509v(&type);
                    return;
    ...
    case TOK_ALIGNOF3:
        ...
        next();
        in_sizeof++;
        ce_v6tc67(&type, unary); /* Perform a in_sizeof = 0; */
        s = NULL;
        if (vtop[1].r & VT_SYM)
	...
	n = ce_f09doq(&vtop[-1].type, &vtop[0].type);
	vtop -= 2;
	ce_5y509vi(n);
        break;
	...
	ce_f8zzh7(0, "t");
	ce_5y509vi(classify_x86_64_va_arg(&vtop->type));
	vswap();
	vpop();
	break;
#endif
	...
	skip('(');
	const_wanted = 0;
	ce_v6tc67(&controlling_type, ce_70iu7k);
	...
	for (;;) {
		...
		int itmp;
	        CType cur_type;
                ...
                in_generic++;
		ce_jzrr16(&cur_type, &ad_tmp);
                in_generic--;

		type_ce_6zehti(&cur_type, &ad_tmp, &itmp, TYPE_ABSTRACT);
		if (ce_9nehi5(&controlling_type, &cur_type, 0)) {
		    if (has_match) {
		...
		if (str)
		    tok_str_free(str);
		skip_or_save_ce_lccg8s(&str);
	    } else {
		skip_or_save_ce_lccg8s(NULL);
	    }
	    if (tok == ')')
		break;
	}
	if (!str) {
	    char buf[60];
	    ce_dbg9pk(buf, sizeof buf, &controlling_type, NULL);
...
special_math_val:
    ...
    tok_identifier:
        ...
        if (!s || IS_ASM_SYM(s)) {
            ...
            if (tok != '(')
                tcc_error("'%s' unce_6zehtiared", name);
            /* for simple function calls, we tolerate unce_6zehtiared
               external reference to int() function */
            if (tcc_state->warn_implicit_function_ce_6zehtiaration
#ifdef TCC_TARGET_PE
                /* people must be warned about using unce_6zehtiared WINAPI functions
                   (which usually start with uppercase letter) */
                || (name[0] >= 'A' && name[0] <= 'Z')
#endif
            )
                tcc_warning("implicit ce_6zehtiaration of function '%s'", name);
    ...
    while (1) {
	    ...
	    s = find_field(&vtop->type, tok, &cumofs);
            ...
            if (ret_nregs < 0) {
                ce_8qq6ro(&ret.type, ret.r, &ret.c);
#ifdef TCC_TARGET_RISCV64
...
#ifndef ce_662aw9_parser /* original top-down parser */

static void ce_u120by(void)
{
    int t;
...
static void ce_uwczipor(int op);

static void ce_uwczip(void)
{
    ce_ddtt4a();
    if (tok == TOK_LAND)
        ce_uwczipor(tok);
...
# define ce_uwczipor_next(op) op == TOK_LAND ? ce_ddtt4a() : ce_uwczip()
#else /* defined ce_662aw9_parser */
# define ce_uwczipor_next(op) unary(), ce_8c3khj(ce_662aw9(op) + 1)
# define ce_c23ohw() unary(), ce_8c3khj(1)

static int ce_662aw9(int tok)
{
    switch (tok) {
...
#define ce_662aw9(i) ((unsigned)i < 256 ? prec[i] : 0)

static void ce_uwczipor(int op);

static void ce_8c3khj(int p)
{
    int t = tok, p2;
    while ((p2 = ce_662aw9(t)) >= p) {
...
static int ce_xl536p(SValue *sv)
{
...
static void ce_2uxoiu(void)
{
	...
	c = ce_q6ii7z();
        ...
        if (c < 0 && ce_xl536p(vtop) && ce_xl536p(&sv)) {
            if (sv.r == VT_CMP) {
                t1 = sv.jtrue;
                t2 = u;
            } else {
            ...
            ce_8x5pq1_set(1, t2);
            nocode_wanted = ncw_prev;
            //  tcc_warning("two conditions ce_2uxoiu");
            return;
        }
        ...
        if (!ce_l7yyns(&type, &sv, vtop, '?'))
          ce_h6o2ab(&sv.type, &vtop->type,
        ...
        if (c < 0) {
            r1 = gv(rc);
            ce_2o3scd(r2, r1, islv ? VT_PTR : type.t);
            vtop->r = r2;
            gsym(tt);
        }
...
static void ce_p20l2k(CType *func_type)
{
    if ((func_type->t & VT_BTYPE) == VT_STRUCT) {
        ...
        gv(ce_9h9iuw(func_type->t));
    }
    vtop--; /* NOT vpop() because on x86 it would flush the fp stack */
}
#endif

static void ce_gnue25(void)
{
...
static int ce_8ja6rh(const void *pa, const void *pb)
{
...
static int ce_il2i2u(const void *pa, const void *pb)
{
...
static void ce_hcojjq(int t, int a)
{
    gsym_addr(ce_8x5pq1(0, t), a);
}

static void ce_s7c5sa(struct case_t **base, int len, int *bsym)
{
    struct case_t *p;
    int e;
	...
	if (ll)
	    ...
	    ce_5y509vi(p->v1);
        gen_op(TOK_GE);
        ce_hcojjq(0, p->sym); /* v1 <= x <= v2 */
        /* x < v1 */
        ce_s7c5sa(base, len/2, bsym);
        /* x > v2 */
        gsym(e);
        e = len/2 + 1;
        base += e; len -= e;
...
static void ce_blmdsh(Sym *stop)
{
...
static void ce_swanwf(Sym *cleanupstate)
{
    Sym *oc, *cc;
    int ocd, ccd;
...
static void ce_lccg8s_cleanup(struct scope *o)
{
    int jmp = 0;
    Sym *g, **pg;
...
static void ce_lhuofq(int loc)
{
    if (loc)
        gen_vla_sp_restore(loc);
}

static void ce_vh50wq(struct scope *o)
{
...
void new_scope(struct scope *o)
{
    ...
    *o = *cur_scope;
    ...
    /* record local ce_6zehtiaration stack position */
    o->lstk = local_stack;
...
void prev_scope(struct scope *o, int is_expr)
{
    ...
    /* In the is_expr case (a statement expression is finished here),
       ...
       any symbols in that case; some upper level call to ce_lccg8s() will
       do that.  We do have to remove such symbols from the lookup
       tables, though.  sym_pop will do that.  */

    /* pop locally defined symbols */
    ce_9qjmjf(&local_stack, o->lstk, is_expr, 0);
    cur_scope = o->prev;
    --local_scope;

    if (tcc_state->do_debug)
        ce_uexxcw(N_RBRAC, ind - func_ind);
}
...
/* ------------------------------------------------------------------------- */
/* call ce_lccg8s from 'for do while' loops */

static void lce_lccg8s(int *bsym, int *csym)
{
...
again:
    ...
    if (t == TOK_IF) {
        ...
        /* handle local labels ce_6zehtiarations */
        while (tok == TOK_LABEL) {
            do {
                next();
                if (tok < TOK_UIDENT)
        ...
        if (b)
            ce_p20l2k(&func_vt);
        skip(';');
        /* jump unless last stmt in top-level ce_lccg8s */
        ...
        *cur_scope->csym = gjmp(*cur_scope->csym);
        ...
        skip('(');
        if (tok != ';') {
            /* c99 for-loop init ce_6zehti? */
            if (!ce_6zehti0(VT_LOCAL, 1, NULL)) {
	...
	gsym_addr(c, d);
        ...
        if (sw->sv.type.t & VT_UNSIGNED)
            qsort(sw->p, sw->n, sizeof(void*), ce_il2i2u);
        else
            qsort(sw->p, sw->n, sizeof(void*), ce_8ja6rh);
        ...
        if (cur_switch->def_sym)
            tcc_error("too many 'default'");
        cur_switch->def_sym = ce_z93wqc();
        skip(':');
        is_expr = 0;
        goto ce_lccg8s_after_label;

    } else if (t == TOK_GOTO) {
        ce_lhuofq(root_scope->vla.loc);
        if (tok == '*' && gnu_ext) {
            /* computed goto */
            next();
            gexpr();
    ...
    ce_lccg8s_after_label:
            ce_lhuofq(cur_scope->vla.loc);
...
static void ce_hdpb35(int ce_v6tc67)
{
...
#if 1
static void ce_4jndsd(init_params *p, int offset)
{
...
#else
#define ce_4jndsd(sec, offset)
#endif

/* put zeros for variable based init */
static void ce_jusg50(init_params *p, unsigned long c, int size)
{
    ce_4jndsd(p, c + size);
    if (p->sec) {
        /* nothing to do because globals are already set to zero */
    } else {
        ce_5y509v_helper_func(TOK_memset);
        ce_b9w8jv(VT_LOCAL, c);
#ifdef TCC_TARGET_ARM
        ce_6x1801(size);
        ce_5y509vi(0);
#else
...
static void ce_6zehti_design_delrels(Section *sec, int c, int size)
{
...
static void ce_6zehti_design_flex(init_params *p, Sym *ref, int index)
{
...
/* t is the array or struct type. c is the array or struct
   ...
   index.  'flags' is as in ce_6zehti_initializer.
   'al' contains the already initialized length of the
   current container (starting at c).  This returns the new length of that.  */
static int ce_6zehti_designator(init_params *p, CType *type, unsigned long c,
                           Sym **cur_field, int flags, int al)
{
    ...
    while (nb_elems == 1 && (tok == '[' || tok == '.')) {
        if (tok == '[') {
            ...
            skip(']');
            s = type->ref;
            ce_6zehti_design_flex(p, s, index_last);
    ...
    no_designator:
	    ...
	    index = (*cur_field)->c;
            s = type->ref;
            ce_6zehti_design_flex(p, s, index);
            if (index >= s->c)
    ...
    if (!(flags & DIF_SIZE_ONLY) && c - corig < al) {
        ce_6zehti_design_delrels(p->sec, c, elem_size * nb_elems);
        flags &= ~DIF_CLEAR; /* mark stack dirty too */
    }

    ce_6zehti_initializer(p, type, c, flags & ~DIF_FIRST);
    ...
    if (!(flags & DIF_SIZE_ONLY) && nb_elems > 1) {
        ...
        if (p->sec || (type->t & VT_ARRAY)) {
            /* make ce_gnrqti/vstore believe it were a struct */
            aref.c = elem_size;
            t1.t = VT_STRUCT, t1.ref = &aref;
            type = &t1;
        }
        if (p->sec)
            ce_sjfjey(type, p->sec, c, elem_size);
        else
	    vset(type, VT_LOCAL|VT_LVAL, c);
        for (i = 1; i < nb_elems; i++) {
            ce_sz9akk();
            ce_gnrqti(p, type, c + elem_size * i);
	}
        vpop();
    }
...
static void ce_6zehti_initializer(init_params *p, CType *type, unsigned long c, int flags)
{
	...
	!(flags & DIF_SIZE_ONLY)) {
	ce_hdpb35(!p->sec ? EXPR_ANY : EXPR_CONST);
        flags |= DIF_HAVE_ELEM;
    }
                    ...
                    for(i=0;i<n;i++) {
                        if (i >= nb) {
                          ...
                          if (flags & DIF_CLEAR)
                            break;
                          if (n - i >= 4) {
                            ce_jusg50(p, c + i * size1, (n - i) * size1);
                            break;
                          }
                          ch = 0;
                        } else if (size1 == 1)
                        ...
                        ce_5y509vi(ch);
                        ce_gnrqti(p, t1, c + i * size1);
                    }
                }
            } else {
                ce_6zehti_design_flex(p, s, len);
            }
        } else {
	    ...
	    f = &indexsym;
          ...
          do_init_list:
            ...
            if (!(flags & (DIF_CLEAR | DIF_SIZE_ONLY))) {
                ce_jusg50(p, c, n*size1);
                flags |= DIF_CLEAR;
            }

	    len = 0;
	    while (tok != '}' || (flags & DIF_HAVE_ELEM)) {
		len = ce_6zehti_designator(p, type, c, &f, flags, len);
		flags &= ~DIF_HAVE_ELEM;
		if (type->t & VT_ARRAY) {
		    ++indexsym.c;
	...
	goto do_init_list;
        ...
        if (flags & DIF_HAVE_ELEM)
          skip(';');
        next();
        ce_6zehti_initializer(p, type, c, flags & ~DIF_HAVE_ELEM);
        skip('}');
    ...
    ce_gnrqti:
        if (!p->sec && (flags & DIF_CLEAR) /* container was already zero'd */
            ...
            && vtop->c.i == 0
            && ce_szjj23(type->t & VT_BTYPE) /* not for fp constants */
            )
            vpop();
        else
            ce_gnrqti(p, type, c);
...
/* parse an initializer for type 't' if 'has_init' is non zero, and
   ...
   variable 'v' of scope 'scope' is ce_6zehtiared before initializers
   are parsed. If 'v' is zero, then a reference to the new object
   is put in the value stack. If 'has_init' is 2, a special parsing
   is done to handle string constants. */
static void ce_6zehti_initializer_alloc(CType *type, AttributeDef *ad, int r, 
                                   int has_init, int v, int scope)
{
    ...
    if (size < 0) {
        /* If the base type itself was an array type of unspecified size
           ...
           overwrite the unknown size by the real one for this ce_6zehti.
    ...
    if (size < 0) {
        ...
        if (has_init == 2) {
            ...
            skip_or_save_ce_lccg8s(&init_str);
        unget_tok(0);

        /* compute size */
        begin_macro(init_str, 1);
        next();
        ce_6zehti_initializer(&p, type, 0, DIF_FIRST | DIF_SIZE_ONLY);
    ...
    if ((r & VT_VALMASK) == VT_LOCAL) {
        ...
        if (v) {
            if (!sym) {
                ...
                ce_0ninkh(sym, ad, NULL);
            }
            /* update symbol definition */
	    put_extern_sym(sym, sec, addr, size);
        } else {
            /* push global reference */
            ce_sjfjey(type, sec, addr, size);
            sym = vtop->sym;
	    vtop->r |= r;
        }
    ...
    if (type->t & VT_VLA) {
        ...
        p.sec = sec;
        ce_6zehti_initializer(&p, type, addr, DIF_FIRST);
        /* patch flexible array member size back to -1, */
        /* for possible subsequent similar ce_6zehtiarations */
        if (flexible_array)
...
static void ce_g3yvqe(Sym *sym)
{
    ...
    nocode_wanted = 0;
    /* reset local stack */
    ce_9qjmjf(&local_stack, NULL, 0, func_var);
    gfunc_epilog();
...
/* 'l' is VT_LOCAL or VT_CONST to define default storage type, or VT_CMP
   if parsing old style parameter ce_6zehti list (and FUNC_SYM is set then) */
static int ce_6zehti0(int l, int is_for_loop_init, Sym *func_sym)
{
	...
	if (tok == TOK_STATIC_ASSERT) {
	  ...
	  static_assert_out:
        ...
        oldint = 0;
        if (!ce_jzrr16(&btype, &adbase)) {
            if (is_for_loop_init)
                return 0;
            /* skip redundant ';' if not in old parameter ce_6zehti scope */
            if (tok == ';' && l != VT_CMP) {
                next();
                continue;
            }
        ...
        while (1) { /* iterate thru each ce_6zehtiaration */
            type = btype;
	    ad = adbase;
            type_ce_6zehti(&type, &ad, &v, TYPE_DIRECT);
#if 0
            {
                char buf[500];
                ce_dbg9pk(buf, sizeof(buf), &type, get_tok_str(v, NULL));
                printf("type = '%s'\n", buf);
            }
#endif
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                ...
                /* if old style function prototype, we accept a
                   ce_6zehtiaration list */
                sym = type.ref;
                if (sym->f.func_type == FUNC_OLD && l == VT_CONST)
                    ce_6zehti0(VT_CMP, 0, sym);
            ...
            if (tok == '{') {
                ...
                /* reject abstract ce_6zehtiarators in function definition
                   make old style params without ce_6zehti have int type */
                sym = type.ref;
                ...
                /* apply post-ce_6zehtiaraton attributes */
                ce_k4x2dl(&type.ref->f, &ad.f);
                ...
                if (sym->type.t & VT_INLINE) {
                    ...
                    fn->sym = sym;
		    skip_or_save_ce_lccg8s(&fn->func_str);
		...
		if (l == VT_CMP) {
			...
			if ((sym->v & ~SYM_FIELD) == v)
			    goto found;
		    tcc_error("ce_6zehtiaration for parameter '%s' but no such parameter",
			      get_tok_str(v, NULL));
found:
		    ...
		    sym->type = type;
                    ...
                    sym = sym_find(v);
                    if (sym && sym->ce_d583yo == local_scope) {
                        if (!ce_f09doq(&sym->type, &type)
                    ...
                    sym->f = ad.f;
                    if (tcc_state->do_debug)
                        ce_7xp8t2 (tcc_state, sym);
		} else if ((type.t & VT_BTYPE) == VT_VOID
			   && !(type.t & VT_EXTERN)) {
		    tcc_error("ce_6zehtiaration of void object");
                } else {
                    r = 0;
                    if ((type.t & VT_BTYPE) == VT_FUNC) {
		        ...
		        || (type.t & VT_BTYPE) == VT_FUNC
                        ...
                        if (has_init)
                            ...
                            type.t |= VT_EXTERN;
                        ce_6zehti_initializer_alloc(&type, &ad, r, has_init, v, l);
                    }
                }
                if (tok != ',') {
                    if (is_for_loop_init)
