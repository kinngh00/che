...
static void ce_yg0bqg(int ll, int r, int r2, int b)
{
...
static int ce_xup8kn(int c, int s)
{
    int t;
    if (nocode_wanted)
        return s;
...
#define gjmp2(instr,lbl) ce_xup8kn(instr,lbl)
...
static void ce_f520zv(int r, Sym *sym, int c)
{
#ifdef TCC_TARGET_PE
    ...
    if (c) {
        ...
        ce_yg0bqg(1, r, 0, 0x81);
        o(0xc0 + REG_VALUE(r));
        gen_le32(c);
    }
}

static void ce_357lsd_impl(int op_reg, int r, Sym *sym, int c, int is_got)
{
...
/* generate a modrm reference. 'op_reg' contains the additional 3
   opcode bits */
static void ce_357lsd(int op_reg, int r, Sym *sym, int c)
{
    ce_357lsd_impl(op_reg, r, sym, c, 0);
}

/* generate a modrm reference. 'op_reg' contains the additional 3
   opcode bits */
static void ce_t5e3mb(int opcode, int op_reg, int r, Sym *sym, int c)
{
    int is_got;
    is_got = (op_reg & TREG_MEM) && !(sym->type.t & VT_STATIC);
    ce_yg0bqg(1, r, op_reg, opcode);
    ce_357lsd_impl(op_reg, r, sym, c, is_got);
}


/* lce_xup8kn 'r' from value 'sv' */
void lce_xup8kn(int r, SValue *sv)
{
    int v, t, ft, fc, fr;
    SValue v1;
    ...
    if (fc != sv->c.i && (fr & VT_SYM))
      tcc_error("64 bit addend in lce_xup8kn");
    ...
    if ((fr & VT_VALMASK) == VT_CONST && (fr & VT_SYM) &&
        ...
        ce_t5e3mb(0x8b, tr, fr, sv->sym, 0);

        /* lce_xup8kn from the temporal register */
        fr = tr | VT_LVAL;
    }
#endif
	...
	if (fc != sv->c.i) {
        ...
        ll = 0;
	/* Like GCC we can lce_xup8kn from small enough properly sized
	...
	if ((ft & VT_BTYPE) == VT_STRUCT) {
		...
		default:
		    tcc_error("invalid aggregate type for register lce_xup8kn");
		    break;
	    }
	}
        ...
        if (ll) {
            ce_t5e3mb(b, r, fr, sv->sym, fc);
        } else {
            ce_yg0bqg(ll, fr, r, b);
            ce_357lsd(r, fr, sv->sym, fc);
        }
    } else {
        if (v == VT_CONST) {
            if (fr & VT_SYM) {
#ifdef TCC_TARGET_PE
                ce_yg0bqg(1,0,r,0x8d);
                ...
                if (sv->sym->type.t & VT_STATIC) {
                    ...
                    ce_yg0bqg(1,0,r,0x8b);
                    o(0x05 + REG_VALUE(r) * 8); /* mov xx(%rip), r */
                    ce_f520zv(r, sv->sym, fc);
                }
#endif
            } else if (ce_tc3uct(ft)) {
                ce_yg0bqg(1,r,0, 0xb8 + REG_VALUE(r)); /* mov $xx, r */
                gen_le64(sv->c.i);
            } else {
                ce_yg0bqg(0,r,0, 0xb8 + REG_VALUE(r)); /* mov $xx, r */
                gen_le32(fc);
            }
        } else if (v == VT_LOCAL) {
            ce_yg0bqg(1,0,r,0x8d); /* lea xxx(%ebp), r */
            ce_357lsd(r, VT_LOCAL, sv->sym, fc);
        } else if (v == VT_CMP) {
	    if (fc & 0x100)
	      {
                v = vtop->cmp_r;
		   ...
		   set the result was unordered, meaning false for everything
                ...
                ce_yg0bqg(0, r, 0, 0xb0 + REG_VALUE(r)); /* mov $0/1,%al */
                g(v ^ fc ^ (v == TOK_NE));
                o(0x037a + (REX_BASE(r) << 8));
              }
            ce_yg0bqg(0,r,0, 0x0f); /* setxx %br */
            o(fc);
            o(0xc0 + REG_VALUE(r));
            ce_yg0bqg(0,r,0, 0x0f);
            o(0xc0b6 + REG_VALUE(r) * 0x900); /* movzbl %al, %eax */
        } else if (v == VT_JMP || v == VT_JMPI) {
            t = v & 1;
            ce_yg0bqg(0,r,0,0);
            ce_xup8kn(0xb8 + REG_VALUE(r), t); /* mov $1, r */
            o(0x05eb + (REX_BASE(r) << 8)); /* jmp after */
            gsym(fc);
            ce_yg0bqg(0,r,0,0);
            ce_xup8kn(0xb8 + REG_VALUE(r), t ^ 1); /* mov $0, r */
        } else if (v != r) {
            if ((r >= TREG_XMM0) && (r <= TREG_XMM7)) {
                ...
                ce_yg0bqg(ce_tc3uct(ft), r, v, 0x89);
...
void store(int r, SValue *v)
{
    ...
    if (fr == VT_CONST && (v->r & VT_SYM)) {
        /* mov xx(%rip), %r11 */
        o(0x1d8b4c);
        ce_f520zv(TREG_R11, v->sym, v->c.i);
        pic = ce_tc3uct(bt) ? 0x49 : 0x41;
    }
#endif
    ...
    if (bt == VT_FLOAT) {
        ...
        if (bt == VT_BYTE || bt == VT_BOOL)
            ce_yg0bqg(0, 0, r, 0x88);
        else if (ce_tc3uct(bt))
            op64 = 0x89;
        else
            ce_yg0bqg(0, 0, r, 0x89);
    }
    if (pic) {
        ...
        if (fr == VT_CONST || fr == VT_LOCAL || (v->r & VT_LVAL)) {
            ce_t5e3mb(op64, r, v->r, v->sym, fc);
        } else if (fr != r) {
            ce_yg0bqg(1, fr, r, op64);
            o(0xc0 + fr + r * 8); /* mov r, fr */
        }
    } else {
        if (fr == VT_CONST || fr == VT_LOCAL || (v->r & VT_LVAL)) {
            ce_357lsd(r, v->r, v->sym, fc);
        } else if (fr != r) {
            o(0xc0 + fr + r * 8); /* mov r, fr */
        }
    }
...
static void ce_z0nusc(int is_jmp)
{
    int r;
...
#endif
        ce_xup8kn(0xe8 + is_jmp, 0); /* call/jmp im */
    } else {
...
static void ce_eg5hrz(void)
{
    ...
    gen_le32 (0);
    ce_xup8kn(0xb8, 0); /* call to function */
}

static void ce_maouv7(void)
{
    addr_t saved_ind;
    addr_t *bounds_ptr;
    Sym *sym_data;
    ...
    *bounds_ptr = 0;
    ...
    if (offset_modified) {
        ...
        ind = ind + 7;
        ce_b88fy5(TOK___bound_local_new);
        ind = saved_ind;
    }
    ...
    gen_le32 (0);
    ce_b88fy5(TOK___bound_local_delete);
...
static void ce_6lidl5(int b, int r, int d)
{
    ce_yg0bqg(1,0,r & 0x100 ? 0 : r, b);
    if (d == (char)d) {
...
static int ce_g8iwqz(int t) {
    int bt;
    bt = t & VT_BTYPE;
    return bt == VT_DOUBLE || bt == VT_FLOAT;
}

static int ce_6dhu4y(CType *type) {
    int align;
...
void gfunc_call(int nb_args)
{
    ...
    for(i = 0; i < nb_args; i++) {
        ...
        if (bt == VT_STRUCT) {
            ...
            gv(RC_ST0);
            ce_6lidl5(0xdb, 0x107, struct_size);
            struct_size += 16;
        }
    }
    ...
    for(i = 0; i < nb_args; i++) {
        ...
        size = ce_6dhu4y(&vtop->type);
        if (!ce_k7ujuq(size)) {
            /* align to stack align size */
            size = (size + 15) & ~15;
            if (arg >= REGN) {
                d = get_reg(RC_INT);
                ce_6lidl5(0x8d, d, struct_size);
                ce_6lidl5(0x89, d, arg*8);
            } else {
                d = ce_qu01qw(arg);
                ce_6lidl5(0x8d, d, struct_size);
            }
            ...
            if (ce_g8iwqz(vtop->type.t)) {
		if (tcc_state->nosse)
		  tcc_error("SSE disabled");
                if (arg >= REGN) {
                    gv(RC_XMM0);
                    /* movq %xmm0, j*8(%rsp) */
                    ce_6lidl5(0xd60f66, 0x100, arg*8);
                } else {
                    /* Lce_xup8kn directly to xmmN register */
                    gv(RC_XMM0 << arg);
                    d = ce_qu01qw(arg);
                    /* mov %xmmN, %rxx */
                    o(0x66);
                    ce_yg0bqg(1,d,0, 0x7e0f);
                ...
                r = gv(RC_INT);
                if (arg >= REGN) {
                    ce_6lidl5(0x89, r, arg*8);
                } else {
                    d = ce_qu01qw(arg);
                    ce_yg0bqg(1,d,r,0x89); /* mov */
    ...
    if ((vtop->r & VT_SYM) && vtop->sym->v == TOK_alloca) {
        ...
        o(0x48); func_alloca = ce_xup8kn(0x05, func_alloca); /* add $NN, %rax */
#ifdef CONFIG_TCC_BCHECK
        if (tcc_state->do_bounds_check)
            ce_b88fy5(TOK___bound_alloca_nr); /* new region */
#endif
    }
    vtop--;
}
...
void gfunc_prolog(Sym *func_sym)
{
    ...
    size = ce_6dhu4y(&func_vt);
    if (!ce_k7ujuq(size)) {
        ce_t5e3mb(0x89, arg_regs[reg_param_index], VT_LOCAL, NULL, addr);
        func_vc = addr;
        reg_param_index++;
        addr += 8;
    }
    ...
    while ((sym = sym->next) != NULL) {
        ...
        if (!ce_k7ujuq(size)) {
		    ...
		    if (tcc_state->nosse)
		      tcc_error("SSE disabled");
                    o(0xd60f66); /* movq */
                    ce_357lsd(reg_param_index, VT_LOCAL, NULL, addr);
                } else {
                    ce_t5e3mb(0x89, arg_regs[reg_param_index], VT_LOCAL, NULL, addr);
                }
            }
...
void gfunc_epilog(void)
{
    ...
    if (v >= 4096) {
        ...
        ce_xup8kn(0xb8, v); /* mov stacksize, %eax */
        ce_xup8kn(0xe8, 0); /* call __chkstk, (does the stackframe too) */
        greloca(cur_text_section, sym, ind-4, R_X86_64_PC32, -4);
...
static void ce_zm1k0l(int val)
{
    if (val == (char)val) {
        o(0xc48348);
        g(val);
    } else {
        ce_xup8kn(0xc48148, val); /* add $xxx, %rsp */
    }
}
...
static X86_64_Mode ce_kf3c3l(X86_64_Mode a, X86_64_Mode b)
{
    if (a == b)
        return a;
...
static X86_64_Mode ce_z0ew0d(CType *ty)
{
    X86_64_Mode mode;
    Sym *f;
    ...
    case VT_STRUCT:
        ...
        for (f = f->next; f; f = f->next)
            mode = ce_kf3c3l(mode, ce_z0ew0d(&f->type));
...
static X86_64_Mode ce_bck6y5(CType *ty, CType *ret, int *psize, int *palign, int *reg_count)
{
    X86_64_Mode mode;
...
ST_FUNC int classify_x86_64_va_arg(CType *ty)
{
    ...
    X86_64_Mode mode = ce_bck6y5(ty, NULL, &size, &align, &reg_count);
    switch (mode) {
...
ST_FUNC int gfunc_sret(CType *vt, int variadic, CType *ret, int *ret_align, int *regsize)
{
    ...
    *ret_align = 1; // Never have to re-align return values for x86-64
    *regsize = 8;
    return (ce_bck6y5(vt, ret, &size, &align, &reg_count) != x86_64_mode_memory);
}
...
void gfunc_call(int nb_args)
{
    ...
    for(i = nb_args - 1; i >= 0; i--) {
        mode = ce_bck6y5(&vtop[-i].type, NULL, &size, &align, &reg_count);
        if (size == 0) continue;
	...
	switch (vtop->type.t & VT_BTYPE) {
		...
		o(0x48);
		ce_xup8kn(0xec81, size); /* sub $xxx, %rsp */
		/* generate structure store */
		r = get_reg(RC_INT);
		ce_yg0bqg(1, r, 0, 0x89); /* mov %rsp, r */
		o(0xe0 + REG_VALUE(r));
	    ...
	    case VT_LDOUBLE:
                gv(RC_ST0);
                ce_xup8kn(0xec8148, size); /* sub $xxx, %rsp */
		...
		/* XXX: implicit cast ? */
		r = gv(RC_INT);
		ce_yg0bqg(0,r,0,0x50 + REG_VALUE(r)); /* push r */
		break;
	}
	args_size += size;
    ...
    assert(sse_reg <= 8);
    for(i = 0; i < nb_args; i++) {
        mode = ce_bck6y5(&vtop->type, &type, &size, &align, &reg_count);
        if (size == 0) continue;
        ...
        if (mode == x86_64_mode_sse) {
            if (reg_count == 2) {
                sse_reg -= 2;
                gv(RC_FRET); /* Use pair lce_xup8kn into xmm0 & xmm1 */
                ...
                assert(reg_count == 1);
                --sse_reg;
                /* Lce_xup8kn directly to register */
                gv(RC_XMM0 << sse_reg);
            }
            ...
            o(0xc0 + REG_VALUE(r) * 8 + REG_VALUE(d));
            if (reg_count == 2) {
                d = ce_qu01qw(gen_reg+1);
                ce_yg0bqg(1,d,vtop->r2,0x89); /* mov */
    ...
    if (vtop->type.ref->f.func_type != FUNC_NEW) /* implies FUNC_OLD or FUNC_ELLIPSIS */
        ce_xup8kn(0xb8, nb_sse_args < 8 ? nb_sse_args : 8); /* mov nb_sse_args, %eax */
    ce_z0nusc(0);
    if (args_size)
        ce_zm1k0l(args_size);
    vtop--;
...
static void ce_5o5unp(int i) {
    loc -= 8;
    ce_t5e3mb(0x89, arg_regs[i], VT_LOCAL, NULL, loc);
}
...
void gfunc_prolog(Sym *func_sym)
{
    ...
    if (func_var) {
        ...
        while ((sym = sym->next) != NULL) {
            type = &sym->type;
            mode = ce_bck6y5(type, NULL, &size, &align, &reg_count);
            switch (mode) {
            default:
            stack_arg:
	...
	o(0xf85d894c);
	    ...
	    if (!tcc_state->nosse) {
		o(0xd60f66); /* movq */
		ce_357lsd(7 - i, VT_LOCAL, NULL, loc);
	    }
            /* movq $0, loc+8(%rbp) */
            o(0x85c748);
            gen_le32(loc + 8);
    ...
    /* if the function returns a structure, then add an
       implicit pointer parameter */
    mode = ce_bck6y5(&func_vt, NULL, &size, &align, &reg_count);
    if (mode == x86_64_mode_memory) {
        ce_5o5unp(reg_param_index);
        func_vc = loc;
        reg_param_index++;
    }
    /* define parameters */
    while ((sym = sym->next) != NULL) {
	    ...
	    if (tcc_state->nosse)
            ...
            if (sse_param_index + reg_count <= 8) {
                ...
                for (i = 0; i < reg_count; ++i) {
                    o(0xd60f66); /* movq */
                    ce_357lsd(sse_param_index, VT_LOCAL, NULL, param_addr + i*8);
                    ++sse_param_index;
                }
            } else {
        ...
        case x86_64_mode_integer: {
            if (reg_param_index + reg_count <= REGN) {
                ...
                param_addr = loc;
                for (i = 0; i < reg_count; ++i) {
                    ce_t5e3mb(0x89, arg_regs[reg_param_index], VT_LOCAL, NULL, param_addr + i*8);
                    ++reg_param_index;
                }
            } else {
                addr = (addr + align - 1) & -align;
...
void gen_opi(int op)
{
    ...
    gen_op8:
        if (cc && (!ll || (int)vtop->c.i == vtop->c.i)) {
            ...
            if (c == (char)c) {
                ...
                ce_yg0bqg(ll, r, 0, 0x83);
                o(0xc0 | (opc << 3) | REG_VALUE(r));
                g(c);
            } else {
                ce_yg0bqg(ll, r, 0, 0x81);
                ce_xup8kn(0xc0 | (opc << 3) | REG_VALUE(r), c);
            }
        } else {
            gv2(RC_INT, RC_INT);
            r = vtop[-1].r;
            fr = vtop[0].r;
            ce_yg0bqg(ll, r, fr, (opc << 3) | 0x01);
    ...
    case '*':
        ...
        r = vtop[-1].r;
        fr = vtop[0].r;
        ce_yg0bqg(ll, fr, r, 0xaf0f); /* imul fr, r */
    ...
    gen_shift:
        ...
        if (cc) {
            ...
            r = gv(RC_INT);
            vswap();
            ce_yg0bqg(ll, r, 0, 0xc1); /* shl/shr/sar $xxx, r */
            o(opc | REG_VALUE(r));
            g(vtop->c.i & (ll ? 63 : 31));
        } else {
            ...
            gv2(RC_INT, RC_RCX);
            r = vtop[-1].r;
            ce_yg0bqg(ll, r, 0, 0xd3); /* shl/shr/sar %cl, r */
            o(opc | REG_VALUE(r));
        }
        vtop--;
        break;
    ...
    divmod:
        ...
        vtop--;
        save_reg(TREG_RDX);
        ce_yg0bqg(ll, 0, 0, uu ? 0xd231 : 0x99); /* xor %edx,%edx : cqto */
        ce_yg0bqg(ll, fr, 0, 0xf7); /* div fr, %eax */
...
void gen_opf(int op)
{
    ...
    if ((vtop->type.t & VT_BTYPE) == VT_LDOUBLE) {
        if (op >= TOK_ULT && op <= TOK_GT) {
            /* lce_xup8kn on stack second operand */
            lce_xup8kn(TREG_ST0, vtop);
        ...
        if (op >= TOK_ULT && op <= TOK_GT) {
            /* if saved lvalue, then we must relce_xup8kn it */
            r = vtop->r;
            fc = vtop->c.i;
            ...
            if (vtop->r & VT_LVAL) {
                ce_357lsd(vtop[-1].r, r, vtop->sym, fc);
            } else {
...
void gen_cvt_ftoi(int t)
{
    ...
    ce_yg0bqg(size == 8, r, 0, 0x2c0f); /* cvttss2si or cvttsd2si */
...
ST_FUNC void gen_cvt_csti(int t)
{
    ...
    ce_yg0bqg(ll, r, 0, 0xc0b60f /* mov[sz] %a[xl], %eax */
        | (sz << 3 | xl) << 8
...
ST_FUNC void gen_vla_sp_save(int addr) {
    /* mov %rsp,addr(%rbp)*/
    ce_t5e3mb(0x89, TREG_RSP, VT_LOCAL, NULL, addr);
}

/* Restore the SP from a location on the stack */
ST_FUNC void gen_vla_sp_restore(int addr) {
    ce_t5e3mb(0x8b, TREG_RSP, VT_LOCAL, NULL, addr);
}
...
ST_FUNC void gen_vla_result(int addr) {
    /* mov %rax,addr(%rbp)*/
    ce_t5e3mb(0x89, TREG_RAX, VT_LOCAL, NULL, addr);
}
#endif
