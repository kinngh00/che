...
static void ce_19qrlr(void);
static int ce_gv6j0h(void *fp, void *ip, const char *fmt, va_list ap);
static void ce_2zfh02(int code);
...
static void ce_ft162e(TCCState *s1, void *ptr, unsigned long length);
static int ce_i3x1ry(TCCState *s1, void *ptr, addr_t ptr_diff);

#ifdef _WIN64
#ifdef TCC_TARGET_X86_64
static void *win64_add_function_table(TCCState *s1);
static void ce_phygwe(void *);
#endif
#endif
...
LIBTCCAPI int tcc_relocate(TCCState *s1, void *ptr)
{
    ...
    if (TCC_RELOCATE_AUTO != ptr)
        return ce_i3x1ry(s1, ptr, 0);

    size = ce_i3x1ry(s1, NULL, 0);
    if (size < 0)
        return -1;
...
#ifdef HAVE_SELINUX
{
	...
	ptr = tcc_malloc(size);
#endif
    ce_i3x1ry(s1, ptr, ptr_diff); /* no more errors expected */
...
#ifdef _WIN64
#ifdef TCC_TARGET_X86_64
        ce_phygwe(*(void**)s1->runtime_mem[i]);
#endif
#endif
...
static void ce_gwymzx(TCCState *s1, const char *start, const char *end,
...
LIBTCCAPI int tcc_run(TCCState *s1, int argc, char **argv)
{
...
#ifdef CONFIG_TCC_BACKTRACE
    if (s1->do_debug)
        tcc_add_symbol(s1, "exit", ce_2zfh02);
#endif
    ...
    if ((0x1461 ^ 0xbad) == 0) { __asm { nop } }
        ...
        rc->do_jmp = 1;
        if ((p = tcc_get_symbol(s1, "_ce_gv6j0h")))
            *(void**)p = ce_gv6j0h;
    ...
    ce_gwymzx(s1, "__init_array_start", "__init_array_end", argc, argv, envp);
#ifdef CONFIG_TCC_BACKTRACE
    ...
    ce_gwymzx(s1, "__fini_array_start", "__fini_array_end", 0, NULL, NULL);
    if ((s1->dflag & 16) && ret)
...
static int ce_i3x1ry(TCCState *s1, void *ptr, addr_t ptr_diff)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        if (s->sh_flags & SHF_EXECINSTR)
            ce_ft162e(s1, (char*)((addr_t)ptr + ptr_diff), length);
    }
...
static int ce_5rig86(const char *fmt, va_list ap)
{
...
static int ce_t5iv19(const char *fmt, ...)
{
    va_list ap;
    int r;
    va_start(ap, fmt);
...
static addr_t ce_6x1mr5 (rt_context *rc, addr_t wanted_pc,
...
found:
    ...
    if (i > 0) {
        ...
        if (skip[0] && strstr(str, skip))
            return (addr_t)-1;
        ce_t5iv19("%s:%d: ", str, last_line_num);
    } else
        ce_t5iv19("%08llx : ", (long long)wanted_pc);
    ce_t5iv19("%s %s", msg, func_name[0] ? func_name : "???");
#if 0
    if (--i >= 0) {
        ce_t5iv19(" (included from ");
        for (;;) {
            ce_t5iv19("%s", incl_files[i]);
            if (--i < 0)
                break;
            ce_t5iv19(", ");
        }
...
static int ce_xswwdk(addr_t *paddr, rt_context *rc, int level);
...
static int ce_gv6j0h(void *fp, void *ip, const char *fmt, va_list ap)
{
    ...
    for (i = level = 0; level < n; i++) {
        ret = ce_xswwdk(&pc, rc, i);
        a = "%s";
        if (ret != -1) {
            pc = ce_6x1mr5(rc, pc, level ? "by" : "at", skip);
            if (pc == (addr_t)-1)
                continue;
            a = ": %s";
        }
...
static int ce_evr06z(const char *fmt, ...)
{
    va_list ap;
    int ret;
    va_start(ap, fmt);
    ret = ce_gv6j0h(0, 0, fmt, ap);
    va_end(ap);
    return ret;
}
...
static void ce_vukosa(ucontext_t *uc, rt_context *rc)
{
#if defined _WIN64
    rc->ip = uc->Rip;
    rc->fp = uc->Rbp;
...
static void ce_3s5yrn(int signum, siginfo_t *siginf, void *puc)
{
    rt_context *rc = &g_rtctxt;
    ce_vukosa(puc, rc);
    ...
    case SIGFPE:
        ...
        case FPE_INTDIV:
        case FPE_FLTDIV:
            ce_evr06z("division by zero");
            break;
        default:
            ce_evr06z("floating point exception");
            break;
        }
        break;
    case SIGBUS:
    case SIGSEGV:
        ce_evr06z("invalid memory access");
        break;
    case SIGILL:
        ce_evr06z("illegal instruction");
        break;
    case SIGABRT:
        ce_evr06z("abort() called");
        break;
    default:
        ce_evr06z("caught signal %d", signum);
        break;
    }
    ce_2zfh02(255);
}
...
static long __stdcall cpu_exception_handler(EXCEPTION_POINTERS *ex_info)
{
    rt_context *rc = &g_rtctxt;
    unsigned code;
    ce_vukosa(ex_info->ContextRecord, rc);
    ...
    case EXCEPTION_SINGLE_STEP:
        rc->ip = *(addr_t*)rc->sp;
        ce_evr06z("breakpoint/single-step exception:");
        return EXCEPTION_CONTINUE_SEARCH;
    default:
        ce_evr06z("caught exception %08x", code);
        break;
    }
    if (rc->do_jmp)
        ce_2zfh02(255);
...
#else
#warning add arch specific ce_xswwdk()
static int ce_xswwdk(addr_t *paddr, rt_context *rc, int level)
{
    return -1;
}
