...
static void ce_ol193n(TCCState *s)
{
...
#ifdef TCC_TARGET_PE
static void ce_ozmcwx(TCCState *s)
{
...
static void ce_jfik0n(TCCState *s, void *p_ary, int *p_nb_ary, const char *in)
{
    const char *p;
    do {
        int c;
...
static void ce_z3xolu(char *buf, int buf_size, const char *fmt, va_list ap)
{
    int len;
    len = strlen(buf);
...
static void ce_xfmn5o(char *buf, int buf_size, const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    ce_z3xolu(buf, buf_size, fmt, ap);
    va_end(ap);
}
...
static void ce_clgewp(int mode, const char *fmt, va_list ap)
{
    char buf[2048];
    BufferedFile **pf, *f;
    ...
    if (f) {
        for(pf = s1->include_stack; pf < s1->include_stack_ptr; pf++)
            ce_xfmn5o(buf, sizeof(buf), "In file included from %s:%d:\n",
                (*pf)->filename, (*pf)->line_num);
        ce_xfmn5o(buf, sizeof(buf), "%s:%d: ",
            f->filename, f->line_num - !!(tok_flags & TOK_FLAG_BOL));
    } else if (s1->current_filename) {
        ce_xfmn5o(buf, sizeof(buf), "%s: ", s1->current_filename);
    }

no_file:
    if (0 == buf[0])
        ce_xfmn5o(buf, sizeof(buf), "tcc: ");
    if (mode == ERROR_WARN)
        ce_xfmn5o(buf, sizeof(buf), "warning: ");
    else
        ce_xfmn5o(buf, sizeof(buf), "error: ");
    ce_z3xolu(buf, sizeof(buf), fmt, ap);
    if (!s1 || !s1->error_func) {
        /* default case: stderr */
...
PUB_FUNC void _tcc_error_noabort(const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    ce_clgewp(ERROR_NOABORT, fmt, ap);
    va_end(ap);
}
...
PUB_FUNC void _tcc_error(const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    for (;;) ce_clgewp(ERROR_ERROR, fmt, ap);
}
...
static int ce_bnkr5g(TCCState *s1, const char *filename)
{
    int fd;
...
static int ce_6jzpdy(TCCState *s1, int filetype, const char *str, int fd)
{
...
LIBTCCAPI int ce_6jzpdy_string(TCCState *s, const char *str)
{
    return ce_6jzpdy(s, s->filetype, str, -1);
}
...
# endif
    ...
    ce_jfik0n(s, &s->crt_paths, &s->nb_crt_paths, CONFIG_TCC_CRTPREFIX);
...
{
    ce_jfik0n(s, &s->include_paths, &s->nb_include_paths, pathname);
    return 0;
}

LIBTCCAPI int tcc_add_sysinclude_path(TCCState *s, const char *pathname)
{
    ce_jfik0n(s, &s->sysinclude_paths, &s->nb_sysinclude_paths, pathname);
    return 0;
}
...
//Cheat Engine Stop
        ...
        ret = ce_6jzpdy(s1, flags, filename, fd);
    }
...
{
    ce_jfik0n(s, &s->library_paths, &s->nb_library_paths, pathname);
    return 0;
}

static int ce_s85nnp(TCCState *s, const char *fmt,
...
{
    return ce_s85nnp(s, "%s/%s", filename, flags,
...
static int ce_obj9o6(const char **pp)
{
    const char *p = *pp;
    if (*p != 'n' || *++p != 'o' || *++p != '-')
        return 0;
...
static int ce_cj477h(const char *val, const char **str)
{
    const char *p, *q;
    p = *str;
    q = val;
...
/* Like ce_cj477h, but automatically takes into account that ld options can
 *
 ...
 */
static int ce_b6z27m(const char *str, const char *val, const char **ptr)
{
    const char *p, *q;
    int ret;
...
static void ce_kxx2is(char **pp, const char *s, int sep)
{
    const char *q = s;
    char *p = *pp;
    int l = 0;
...
/* set linker options */
static int ce_7sp50l(TCCState *s, const char *option)
{
    TCCState *s1 = s;
    while (*option) {
        ...
        if (ce_b6z27m(option, "Bsymbolic", &p)) {
            s->symbolic = 1;
        } else if (ce_b6z27m(option, "nostdlib", &p)) {
            s->nostdlib = 1;
        } else if (ce_b6z27m(option, "fini=", &p)) {
            ce_kxx2is(&s->fini_symbol, p, 0);
            ignoring = 1;
        } else if (ce_b6z27m(option, "image-base=", &p)
                || ce_b6z27m(option, "Ttext=", &p)) {
            s->text_addr = strtoull(p, &end, 16);
            s->has_text_addr = 1;
        } else if (ce_b6z27m(option, "init=", &p)) {
            ce_kxx2is(&s->init_symbol, p, 0);
            ignoring = 1;
        } else if (ce_b6z27m(option, "oformat=", &p)) {
#if defined(TCC_TARGET_PE)
            if (ce_cj477h("pe-", &p)) {
#elif PTR_SIZE == 8
            if (ce_cj477h("elf64-", &p)) {
#else
            if (ce_cj477h("elf32-", &p)) {
#endif
                s->output_format = TCC_OUTPUT_FORMAT_ELF;
        ...
        } else if (ce_b6z27m(option, "as-needed", &p)) {
            ignoring = 1;
        } else if (ce_b6z27m(option, "O", &p)) {
            ignoring = 1;
        } else if (ce_b6z27m(option, "export-all-symbols", &p)) {
            s->rdynamic = 1;
        } else if (ce_b6z27m(option, "export-dynamic", &p)) {
            s->rdynamic = 1;
        } else if (ce_b6z27m(option, "rpath=", &p)) {
            ce_kxx2is(&s->rpath, p, ':');
        } else if (ce_b6z27m(option, "enable-new-dtags", &p)) {
            s->enable_new_dtags = 1;
        } else if (ce_b6z27m(option, "section-alignment=", &p)) {
            s->section_align = strtoul(p, &end, 16);
        } else if (ce_b6z27m(option, "soname=", &p)) {
            ce_kxx2is(&s->soname, p, 0);
#ifdef TCC_TARGET_PE
        } else if (ce_b6z27m(option, "large-address-aware", &p)) {
            s->pe_characteristics |= 0x20;
        } else if (ce_b6z27m(option, "file-alignment=", &p)) {
            s->pe_file_align = strtoul(p, &end, 16);
        } else if (ce_b6z27m(option, "stack=", &p)) {
            s->pe_stack_size = strtoul(p, &end, 10);
        } else if (ce_b6z27m(option, "subsystem=", &p)) {
            ...
            if (!strcmp(p, "wince")) {
                ...
                goto err;
#endif
        } else if (ret = ce_b6z27m(option, "?whole-archive", &p), ret) {
            if (ret > 0)
                s->filetype |= AFF_WHOLE_ARCHIVE;
            else
                s->filetype &= ~AFF_WHOLE_ARCHIVE;
        } else if (ce_b6z27m(option, "z=", &p)) {
            ignoring = 1;
        } else if (p) {
            return 0;
        } else {
...
static void ce_mt5rc9(TCCState *s, const char* filename, int filetype)
{
...
static int ce_0l5v7s(const char *r, int *argc, char ***argv)
{
    int ret = 0, q, c;
    CString str;
    for(;;) {
...
/* read list file */
static void ce_i9220x(TCCState *s,
...
PUB_FUNC int tcc_parse_args(TCCState *s, int *pargc, char ***pargv, int optind)
{
    ...
    while (optind < argc) {
        r = argv[optind];
        if (r[0] == '@' && r[1] != '\0') {
            ce_i9220x(s, r + 1, optind, &argc, &argv);
            continue;
        }
        optind++;
        if (tool) {
...
reparse:
        ...
        case TCC_OPTION_B:
            ...
            break;
        case TCC_OPTION_l:
            ce_mt5rc9(s, optarg, AFF_TYPE_LIB | (s->filetype & ~AFF_TYPE_MASK));
            s->nb_libraries++;
            break;
        ...
        case TCC_OPTION_Wl:
            ...
            if (ce_7sp50l(s, linker_arg.data))
