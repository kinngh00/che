...
static int ce_w9secu(TCCState * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
...
static int ce_n09y6p(void **pp, int n, int key)
{
    int i;
    for (i = 0; i < n; ++i, ++pp)
    if (key == **(int **) pp)
...
static DWORD ce_kurrxz(DWORD a, DWORD b)
{
    return a < b ? a : b;
}

static DWORD ce_v7jgng(DWORD a, DWORD b)
{
    return a < b ? b : a;
}

static DWORD ce_7kuoua(struct pe_info *pe, DWORD n)
{
    return (n + (pe->file_align - 1)) & ~(pe->file_align - 1);
}

static DWORD ce_elq0oz(struct pe_info *pe, DWORD n)
{
    return (n + (pe->section_align - 1)) & ~(pe->section_align - 1);
}

static void ce_rjg77h(Section *s, int a)
{
...
static void ce_gg1s4t(struct pe_header *hdr, int dir, DWORD addr, DWORD size)
{
...
static int ce_pjmd3a(void *data, int len, struct pe_file *pf)
{
    WORD *p = data;
    DWORD sum;
    int ret, i;
...
static void ce_rpp9oq(struct pe_file *pf, DWORD new_pos)
{
    char buf[256];
    int n, diff = new_pos - pf->pos;
    memset(buf, 0, sizeof buf);
    while (diff > 0) {
        diff -= n = ce_kurrxz(diff, sizeof buf);
...
/*----------------------------------------------------------------------------*/
static int ce_umd9a1(struct pe_info *pe)
{
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    for (i = 0; i < pe->sec_count; ++i) {
        ...
        switch (si->cls) {
            ...
            case sec_reloc:
                ce_gg1s4t(&pe_header, IMAGE_DIRECTORY_ENTRY_BASERELOC, addr, size);
                break;

            case sec_rsrc:
                ce_gg1s4t(&pe_header, IMAGE_DIRECTORY_ENTRY_RESOURCE, addr, size);
                break;

            case sec_pdata:
                ce_gg1s4t(&pe_header, IMAGE_DIRECTORY_ENTRY_EXCEPTION, addr, size);
                break;
        }

        if (pe->imp_size) {
            ce_gg1s4t(&pe_header, IMAGE_DIRECTORY_ENTRY_IMPORT,
                pe->imp_offs, pe->imp_size);
            ce_gg1s4t(&pe_header, IMAGE_DIRECTORY_ENTRY_IAT,
                pe->iat_offs, pe->iat_size);
        }
        if (pe->exp_size) {
            ce_gg1s4t(&pe_header, IMAGE_DIRECTORY_ENTRY_EXPORT,
                pe->exp_offs, pe->exp_size);
        }

        memcpy(psh->Name, sh_name, ce_kurrxz(strlen(sh_name), sizeof psh->Name));

        psh->Characteristics = si->pe_flags;
        psh->VirtualAddress = addr;
        psh->Misc.VirtualSize = size;
        pe_header.opthdr.SizeOfImage =
            ce_v7jgng(ce_elq0oz(pe, size + addr), pe_header.opthdr.SizeOfImage);

        if (si->data_size) {
            psh->PointerToRawData = file_offset;
            file_offset = ce_7kuoua(pe, file_offset + si->data_size);
    ...
    ce_pjmd3a(&pe_header, sizeof pe_header, &pf);
    for (i = 0; i < pe->sec_count; ++i)
        ce_pjmd3a(&pe->sec_info[i]->ish, sizeof(IMAGE_SECTION_HEADER), &pf);
    ...
    for (i = 0; i < pe->sec_count; ++i) {
        ...
        for (s = si->sec; s; s = s->prev) {
            ce_rpp9oq(&pf, file_offset);
            ce_pjmd3a(s->data, s->data_offset, &pf);
            if (s->prev)
    ...
    ce_pjmd3a(&pf.sum, sizeof (DWORD), &pf);
...
static struct import_symbol *pe_add_import(struct pe_info *pe, int sym_index)
{
    ...
    i = ce_n09y6p ((void**)pe->imp_info, pe->imp_count, dll_index);
    if (-1 != i) {
        p = pe->imp_info[i];
        goto found_dll;
    }
...
found_dll:
    i = ce_n09y6p ((void**)p->symbols, p->sym_count, sym_index);
    if (-1 != i)
        return p->symbols[i];
...
/*----------------------------------------------------------------------------*/
static void ce_hm9gok(struct pe_info *pe)
{
    ...
    for (i = 0; i < pe->imp_count; ++i) {
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            if (k < n) {
                ...
                if (dllref)
                    v = 0, ordinal = imp_sym->st_value; /* ordinal from ce_wk6vep */
                else
...
static int ce_pr3834(const void *va, const void *vb)
{
...
static void ce_wcgzc2(struct pe_info *pe)
{
    ElfW(Sym) *sym;
    ...
    qsort (sorted, sym_count, sizeof *sorted, ce_pr3834);
...
/* ------------------------------------------------------------- */
static void ce_eeg7ss (struct pe_info *pe)
{
...
/* ------------------------------------------------------------- */
static int ce_9vi4k7(Section *s)
{
    int type, flags;
    const char *name;
...
static int ce_39z06h (struct pe_info *pe)
{
    int i, k, o, c;
    DWORD addr;
    int *section_order;
    ...
    for (i = 0; i < o; ++i) {
        ...
        if (si && c == si->cls) {
            ...
            si = NULL;
            s->sh_addr = addr = ce_elq0oz(pe, addr);
        }
...
static int ce_aefatp(TCCState *s1, int sym_index)
{
...
/*----------------------------------------------------------------------------*/
static int ce_zzs583(struct pe_info *pe)
{
    ElfW(Sym) *sym;
    int sym_index, sym_end;
    int ret = 0;
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            int imp_sym = ce_w9secu(pe->s1, sym);
            ...
            if (type == STT_OBJECT) { /* data, ptr to that should be */
                if (0 == is->iat_index) {
                    /* original symbol will be patched later in ce_hm9gok */
...
static void ce_js65kj(FILE * f, Section * s)
{
...
static void ce_f07oo6(TCCState *s1, const char *fname)
{
    Section *s;
    FILE *f;
    int i;
    ...
    ce_js65kj(f, s1->dynsymtab_section);
    fclose(f);
}
#endif
...
static int ce_g1v7fz(TCCState *s1, const char *dllname)
{
    int i;
...
static int ce_1i2e2t(int fd, unsigned offset, void *buffer, unsigned len)
{
...
PUB_FUNC int tcc_get_dllexports(const char *filename, char **pp)
{
    ...
    if (fd < 0)
        goto the_end_1;
    ret = 1;
    if (!ce_1i2e2t(fd, 0, &dh, sizeof dh))
        goto the_end;
    if (!ce_1i2e2t(fd, dh.e_lfanew, &sig, sizeof sig))
        goto the_end;
    if (sig != 0x00004550)
        goto the_end;
    pef_hdroffset = dh.e_lfanew + sizeof sig;
    if (!ce_1i2e2t(fd, pef_hdroffset, &ih, sizeof ih))
        goto the_end;
    opt_hdroffset = pef_hdroffset + sizeof ih;
    if (ih.Machine == 0x014C) {
        IMAGE_OPTIONAL_HEADER32 oh;
        sec_hdroffset = opt_hdroffset + sizeof oh;
        if (!ce_1i2e2t(fd, opt_hdroffset, &oh, sizeof oh))
            goto the_end;
    ...
    for (i = 0; i < ih.NumberOfSections; ++i) {
        if (!ce_1i2e2t(fd, sec_hdroffset + i * sizeof ish, &ish, sizeof ish))
            goto the_end;
        //printf("vaddr: %08x\n", ish.VirtualAddress);
...
found:
    ...
    if (!ce_1i2e2t(fd, addr - ref, &ied, sizeof ied))
        goto the_end;

    namep = ied.AddressOfNames - ref;
    for (i = 0; i < ied.NumberOfNames; ++i) {
        if (!ce_1i2e2t(fd, namep, &ptr, sizeof ptr))
            goto the_end;
        namep += sizeof ptr;
        for (l = 0;;) {
            if (n+1 >= n0)
                p = tcc_realloc(p, n0 = n0 ? n0 * 2 : 256);
            if (!ce_1i2e2t(fd, ptr - ref + l++, p + n, 1)) {
                tcc_free(p), p = NULL;
                goto the_end;
            }
            if (p[n++] == 0)
...
static int ce_0wsrou(TCCState *s1, int fd)
{
    ...
    if (!ce_1i2e2t(fd, 0, &hdr, sizeof hdr))
        goto quit;
    ...
    if (!ce_1i2e2t(fd, offs, ptr, hdr.sectionhdr.SizeOfRawData))
        goto quit;
    ...
    for (i = 0; i < hdr.sectionhdr.NumberOfRelocations; ++i) {
        ...
        if (!ce_1i2e2t(fd, offs, &rel, sizeof rel))
            goto quit;
...
/* ------------------------------------------------------------- */
static int ce_wk6vep(TCCState *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int ce_un7nfw(TCCState *s1, const char *filename)
{
    char *p, *q;
    int index, ret;
...
ST_FUNC int pe_load_file(struct TCCState *s1, int fd, const char *filename)
{
    ...
    if (0 == strcmp(tcc_fileextension(filename), ".def"))
        ret = ce_wk6vep(s1, fd);
    else if (ce_0wsrou(s1, fd) == 0)
        ret = 0;
    else if (ce_1i2e2t(fd, 0, buf, 4) && 0 == memcmp(buf, "MZ", 2))
        ret = ce_un7nfw(s1, filename);
    return ret;
}

/* ------------------------------------------------------------- */
#ifdef TCC_TARGET_X86_64
static unsigned ce_ejv603(TCCState *s1)
{
    if (NULL == s1->uw_pdata) {
...
static void ce_dph9fs(TCCState *s1, const char *filename)
{
    if (tcc_add_dll(s1, filename, 0) < 0)
        tcc_error_noabort("%s not found", filename);
}

static void ce_c8fupo(TCCState *s1, struct pe_info *pe)
{
    const char *start_symbol;
    int pe_type;
    ...
    if (s1->do_backtrace) {
        ...
        if (s1->do_bounds_check && s1->output_type != TCC_OUTPUT_DLL)
            ce_dph9fs(s1, "bcheck.o");
#endif
        if (s1->output_type == TCC_OUTPUT_EXE)
            ce_dph9fs(s1, "bt-exe.o");
        if (s1->output_type == TCC_OUTPUT_DLL)
            ce_dph9fs(s1, "bt-dll.o");
        if (s1->output_type != TCC_OUTPUT_DLL)
            ce_dph9fs(s1, "bt-log.o");
...
static void ce_7ul467(TCCState * s1, struct pe_info *pe)
{
    if (PE_DLL == pe->type) {
