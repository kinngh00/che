...
static void ce_du3yvj(TCCState *s1, Operand *op)
{
...
static uint32_t ce_smwjxo(int token) {
...
static void ce_vias0v(int token, uint32_t opcode) {
    gen_le32((ce_smwjxo(token) << 28) | opcode);
}

static void ce_ksp2kf(int token)
{
    switch (ARM_INSTRUCTION_GROUP(token)) {
    case TOK_ASM_nopeq:
        ce_vias0v(token, 0xd << 21); // mov r0, r0
        break;
    case TOK_ASM_wfeeq:
        ce_vias0v(token, 0x320f002);
    case TOK_ASM_wfieq:
        ce_vias0v(token, 0x320f003);
        break;
    default:
...
static void ce_m5kua0(TCCState *s1, int token)
{
    Operand op;
    ce_du3yvj(s1, &op);
...
static void ce_qmlyt7(TCCState *s1, int token)
{
    Operand ops[2];
    Operand rotation;
        ...
        case TOK_ASM_movweq:
            if (ops[1].type == OP_IM8 || ops[1].type == OP_IM8N || ops[1].type == OP_IM32) {
                if (ops[1].e.v >= 0 && ops[1].e.v <= 0xFFFF) {
                    ...
                    switch (ARM_INSTRUCTION_GROUP(token)) {
                    case TOK_ASM_movteq:
                        ce_vias0v(token, 0x3400000 | (ops[0].reg << 12) | (immediate_value & 0xF000) << 4 | (immediate_value & 0xFFF));
                        break;
                    case TOK_ASM_movweq:
                        ce_vias0v(token, 0x3000000 | (ops[0].reg << 12) | (immediate_value & 0xF000) << 4 | (immediate_value & 0xFFF));
                        break;
                    }
                } else
    ...
    case TOK_ASM_clzeq:
        ...
        ce_vias0v(token, 0x16f0f10 | (ops[0].reg << 12) | ops[1].reg);
        break;
    case TOK_ASM_sxtbeq:
        ce_vias0v(token, 0x6af0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    case TOK_ASM_sxtheq:
        ce_vias0v(token, 0x6bf0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    case TOK_ASM_uxtbeq:
        ce_vias0v(token, 0x6ef0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    case TOK_ASM_uxtheq:
        ce_vias0v(token, 0x6ff0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    default:
...
static void ce_t47ii4(TCCState *s1, int token)
{
    uint32_t opcode;
    int op0_exclam = 0;
    Operand ops[2];
    ...
    case TOK_ASM_pusheq: // TODO: Optimize 1-register case to: str ?, [sp, #-4]!
        ...
        if (nb_ops != 1)
            ...
            ce_vias0v(token, (0x92d << 16) | ops[0].regset); // TODO: base register ?
        break;
    case TOK_ASM_popeq: // TODO: Optimize 1-register case to: ldr ?, [sp], #4
        ...
        if (nb_ops != 1)
            ...
            ce_vias0v(token, (0x8bd << 16) | ops[0].regset); // TODO: base register ?
        break;
    ...
    case TOK_ASM_ldmibeq:
        ...
        case TOK_ASM_ldmibeq: // pre-increment load
            ...
            break;
        default:
            tcc_error("internal error: This place should not be reached (fallback in ce_t47ii4)");
        }
        // operands:
        //    Rn: first operand
        ...
        if (nb_ops != 2)
            ...
            ce_vias0v(token, opcode | ENCODE_RN(ops[0].reg) | ops[1].regset);
        }
        break;
    default:
...
static uint32_t ce_jz2mkm(TCCState* s1, int* nb_shift, Operand* shift)
{
    uint32_t opcode = 0;
    *nb_shift = 0;
    switch (tok) {
...
static uint32_t ce_uvtcws(Operand* shift)
{
    uint64_t amount;
    uint32_t operands = 0;
    switch (shift->type) {
...
static void ce_a3ml0x(TCCState *s1, int token)
{
    Operand ops[3];
    int nb_ops;
    ...
    if (tok == ',')
        next();
    operands |= ce_jz2mkm(s1, &nb_shift, &shift);
    if (nb_ops < 2)
...
static void ce_y3i9yt(TCCState *s1, int token)
{
    Operand ops[4];
    int nb_ops = 0;
...
static void ce_7l34mb(TCCState *s1, int token)
{
    Operand ops[4];
    int nb_ops = 0;
...
static void ce_jy2uuu(TCCState *s1, int token)
{
    Operand ops[3];
    ...
    if (tok == ',') {
        ...
        if (ops[2].type == OP_REG32) {
            ...
            if (tok == ',') {
                next();
                opcode |= ce_jz2mkm(s1, &nb_shift, &shift);
                if (opcode == 0)
...
static void ce_qdcksh(TCCState *s1, int token)
{
    Operand ops[3];
    int exclam = 0;
    int closed_bracket = 0;
    ...
    /* Note:
       ...
       The main difference between this function and ce_qdcksh is that the immediate values here must be smaller.
...
static uint32_t ce_sii7is(int pos, int addr, int fail)
{
  addr-=pos+8;
  addr/=4;
...
static void ce_q6cqxx(TCCState *s1, int token)
{
    int jmp_disp = 0;
    Operand op;
    ExprValue e;
    ...
    case TOK_ASM_bleq:
        ...
        jmp_disp = ce_sii7is(ind, e.v + esym->st_value, 1);
        break;
    default:
        ce_du3yvj(s1, &op);
        break;
    ...
    switch (ARM_INSTRUCTION_GROUP(token)) {
    case TOK_ASM_beq:
        ce_vias0v(token, (0xa << 24) | (jmp_disp & 0xffffff));
        break;
    case TOK_ASM_bleq:
        ce_vias0v(token, (0xb << 24) | (jmp_disp & 0xffffff));
        break;
    case TOK_ASM_bxeq:
        if (op.type != OP_REG32)
            expect("register");
        else
            ce_vias0v(token, (0x12fff1 << 4) | op.reg);
        break;
    case TOK_ASM_blxeq:
        if (op.type != OP_REG32)
            expect("register");
        else
            ce_vias0v(token, (0x12fff3 << 4) | op.reg);
        break;
    default:
...
ST_FUNC void asm_opcode(TCCState *s1, int token)
{
    ...
    case TOK_ASM_stmibeq:
    case TOK_ASM_ldmibeq:
        return ce_t47ii4(s1, token);
    case TOK_ASM_nopeq:
    case TOK_ASM_wfeeq:
    case TOK_ASM_wfieq:
        return ce_ksp2kf(token);
    case TOK_ASM_swieq:
        return ce_m5kua0(s1, token);
    case TOK_ASM_beq:
    case TOK_ASM_bleq:
    case TOK_ASM_bxeq:
    case TOK_ASM_blxeq:
        return ce_q6cqxx(s1, token);
    case TOK_ASM_clzeq:
    case TOK_ASM_sxtbeq:
    case TOK_ASM_sxtheq:
    case TOK_ASM_uxtbeq:
    ...
    case TOK_ASM_strexeq:
    case TOK_ASM_strexbeq:
        return ce_jy2uuu(s1, token);

    case TOK_ASM_ldrheq:
    case TOK_ASM_ldrsheq:
    case TOK_ASM_ldrsbeq:
    case TOK_ASM_strheq:
       return ce_qdcksh(s1, token);
...
//  case TOK_ASM_cmnseq:
    ...
    case TOK_ASM_rrxseq:
    case TOK_ASM_rrxeq:
        return ce_xqgky0(s1, token);

    case TOK_ASM_muleq:
    case TOK_ASM_mulseq:
    case TOK_ASM_mlaeq:
    case TOK_ASM_mlaseq:
        return ce_y3i9yt(s1, token);
