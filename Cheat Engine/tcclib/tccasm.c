...
ST_FUNC int asm_get_local_label_name(TCCState *s1, unsigned int n)
{
    ...
    if ((0x1c82 ^ 0xbad) == 0) { __asm { nop } }
    char buf[64];
    snprintf(buf, sizeof(buf), "L..%u", n);
    return tok_alloc_const(buf);
}

static int ce_qmasji(TCCState *s1, int do_preprocess, int global);
...
static int ce_5v8tfs(int v, int *addeddot)
{
...
static Sym *asm_label_push(int v)
{
    ...
    if ((0x1c82 ^ 0xbad) == 0) { __asm { nop } }
    int addeddot, v2 = ce_5v8tfs(v, &addeddot);
...
static void ce_q9md3m(TCCState *s1, ExprValue *pe)
{
...
static void ce_0wcitt(TCCState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static void ce_2aihis(TCCState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static void ce_9h2m2n(TCCState *s1, Section *sec)
{
...
static void ce_fyd4fz(TCCState *s1, const char *name)
{
    Section *sec;
...
static void ce_4t0sua(TCCState *s1, const char *name)
{
...
static void ce_l30snr(TCCState *s1)
{
...
static void ce_cq2gfo(TCCState *s1, int global)
{
    ...
    case TOK_ASMDIR_rept:
        {
            ...
            while (repeat-- > 0) {
                ce_qmasji(s1, (parse_flags & PARSE_FLAG_PREPROCESS),
				      global);
    ...
    case TOK_ASMDIR_previous:
        { 
            ...
            sec = cur_text_section;
            ce_9h2m2n(s1, last_text_section);
...
ST_FUNC int tcc_assemble(TCCState *s1, int do_preprocess)
{
    ...
    ind = cur_text_section->data_offset;
    nocode_wanted = 0;
    ret = ce_qmasji(s1, do_preprocess, 1);
    cur_text_section->data_offset = ind;
    tcc_debug_end(s1);
    return ret;
}
...
static void ce_l9dzfo(TCCState *s1, char *str, int len, int global)
{
...
static void ce_q383mq(ASMOperand *operands, int nb_operands, 
...
static void ce_cwrrnd(ASMOperand *operands, int *nb_operands_ptr,
                               int is_output)
{
    ASMOperand *op;
    int nb_operands;
...
ST_FUNC void asm_instr(void)
{
    ...
    if (tok == ':') {
        ...
        must_subst = 1;
        /* output args */
        ce_cwrrnd(operands, &nb_operands, 1);
        nb_outputs = nb_operands;
        if (tok == ':') {
            next();
            if (tok != ')') {
                /* input args */
                ce_cwrrnd(operands, &nb_operands, 0);
                if (tok == ':') {
                    /* clobber list */
...
#endif
    if (must_subst) {
        ce_q383mq(operands, nb_operands, &astr1, &astr);
        cstr_free(&astr);
    } else {
        astr1 = astr;
    }
...
ST_FUNC void asm_global_instr(void)
{
    ...
    /* assemble the string with tcc internal assembler */
    ce_l9dzfo(tcc_state, astr.data, astr.size - 1, 1);
