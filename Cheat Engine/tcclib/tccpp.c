...
static void ce_6b4qqb(const char *msg, const int *str);
...
#define tal_free(al, p) ce_o85dc2(al, p)
#define tal_realloc(al, p, size) tal_realloc_impl(&al, p, size)
#define TAL_DEBUG_PARAMS
#else
#define TAL_DEBUG 1
//#define TAL_INFO 1 /* collect and dump allocators stats */
#define tal_free(al, p) ce_o85dc2(al, p, __FILE__, __LINE__)
...
static void ce_jvw00a(TinyAlloc *al)
{
...
static void ce_o85dc2(TinyAlloc *al, void *p TAL_DEBUG_PARAMS)
{
    if (!p)
        return;
tail_call:
...
/* ------------------------------------------------------------------------- */
/* CString handling */
static void ce_8ti023(CString *cstr, int new_size)
{
    int size;
...
/* XXX: unicode ? */
static void ce_t92fur(CString *cstr, int c)
{
    if (c == '\'' || c == '\"' || c == '\\') {
...
ST_FUNC const char *get_tok_str(int v, CValue *cv)
{
    ...
    case TOK_STR:
        ...
        if (v == TOK_STR) {
            len = cv->str.size - 1;
            for(i=0;i<len;i++)
                ce_t92fur(&cstr_buf, ((unsigned char *)cv->str.data)[i]);
        } else {
            len = (cv->str.size / sizeof(nwchar_t)) - 1;
            for(i=0;i<len;i++)
                ce_t92fur(&cstr_buf, ((nwchar_t *)cv->str.data)[i]);
        }
        cstr_ccat(&cstr_buf, '\"');
        cstr_ccat(&cstr_buf, '\0');
        break;
...
static int ce_m5ot5m(uint8_t *p)
{
    int c;
...
static void ce_8fbgus(TokenString *s, int t, CValue *cv)
{
    int len, *str;
...
ST_FUNC void tok_str_add_tok(TokenString *s)
{
    ...
    if (file->line_num != s->last_line_num) {
        ...
        ce_8fbgus(s, TOK_LINENUM, &cval);
    }
    ce_8fbgus(s, tok, &tokc);
}
...
static int ce_wyzlw8(const int *a, const int *b)
{
    CValue cv;
    int t;
...
static void ce_v8mune(TCCState *s)
{
    const char *p;
...
static void ce_j0woe1(TCCState *s1)
{
    ce_6qzlbv();
...
ST_FUNC void preprocess(int is_bof)
{
 ...
 redo:
    ...
    case TOK_INCLUDE_NEXT:
        ch = file->buf_ptr[0];
        /* XXX: incorrect if comments : use ce_6qzlbv with a special mode */
        skip_spaces();
        if (ch == '<') {
            c = '>';
            goto read_name;
...
static void ce_iek85i(CString *outstr, const uint8_t *buf, int is_long)
{
    int c, n;
    const uint8_t *p;
    ...
    for(;;) {
        ...
        if (c == '\\') {
            ...
            case 'U': ucn_chars_nr = 8; goto parse_hex_or_ucn;
                ...
                c = n;
                if(ucn_chars_nr==-1) goto ce_t92fur_nonext;
                cstr_u8cat(outstr, c);
                continue;
             }
            case 'a':
...
static void ce_dnr0cs(const char *s, int len)
{
...
/* bn = (bn << shift) | or_val */
static void ce_3f1a1k(unsigned int *bn, int shift, int or_val)
{
    int i;
    unsigned int v;
    for(i=0;i<BN_SIZE;i++) {
...
static void ce_uo4zdw(unsigned int *bn)
{
    int i;
    for(i=0;i<BN_SIZE;i++) {
        bn[i] = 0;
...
/* parse number in null terminated string 'p' and return it in the
   current token */
static void ce_z8s0br(const char *p)
{
...
static inline void ce_6qzlbv1(void)
{
    int t, c, is_long, len;
    TokenSym *ts;
    uint8_t *p, *p1;
...
static int ce_3jvo19(int t1, CValue *v1, int t2, CValue *v2)
{
    CString cstr;
    int n, ret = 1;
        ...
        while (*ptr == TOK_PPJOIN) {
            ...
            if (t1 && t1 != TOK_PPJOIN) {
                ...
                if (t != TOK_PLCHLDR || t1 != TOK_PLCHLDR) {
                    if (ce_3jvo19(t, &cval, t1, &cv1)) {
                        t = tok, cval = tokc;
                    } else {
                        ce_8fbgus(&macro_str1, t, &cval);
                        t = t1, cval = cv1;
                    }
                }
            }
    ...
    Sym *s)
{
	...
	int *joined_str = NULL;
        ...
        if (s->type.t == MACRO_FUNC) {
            ...
            t = ce_l2ywxy(nested_list, &ws_str);
            if (t != '(') {
    ...
    empty_arg:
        ...
        ce_69rxh1(tok_str, nested_list, joined_str ? joined_str : mstr);
    ...
    while (1) {
        ...
        if (t >= TOK_IDENT && 0 == nosubst) {
            ...
            if (sym_find2(*nested_list, t)) {
                ...
                ce_8fbgus(tok_str, TOK_NOSUBST, NULL);
                goto no_subst;
            }

            {
                ...
                tok = t;
                ce_mmu520(tok_str, nested_list, s);
 ...
 redo:
    ...
    if (macro_ptr) {
        ...
        if (s) {
            ...
            tokstr_buf.len = 0;
            ce_mmu520(&tokstr_buf, &nested_list, s);
            tok_str_add(&tokstr_buf, 0);
    ...
    if (t == TOK_PPNUM) {
        if  (parse_flags & PARSE_FLAG_TOK_NUM)
            ce_z8s0br((char *)tokc.str.data);
    } else if (t == TOK_PPSTR) {
        if (parse_flags & PARSE_FLAG_TOK_STR)
            ce_dnr0cs((char *)tokc.str.data, tokc.str.size - 1);
    }
}
...
static void ce_8f27fo(CString *cs, const char *p)
{
    cstr_printf(cs, "#define %s%s\n", p, &" 1"[!!strchr(p, ' ')*2]);
}

static void ce_g8td1u(TCCState *s1, CString *cs, int is_asm)
{
    int a, b, c;
    ...
    if (s1->float_abi == ARM_HARD_FLOAT)
      ce_8f27fo(cs, "__ARM_PCS_VFP");
#endif
    if (is_asm)
      ce_8f27fo(cs, "__ASSEMBLER__");
    if (s1->output_type == TCC_OUTPUT_PREPROCESS)
      ce_8f27fo(cs, "__TCC_PP__");
    if (s1->output_type == TCC_OUTPUT_MEMORY)
      ce_8f27fo(cs, "__TCC_RUN__");
    if (s1->char_is_unsigned)
      ce_8f27fo(cs, "__CHAR_UNSIGNED__");
    if (s1->optimize > 0)
      ce_8f27fo(cs, "__OPTIMIZE__");
    if (s1->option_pthread)
      ce_8f27fo(cs, "_REENTRANT");
    if (s1->leading_underscore)
      ce_8f27fo(cs, "__leading_underscore");
#ifdef CONFIG_TCC_BCHECK
    if (s1->do_bounds_check)
      ce_8f27fo(cs, "__BOUNDS_CHECKING_ON");
#endif
    cstr_printf(cs, "#define __SIZEOF_POINTER__ %d\n", PTR_SIZE);
    cstr_printf(cs, "#define __SIZEOF_LONG__ %d\n", LONG_SIZE);
    if (!is_asm) {
      ce_8f27fo(cs, "__STDC__");
...
ST_FUNC void tccpp_new(TCCState *s)
{
    ...
    cstr_new(&cstr_buf);
    ce_8ti023(&cstr_buf, STRING_MAX_SIZE);
...
ST_FUNC void tccpp_delete(TCCState *s)
{
    ...
    /* free allocators */
    ce_jvw00a(toksym_alloc);
    toksym_alloc = NULL;
    ce_jvw00a(tokstr_alloc);
    tokstr_alloc = NULL;
}
...
static void ce_na6sm6(TCCState *s1, BufferedFile *f, int level)
{
...
static void ce_kd6dij(TCCState *s1, int v)
{
    FILE *fp;
    Sym *s;
...
static void ce_4xw8cn(TCCState *s1)
{
    int v, t;
    const char *vs;
    FILE *fp;
...
static void ce_4dtj7g(TCCState *s1)
{
    int v;
...
static int ce_es9cl3(int a, int b)
{
    return 'E' == a ? '+' == b || '-' == b
        : '+' == a ? TOK_INC == b || '+' == b
        : '-' == a ? TOK_DEC == b || '-' == b
...
/* maybe hex like 0x1e */
static int ce_opd8b4(int t, const char *p)
{
...
ST_FUNC int tcc_preprocess(TCCState *s1)
{
    ...
    if (file->prev)
        ce_na6sm6(s1, file->prev, level++);
    ce_na6sm6(s1, file, level);
    for (;;) {
        ...
        if (is_space(tok)) {
            ...
            ce_na6sm6(s1, file, 0);
        } else if (spcs == 0 && ce_es9cl3(token_seen, tok)) {
            white[spcs++] = ' ';
        }

        white[spcs] = 0, fputs(white, s1->ppfp), spcs = 0;
        fputs(p = get_tok_str(tok, &tokc), s1->ppfp);
        token_seen = ce_opd8b4(tok, p);
    }
    return 0;
}
