...
static uint32_t ce_ape8ss(int r)
{
...
static uint32_t ce_1l72y3(int r)
{
...
static int ce_c29gjy(uint64_t x)
{
...
static uint32_t ce_p418x0(int r, uint64_t x)
{
...
static void ce_oal0r9(int r, uint64_t x)
{
...
static void ce_rrntq3(int reg, uint64_t off)
{
    uint32_t sub = off >> 63;
    if (sub)
        off = -off;
    if (off < 4096)
        ...
        ce_oal0r9(30, off); // use x30 for offset
        o(0x8b3e63e0 | sub << 30 | reg); // (add|sub) x(reg),sp,x30
    }
}

/* invert 0: return value to use for store/load */
/* invert 1: return value to use for ce_ssu853 */
static uint64_t ce_fjp8wv(int invert, int sz_, uint64_t off)
{
    uint32_t sz = sz_;
...
static void ce_m1aqio(int sg, int sz_, int dst, int bas, uint64_t off)
{
    uint32_t sz = sz_;
    if (sz >= 2)
        sg = 0;
...
static void ce_96j2l6(int sz_, int dst, int bas, uint64_t off)
{
    uint32_t sz = sz_;
    if (!(off & ~((uint32_t)0xfff << sz)))
...
static void ce_tyrsn2(int reg_, int size)
{
    uint32_t reg = reg_;
    ...
    case 0:
        ...
        break;
    case 1:
        ce_m1aqio(0, 0, reg, reg, 0);
        break;
    case 2:
        ce_m1aqio(0, 1, reg, reg, 0);
        break;
    case 3:
        ce_m1aqio(0, 1, 30, reg, 0);
        ce_m1aqio(0, 0, reg, reg, 2);
        o(0x2a0043c0 | reg | reg << 16); // orr x(reg),x30,x(reg),lsl #16
        break;
    case 4:
        ce_m1aqio(0, 2, reg, reg, 0);
        break;
    case 5:
        ce_m1aqio(0, 2, 30, reg, 0);
        ce_m1aqio(0, 0, reg, reg, 4);
        o(0xaa0083c0 | reg | reg << 16); // orr x(reg),x30,x(reg),lsl #32
        break;
    case 6:
        ce_m1aqio(0, 2, 30, reg, 0);
        ce_m1aqio(0, 1, reg, reg, 4);
        o(0xaa0083c0 | reg | reg << 16); // orr x(reg),x30,x(reg),lsl #32
        break;
    case 7:
        ce_m1aqio(0, 2, 30, reg, 0);
        ce_m1aqio(0, 2, reg, reg, 3);
        o(0x53087c00 | reg | reg << 5); // lsr w(reg), w(reg), #8
        o(0xaa0083c0 | reg | reg << 16); // orr x(reg),x30,x(reg),lsl #32
        break;
    case 8:
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 9:
        ce_m1aqio(0, 0, reg + 1, reg, 8);
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 10:
        ce_m1aqio(0, 1, reg + 1, reg, 8);
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 11:
        ce_m1aqio(0, 2, reg + 1, reg, 7);
        o(0x53087c00 | (reg+1) | (reg+1) << 5); // lsr w(reg+1), w(reg+1), #8
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 12:
        ce_m1aqio(0, 2, reg + 1, reg, 8);
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 13:
        ce_m1aqio(0, 3, reg + 1, reg, 5);
        o(0xd358fc00 | (reg+1) | (reg+1) << 5); // lsr x(reg+1), x(reg+1), #24
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 14:
        ce_m1aqio(0, 3, reg + 1, reg, 6);
        o(0xd350fc00 | (reg+1) | (reg+1) << 5); // lsr x(reg+1), x(reg+1), #16
        ce_m1aqio(0, 3, reg, reg, 0);
        break;
    case 15:
        ce_m1aqio(0, 3, reg + 1, reg, 7);
...
static void ce_ibwb1j(int sz_, int dst, int bas, uint64_t off)
{
    uint32_t sz = sz_;
    if (!(off & ~((uint32_t)0xfff << sz)))
...
static void ce_t0gw00(int sz_, int dst, int bas, uint64_t off)
{
    uint32_t sz = sz_;
    if (!(off & ~((uint32_t)0xfff << sz)))
...
static void ce_ssu853(int r, Sym *sym, unsigned long addend)
{
	...
	if (addend & 0xffful)
		...
		ce_oal0r9(t, addend & ~0xfffffful); // use xt for addent
...
static void ce_zj8zwt(int r, SValue *sv);
...
ST_FUNC void load(int r, SValue *sv)
{
    ...
    if (svr == (VT_LOCAL | VT_LVAL)) {
        if (IS_FREG(r))
            ce_96j2l6(ce_i0dlz8(svtt), ce_1l72y3(r), 29, svcul);
        else
            ce_m1aqio(!(svtt & VT_UNSIGNED), ce_i0dlz8(svtt),
                       ce_ape8ss(r), 29, svcul);
        return;
    }

    if (svr == (VT_CONST | VT_LVAL)) {
	if (sv->sym)
            ce_ssu853(30, sv->sym, // use x30 for address
	              ce_fjp8wv(0, ce_i0dlz8(svtt), sv->c.i));
	else
	    ce_oal0r9 (30, sv->c.i);
        if (IS_FREG(r))
            ce_96j2l6(ce_i0dlz8(svtt), ce_1l72y3(r), 30,
		       ce_fjp8wv(1, ce_i0dlz8(svtt), sv->c.i));
        else
            ce_m1aqio(!(svtt&VT_UNSIGNED), ce_i0dlz8(svtt), ce_ape8ss(r), 30,
		       ce_fjp8wv(1, ce_i0dlz8(svtt), sv->c.i));
        return;
    }
    ...
    if ((svr & ~VT_VALMASK) == VT_LVAL && svrv < VT_CONST) {
        if ((svtt & VT_BTYPE) != VT_VOID) {
            if (IS_FREG(r))
                ce_96j2l6(ce_i0dlz8(svtt), ce_1l72y3(r), ce_ape8ss(svrv), 0);
            else
                ce_m1aqio(!(svtt & VT_UNSIGNED), ce_i0dlz8(svtt),
                           ce_ape8ss(r), ce_ape8ss(svrv), 0);
        }
        return;
    }
    ...
    if (svr == (VT_CONST | VT_LVAL | VT_SYM)) {
        ce_ssu853(30, sv->sym, // use x30 for address
		  ce_fjp8wv(0, ce_i0dlz8(svtt), svcul));
        if (IS_FREG(r))
            ce_96j2l6(ce_i0dlz8(svtt), ce_1l72y3(r), 30,
		       ce_fjp8wv(1, ce_i0dlz8(svtt), svcul));
        else
            ce_m1aqio(!(svtt&VT_UNSIGNED), ce_i0dlz8(svtt), ce_ape8ss(r), 30,
		       ce_fjp8wv(1, ce_i0dlz8(svtt), svcul));
        return;
    ...
    if (svr == (VT_CONST | VT_SYM)) {
        ce_ssu853(ce_ape8ss(r), sv->sym, svcul);
        return;
    }

    if (svr == VT_CONST) {
        if ((svtt & VT_BTYPE) != VT_VOID)
            ce_oal0r9(ce_ape8ss(r), ce_i0dlz8(svtt) == 3 ?
    ...
    if (svr < VT_CONST) {
        if (IS_FREG(r) && IS_FREG(svr))
            if (svtt == VT_LDOUBLE)
                o(0x4ea01c00 | ce_1l72y3(r) | ce_1l72y3(svr) << 5);
                    // mov v(r).16b,v(svr).16b
            else
                o(0x1e604000 | ce_1l72y3(r) | ce_1l72y3(svr) << 5); // fmov d(r),d(svr)
        else if (!IS_FREG(r) && !IS_FREG(svr))
            o(0xaa0003e0 | ce_ape8ss(r) | ce_ape8ss(svr) << 16); // mov x(r),x(svr)
        else
            assert(0);
      return;
    }
    ...
    if (svr == VT_LOCAL) {
        if (-svcul < 0x1000)
            o(0xd10003a0 | ce_ape8ss(r) | -svcul << 10); // sub x(r),x29,#...
        else {
            ce_oal0r9(30, -svcul); // use x30 for offset
            o(0xcb0003a0 | ce_ape8ss(r) | (uint32_t)30 << 16); // sub x(r),x29,x30
        }
        return;
    }
    ...
    if (svr == VT_JMP || svr == VT_JMPI) {
        int t = (svr == VT_JMPI);
        ce_oal0r9(ce_ape8ss(r), t);
        o(0x14000002); // b .+8
        gsym(svcul);
        ce_oal0r9(ce_ape8ss(r), t ^ 1);
        return;
    }

    if (svr == (VT_LLOCAL | VT_LVAL)) {
        ce_m1aqio(0, 3, 30, 29, svcul); // use x30 for offset
        if (IS_FREG(r))
            ce_96j2l6(ce_i0dlz8(svtt), ce_1l72y3(r), 30, 0);
        else
            ce_m1aqio(!(svtt & VT_UNSIGNED), ce_i0dlz8(svtt),
                       ce_ape8ss(r), 30, 0);
        return;
...
ST_FUNC void store(int r, SValue *sv)
{
    ...
    if (svr == (VT_LOCAL | VT_LVAL)) {
        if (IS_FREG(r))
            ce_t0gw00(ce_i0dlz8(svtt), ce_1l72y3(r), 29, svcul);
        else
            ce_ibwb1j(ce_i0dlz8(svtt), ce_ape8ss(r), 29, svcul);
        return;
    }
	...
	if (sv->sym)
	    ...
	    ce_oal0r9 (30, sv->c.i);
        if (IS_FREG(r))
            ce_t0gw00(ce_i0dlz8(svtt), ce_1l72y3(r), 30,
		       ce_fjp8wv(1, ce_i0dlz8(svtt), sv->c.i));
        else
            ce_ibwb1j(ce_i0dlz8(svtt), ce_ape8ss(r), 30,
		       ce_fjp8wv(1, ce_i0dlz8(svtt), sv->c.i));
        return;
    }
    ...
    if ((svr & ~VT_VALMASK) == VT_LVAL && svrv < VT_CONST) {
        if (IS_FREG(r))
            ce_t0gw00(ce_i0dlz8(svtt), ce_1l72y3(r), ce_ape8ss(svrv), 0);
        else
            ce_ibwb1j(ce_i0dlz8(svtt), ce_ape8ss(r), ce_ape8ss(svrv), 0);
        return;
    }
...
static void ce_kqtaff(int b)
{
	...
	o(0x14000000 | (uint32_t)!b << 31); // b/bl .
        ...
        vtop->r &= ~VT_MUSTBOUND;
#endif
        o(0xd61f0000 | (uint32_t)!b << 21 | ce_ape8ss(gv(RC_R30)) << 5); // br/blr
    }
}

#if defined(CONFIG_TCC_BCHECK)

static void ce_06r1v6(int v)
{
...
static void ce_m0dwt1(void)
{
    ...
    *bounds_ptr = 0;
    ...
    if (offset_modified) {
        ...
        ce_06r1v6(TOK___bound_local_new);
        ind = saved_ind;
    }
    ...
    greloca(cur_text_section, sym_data, ind, R_AARCH64_LD64_GOT_LO12_NC, 0);
    o(0xf9400000 | 0 | (0 << 5)); // ld x0,[x0, #sym_data]
    ce_06r1v6(TOK___bound_local_delete);
    o(0x3cc107e0); /* ldr q0, [sp], #16 */
    o(0xf84107e0); /* ldr x0, [sp], #16 */
}
#endif

static int ce_mcyxt5(CType *type, int *fsize, int num)
{
    if (is_float(type->t)) {
        ...
        *fsize = n;
        ...
        if (is_struct) {
            ...
            for (field = type->ref->next; field; field = field->next) {
                if (field->c != (num - num0) * *fsize)
                    return -1;
                num = ce_mcyxt5(&field->type, fsize, num);
                if (num == -1)
                    return -1;
            }
            ...
            int num0 = num;
            for (field = type->ref->next; field; field = field->next) {
                int num1 = ce_mcyxt5(&field->type, fsize, num0);
                if (num1 == -1)
                    return -1;
        ...
        if (!type->ref->c)
            return num;
        num1 = ce_mcyxt5(&type->ref->type, fsize, num);
...
static int ce_ebprsw(CType *type, unsigned *fsize)
{
    if ((type->t & VT_BTYPE) == VT_STRUCT ||
        ((type->t & VT_ARRAY) && ((type->t & VT_BTYPE) != VT_PTR))) {
        int sz = 0;
        int n = ce_mcyxt5(type, &sz, 0);
        if (0 < n && n <= 4) {
            if (fsize)
                *fsize = sz;
            return n;
...
ST_FUNC void gfunc_call(int nb_args)
{
    ...
    for (i = nb_args; i; i--) {
        ...
        if (a[i] & 1) {
            ...
            if (a[i] >= 32) {
                ...
                r = get_reg(RC_INT);
                ce_rrntq3(ce_ape8ss(r), a1[i]);
                ce_ibwb1j(3, ce_ape8ss(r), 31, (a[i] - 32) >> 1 << 1);
            }
        }
        else if (a[i] >= 32) {
            // value on stack
            if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
                ...
                gv(RC_FLOAT);
                ce_t0gw00(ce_i0dlz8(vtop[0].type.t),
                           ce_1l72y3(vtop[0].r), 31, a[i] - 32);
            }
            else {
                gv(RC_INT);
                ce_ibwb1j(ce_i0dlz8(vtop[0].type.t),
                           ce_ape8ss(vtop[0].r), 31, a[i] - 32);
            }
        }
    ...
    for (i = nb_args; i; i--, vtop--) {
        if (a[i] < 16 && !(a[i] & 1)) {
            ...
            ce_rrntq3(a[i] / 2, a1[i]);
        else if (a[i] < 32) {
            // value in floating-point registers
            if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
                uint32_t j, sz, n = ce_ebprsw(&vtop->type, &sz);
                vtop->type.t = VT_PTR;
                gaddrof();
                gv(RC_R30);
                for (j = 0; j < n; j++)
        ...
        if (bt == VT_STRUCT && !(a[0] & 1)) {
            ...
            if (a[0] == 0) {
                ...
                if (size > 8)
                    ...
                    ce_ibwb1j(size > 4 ? 3 : size > 2 ? 2 : size > 1, 0, 8, 0);

            }
            else if (a[0] == 16) {
                uint32_t j, sz, n = ce_ebprsw(return_type, &sz);
                for (j = 0; j < n; j++)
                    o(0x3d000100 |
                      (sz & 16) << 19 | -(sz & 8) << 27 | (sz & 4) << 29 |
                      (a[i] / 2 - 8 + j) |
...
ST_FUNC void gfunc_prolog(Sym *func_sym)
{
    ...
    for (i = 1, sym = func_type->ref->next; sym; i++, sym = sym->next) {
        ...
        if (a[i] < 16) {
            ...
            uint32_t hfa = ce_ebprsw(&sym->type, 0);
        ...
        if (16 <= a[i] && a[i] < 32 && (sym->type.t & VT_BTYPE) == VT_STRUCT) {
            uint32_t j, sz, k = ce_ebprsw(&sym->type, &sz);
            if (sz < 16)
                for (j = 0; j < k; j++) {
...
ST_FUNC void gen_va_start(void)
{
    ...
    gaddrof();
    r = ce_ape8ss(gv(RC_INT));

    if (arm64_func_va_list_stack) {
        //xx could use add (immediate) here
        ce_oal0r9(30, arm64_func_va_list_stack + 224);
    ...
    ce_oal0r9(30, arm64_func_va_list_gr_offs);
    o(0xb900181e | r << 5); // str w30,[x(r),#24]

    ce_oal0r9(30, arm64_func_va_list_vr_offs);
...
ST_FUNC void gen_va_arg(CType *t)
{
    ...
    unsigned fsize, hfa = ce_ebprsw(t, &fsize);
    uint32_t r0, r1;
...
static void ce_j5dpco(int op, uint32_t l);
...
ST_FUNC int gjmp_cond(int op, int t)
{
    ...
    if (bt == VT_LDOUBLE) {
        uint32_t a, b, f = ce_1l72y3(gv(RC_FLOAT));
        a = get_reg(RC_INT);
        vpushi(0);
        vtop[0].r = a;
        b = get_reg(RC_INT);
...
static int ce_qczjc4(int op, uint32_t l, int rev, uint64_t val,
                          uint32_t x, uint32_t a)
{
    ...
    case '&':
    case '|': {
        int e = ce_c29gjy(l ? val : val | val << 32);
        if (e < 0)
            return 0;
        o((op == '&' ? 0x12000000 :
...
static void ce_j5dpco(int op, uint32_t l)
{
    ...
    // Special treatment for operations with a constant operand:
    {
        ...
        if (ce_04de5j(0, &vtop[0])) {
            vswap();
            rev = 0;
        }
        if (ce_04de5j(&val, &vtop[-1])) {
            gv(RC_INT);
            a = ce_ape8ss(vtop[0].r);
            --vtop;
            x = get_reg(RC_INT);
            ++vtop;
            if (ce_qczjc4(op, l, rev, val, ce_ape8ss(x), a)) {
                vtop[0].r = x;
                vswap();
                --vtop;
                return;
    ...
    a = ce_ape8ss(vtop[-1].r);
    b = ce_ape8ss(vtop[0].r);
    vtop -= 2;
    x = get_reg(RC_INT);
    ++vtop;
    vtop[0].r = x;
...
ST_FUNC void gen_opf(int op)
{
    ...
    assert(vtop[-1].r < VT_CONST && vtop[0].r < VT_CONST);
    a = ce_1l72y3(vtop[-1].r);
    b = ce_1l72y3(vtop[0].r);
    vtop -= 2;
    switch (op) {
...
{
    uint32_t r = ce_ape8ss(gv(RC_INT));
...
ST_FUNC void gen_cvt_itof(int t)
{
    if (t == VT_LDOUBLE) {
        ...
        o(0x1e220000 | (uint32_t)!s << 16 |
          (uint32_t)(t != VT_FLOAT) << 22 | ce_1l72y3(d) |
...
ST_FUNC void gen_cvt_ftoi(int t)
{
    if ((vtop->type.t & VT_BTYPE) == VT_LDOUBLE) {
        ...
        o(0x1e380000 |
          ...
          (uint32_t)((t & VT_BTYPE) == VT_LLONG) << 31 | ce_ape8ss(d) |
...
ST_FUNC void gen_clear_cache(void)
{
    ...
    vpushi(0);
    vtop->r = get_reg(RC_INT);
    beg = ce_ape8ss(vtop[-4].r); // x0
    end = ce_ape8ss(vtop[-3].r); // x1
    dsz = ce_ape8ss(vtop[-2].r); // x2
    isz = ce_ape8ss(vtop[-1].r); // x3
    p = ce_ape8ss(vtop[0].r);    // x4
    vtop -= 5;
...
ST_FUNC void gen_vla_sp_save(int addr) {
    uint32_t r = ce_ape8ss(get_reg(RC_INT));
    o(0x910003e0 | r); // mov x(r),sp
    ce_ibwb1j(3, r, 29, addr);
}
