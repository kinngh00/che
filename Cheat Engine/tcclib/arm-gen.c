...
static int ce_amqbrz(int a,int b) {
...
static uint32_t ce_gmlmsf(uint32_t op, uint32_t c)
{
...
//only add,sub
void ce_gmlmsf_harder(uint32_t op, uint32_t v) {
  uint32_t x;
  x=ce_gmlmsf(op,v);
  if(x)
    o(x);
	...
	if((v&(a[i]|a[j]))==v) {
	  o(ce_gmlmsf(op,v&a[i]));
	  o(ce_gmlmsf(o2,v&a[j]));
	  return;
	}
    no=op^0xC00000;
    n2=o2^0xC00000;
    ...
    for(i=0;i<12;i++)
      for(j=i<4?i+12:15;j>=i+4;j--)
	if((nv&(a[i]|a[j]))==nv) {
	  o(ce_gmlmsf(no,nv&a[i]));
	  o(ce_gmlmsf(n2,nv&a[j]));
	  return;
	}
    for(i=0;i<8;i++)
      for(j=i+4;j<12;j++)
	...
	for(k=i<4?i+12:15;k>=j+4;k--)
	  if((nv&(a[i]|a[j]|a[k]))==nv) {
	    o(ce_gmlmsf(no,nv&a[i]));
	    o(ce_gmlmsf(n2,nv&a[j]));
	    o(ce_gmlmsf(n2,nv&a[k]));
	    return;
	  }
    o(ce_gmlmsf(op,v&a[0]));
    o(ce_gmlmsf(o2,v&a[4]));
    o(ce_gmlmsf(o2,v&a[8]));
    o(ce_gmlmsf(o2,v&a[12]));
  }
}
...
#ifdef TCC_ARM_VFP
static uint32_t vce_b92u6i(int r)
{
...
#else
static uint32_t ce_b92u6i(int r)
{
...
static uint32_t ce_fx9php(int r)
{
  if(r == TREG_R12)
    return 12;
...
static void ce_bxrx0g(uint32_t *base, int *off, int *sgn, int maxoff, unsigned shift)
{
  if(*off>maxoff || *off&((1<<shift)-1)) {
    ...
    x|=(*base)<<16;
    *base=14; // lr
    y=ce_gmlmsf(x,*off&~maxoff);
    if(y) {
      o(y);
      *off&=maxoff;
      return;
    }
    y=ce_gmlmsf(x,(*off+maxoff)&~maxoff);
    if(y) {
      o(y);
      *sgn=!*sgn;
    ...
    ce_gmlmsf_harder(x,*off&~maxoff);
    *off&=maxoff;
  }
}

static uint32_t ce_qyo4hx(int cc)
{
  switch(cc)
  {
    case TOK_ULT:
...
static void ce_x48266(SValue *sv, int r)
{
    o(0xE59F0000|(ce_fx9php(r)<<12));
    o(0xEA000000);
    if(sv->r & VT_SYM) {
	if (sv->sym->type.t & VT_STATIC) {
            greloc(cur_text_section, sv->sym, ind, R_ARM_REL32);
            o(sv->c.i - 12);
            o(0xe080000f | (ce_fx9php(r)<<12) | (ce_fx9php(r)<<16));  // add rx,rx,pc
        }
        else {
            greloc(cur_text_section, sv->sym, ind, R_ARM_GOT_PREL);
            o(-12);
            o(0xe080000f | (ce_fx9php(r)<<12) | (ce_fx9php(r)<<16));  // add rx,rx,pc
            o(0xe5900000 | (ce_fx9php(r)<<12) | (ce_fx9php(r)<<16));  // ldr rx,[rx]
            if (sv->c.i)
              ce_gmlmsf_harder(0xe2800000 | (ce_fx9php(r)<<12) | (ce_fx9php(r)<<16),
                                 sv->c.i);
        }
    }
    else
  ...
  if (fr & VT_LVAL) {
	...
	ce_bxrx0g(&base,&fc,&sign,1020,2);
        ...
        if ((ft & VT_BTYPE) != VT_FLOAT)
          op|=0x100;   /* flds -> fldd */
        o(op|(vce_b92u6i(r)<<12)|(fc>>2)|(base<<16));
#else
	op=0xED100100;
	if(!sign)
	  op|=0x800000;
	...
	if ((ft & VT_BTYPE) == VT_DOUBLE)
	  ...
	  op|=0x400000;
#endif
	o(op|(ce_b92u6i(r)<<12)|(fc>>2)|(base<<16));
#endif
      } else if((ft & (VT_BTYPE|VT_UNSIGNED)) == VT_BYTE
                || (ft & VT_BTYPE) == VT_SHORT) {
	ce_bxrx0g(&base,&fc,&sign,255,0);
	op=0xE1500090;
	...
	if(!sign)
	  op|=0x800000;
	o(op|(ce_fx9php(r)<<12)|(base<<16)|((fc&0xf0)<<4)|(fc&0xf));
      } else {
	ce_bxrx0g(&base,&fc,&sign,4095,0);
	op=0xE5100000;
	if(!sign)
	  op|=0x800000;
        if ((ft & VT_BTYPE) == VT_BYTE || (ft & VT_BTYPE) == VT_BOOL)
          op|=0x400000;
        o(op|(ce_fx9php(r)<<12)|fc|(base<<16));
      }
      return;
    }
  } else {
    if (v == VT_CONST) {
      op=ce_gmlmsf(0xE3A00000|(ce_fx9php(r)<<12),sv->c.i);
      if (fr & VT_SYM || !op)
	ce_x48266(sv, r);
      else
        o(op);
      ...
      op=ce_gmlmsf(0xE28B0000|(ce_fx9php(r)<<12),sv->c.i);
      if (fr & VT_SYM || !op) {
	ce_x48266(sv, r);
	o(0xE08B0000|(ce_fx9php(r)<<12)|ce_fx9php(r));
      } else
	o(op);
      return;
    } else if(v == VT_CMP) {
      o(ce_qyo4hx(sv->c.i)|0x3A00001|(ce_fx9php(r)<<12));
      o(ce_qyo4hx(ce_rv8p49(sv->c.i))|0x3A00000|(ce_fx9php(r)<<12));
      return;
    } else if (v == VT_JMP || v == VT_JMPI) {
      int t;
      t = v & 1;
      o(0xE3A00000|(ce_fx9php(r)<<12)|t);
      o(0xEA000000);
      gsym(sv->c.i);
      o(0xE3A00000|(ce_fx9php(r)<<12)|(t^1));
      return;
    } else if (v < VT_CONST) {
      if(is_float(ft))
#ifdef TCC_ARM_VFP
        o(0xEEB00A40|(vce_b92u6i(r)<<12)|vce_b92u6i(v)|T2CPR(ft)); /* fcpyX */
#else
	o(0xEE008180|(ce_b92u6i(r)<<12)|ce_b92u6i(v));
#endif
      else
	o(0xE1A00000|(ce_fx9php(r)<<12)|ce_fx9php(v));
      return;
    }
  }
...
static void ce_6pm315(int val)
{
  ce_gmlmsf_harder(0xE28DD000,val);
}

/* 'is_jmp' is '1' if it is a jump */
static void ce_f25kjj(int is_jmp)
{
  int r;
  uint32_t x;
  if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
	...
	if(x) {
	    ...
	    ce_x48266(vtop, r);
	    if(is_jmp)
	        o(0xE1A0F000 | ce_fx9php(r)); // mov pc, r
	    else
		o(0xe12fff30 | ce_fx9php(r)); // blx r
	}
     }else{
	if(!is_jmp)
...
static void ce_uzio1q(void)
{
    ...
    *bounds_ptr = 0;
	...
	o(0xe080000f);  /* add r0,r0,pc */
        ce_cdgkys(TOK___bound_local_new);
        ind = saved_ind;
    }
    ...
    o(-12);  /* lbounds_section->data_offset */
    o(0xe080000f);  /* add r0,r0,pc */
    ce_cdgkys(TOK___bound_local_delete);
    o(0xecbd0b02); /* vpop {d0} */
    o(0xe8bd0003); /* pop {r0,r1} */
}
#endif

static int ce_answh5(int btype)
{
#if LDOUBLE_SIZE == 8
    if (btype == VT_LDOUBLE)
      btype = VT_DOUBLE;
...
static int ce_lci3b2(CType *type)
{
  if ((type->t & VT_BTYPE) == VT_STRUCT) {
    ...
    ref = type->ref->next;
    if (ref) {
      btype = ce_answh5(ref->type.t & VT_BTYPE);
      if (btype == VT_FLOAT || btype == VT_DOUBLE) {
        for(; ref && btype == ce_answh5(ref->type.t & VT_BTYPE); ref = ref->next, nb_fields++);
...
int assign_vce_b92u6ieg(struct avail_regs *avregs, int align, int size)
{
  int first_reg = 0;
...
/* Parameters are classified according to how they are copied to their final
   ...
   See the comment for the main loop in ce_98cdat() for the reason. */
enum reg_class {
	STACK_CLASS = 0,
	CORE_STRUCT_CLASS,
	VFP_CLASS,
...
static void ce_izzats(struct plan* plan, int cls, int start, int end, SValue *v)
{
...
/* Assign parameters to registers and stack with alignment according to the
   ...
   before ce_98cdat). */
static int ce_o6thnx(int nb_args, int float_abi, struct plan *plan, int *todo)
{
  int i, size, align;
  ...
  for(i = nb_args; i-- ;) {
    ...
    switch(vtop[-i].type.t & VT_BTYPE) {
      ...
      if (float_abi == ARM_HARD_FLOAT) {
        ...
        if (is_float(vtop[-i].type.t)
            || (is_hfa = ce_lci3b2(&vtop[-i].type))) {
          int end_vce_b92u6ieg;

          start_vce_b92u6ieg = assign_vce_b92u6ieg(&avregs, align, size);
          end_vce_b92u6ieg = start_vce_b92u6ieg + ((size - 1) >> 2);
          if (start_vce_b92u6ieg >= 0) {
            ce_izzats(plan, is_hfa ? VFP_STRUCT_CLASS : VFP_CLASS,
                start_vce_b92u6ieg, end_vce_b92u6ieg, &vtop[-i]);
            continue;
          } else
            break;
        }
      ...
      if (ncrn + size/4 <= 4 || (ncrn < 4 && start_vce_b92u6ieg != -1)) {
	 ...
	 * CORE_STRUCT_CLASS or the first of STACK_CLASS. */
        for (j = ncrn; j < 4 && j < ncrn + size / 4; j++)
          *todo|=(1<<j);
        ce_izzats(plan, CORE_STRUCT_CLASS, ncrn, j, &vtop[-i]);
        ncrn += size/4;
        if (ncrn > 4)
          nsaa = (ncrn - 4) * 4;
      } else {
      ...
      if (ncrn < 4) {
        ...
        ce_izzats(plan, CORE_CLASS, ncrn, ncrn + is_long, &vtop[-i]);
        ncrn += 1 + is_long;
        continue;
      }
    }
    nsaa = (nsaa + (align - 1)) & ~(align - 1);
    ce_izzats(plan, STACK_CLASS, nsaa, nsaa + size, &vtop[-i]);
...
static int ce_98cdat(int nb_args, struct plan *plan, int todo)
{
      ...
      switch(i) {
        ...
        case VFP_STRUCT_CLASS:
          if ((pplan->sval->type.t & VT_BTYPE) == VT_STRUCT) {
            ...
            r = get_reg(RC_INT);
            o(0xE28D0000|(ce_fx9php(r)<<12)|padding); /* add r, sp, padding */
	    ...
	    o(0xECBD0A00|(0&1)<<22|(0>>1)<<12|16); /* vpop {s0-s15} */
            ...
            if (i == VFP_STRUCT_CLASS) {
              ...
              /* No need to write the register used to a SValue since VFP regs
                 cannot be used for ce_f25kjj */
            }
          } else {
            if (is_float(pplan->sval->type.t)) {
#ifdef TCC_ARM_VFP
              r = vce_b92u6i(gv(RC_FLOAT)) << 12;
              ...
              if ((pplan->sval->type.t & VT_BTYPE) == VT_LLONG) {
                ...
                size = 8;
                r = gv(RC_INT);
                o(0xE52D0004|(ce_fx9php(r)<<12)); /* push r */
                vtop--;
              }
              r = gv(RC_INT);
              o(0xE52D0004|(ce_fx9php(r)<<12)); /* push r */
            ...
            if (i == STACK_CLASS && pplan->prev)
              ce_6pm315(pplan->prev->end - pplan->start); /* Add padding if any */
          }
          break;

        case VFP_CLASS:
          gv(ce_df9dp8(TREG_F0 + (pplan->start >> 1)));
        ...
        case CORE_CLASS:
          if ((pplan->sval->type.t & VT_BTYPE) == VT_LLONG) {
            lexpand();
            gv(ce_df9dp8(pplan->end));
            pplan->sval->r2 = vtop->r;
            vtop--;
          }
          gv(ce_df9dp8(pplan->start));
          /* Mark register as used so that ce_f25kjj use another one
...
void gfunc_call(int nb_args)
{
  ...
  args_size = ce_o6thnx(nb_args, float_abi, &plan, &todo);
  ...
  nb_args += ce_98cdat(nb_args, &plan, todo);
  tcc_free(plan.pplans);

  /* Move fct SValue on top as required by ce_f25kjj */
  vrotb(nb_args + 1);
  ce_f25kjj(0);
  if (args_size)
      ce_6pm315(args_size); /* pop all parameters passed on the stack */
#if defined(TCC_ARM_EABI) && defined(TCC_ARM_VFP)
...
void gfunc_prolog(Sym *func_sym)
{
  ...
  for(sym2 = sym->next; sym2 && (n < 4 || nf < 16); sym2 = sym2->next) {
    ...
    if (float_abi == ARM_HARD_FLOAT && !func_var &&
        (is_float(sym2->type.t) || ce_lci3b2(&sym2->type))) {
      int tmpnf = assign_vce_b92u6ieg(&avregs, align, size);
      tmpnf += (size + 3) / 4;
  ...
  while ((sym = sym->next)) {
    ...
    if (float_abi == ARM_HARD_FLOAT && !func_var && (is_float(sym->type.t)
        || ce_lci3b2(&sym->type))) {
      int fpn = assign_vce_b92u6ieg(&avregs, align, size << 2);
      if (fpn >= 0)
        addr = fpn * 4;
      else
        goto from_stack;
...
#endif
  if(diff > 0) {
    x=ce_gmlmsf(0xE24BD000, diff); /* sub sp,fp,# */
    if(x)
...
void gen_opi(int op)
{
  ...
  switch(op) {
    ...
    case '*':
      ...
      fr = vtop[0].r;
      vtop--;
      o(0xE0000090|(ce_fx9php(r)<<16)|(ce_fx9php(r)<<8)|ce_fx9php(fr));
      return;
    case TOK_SHL:
      opc = 0;
      c=2;
    ...
    case TOK_UMULL:
      gv2(RC_INT, RC_INT);
      r=ce_fx9php(vtop[-1].r2=get_reg(RC_INT));
      c=vtop[-1].r;
      vtop[-1].r=get_reg_ex(RC_INT,ce_df9dp8(c));
      vtop--;
      o(0xE0800090|(r<<16)|(ce_fx9php(vtop->r)<<12)|(ce_fx9php(c)<<8)|ce_fx9php(vtop[1].r));
      return;
    default:
      opc = 0x15;
      c=1;
  ...
  switch(c) {
	...
	if(opc == 4 || opc == 5 || opc == 0xc) {
      ...
      if ((vtop->r & VT_VALMASK) == VT_CMP ||
        ...
        gv(RC_INT);
      vswap();
      c=ce_fx9php(gv(RC_INT));
      vswap();
      opc=0xE0000000|(opc<<20);
      if((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
	uint32_t x;
	x=ce_gmlmsf(opc|0x2000000|(c<<16),vtop->c.i);
	if(x) {
	  if ((x & 0xfff00000) == 0xe3500000)   // cmp rx,#c
	    o(x);
	  else {
	    r=ce_fx9php(vtop[-1].r=get_reg_ex(RC_INT,ce_df9dp8(vtop[-1].r)));
	    o(x|(r<<12));
	  }
	  goto done;
	}
      }
      fr=ce_fx9php(gv(RC_INT));
	...
	o(opc|(c<<16)|fr);
        ...
        r=ce_fx9php(vtop[-1].r=get_reg_ex(RC_INT,ce_amqbrz(vtop->r,vtop[-1].r)));
        o(opc|(c<<16)|(r<<12)|fr);
      }
done:
      vtop--;
    ...
    case 2:
      ...
      if ((vtop->r & VT_VALMASK) == VT_CMP ||
        ...
        gv(RC_INT);
      vswap();
      r=ce_fx9php(gv(RC_INT));
      vswap();
      if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
	fr=ce_fx9php(vtop[-1].r=get_reg_ex(RC_INT,ce_df9dp8(vtop[-1].r)));
	c = vtop->c.i & 0x1f;
	o(opc|r|(c<<7)|(fr<<12));
      } else {
        fr=ce_fx9php(gv(RC_INT));
	...
	c=ce_fx9php(vtop[-1].r=get_reg_ex(RC_INT,ce_amqbrz(vtop->r,vtop[-1].r)));
...
void gen_opf(int op)
{
  ...
  switch(op) {
    ...
    default:
      ...
      if(ce_6euxaf(0)) {
        vtop--;
        o(x|0x10000|(vce_b92u6i(gv(RC_FLOAT))<<12)); /* fcmp(e)X -> fcmp(e)zX */
      } else {
        gv2(RC_FLOAT,RC_FLOAT);
        x|=vce_b92u6i(vtop[0].r);
        o(x|(vce_b92u6i(vtop[-1].r) << 12));
        vtop--;
      }
      o(0xEEF1FA10); /* fmstat */
  ...
  if(!fneg) {
    ...
    if ((vtop[-1].r & VT_VALMASK) >= VT_CONST) {
      ...
      r=gv(RC_FLOAT);
      vswap();
      x=(x&~0xf)|vce_b92u6i(r);
    }
  }
  vtop->r=get_reg_ex(RC_FLOAT,r);
  if(!fneg)
    vtop--;
  o(x|(vce_b92u6i(vtop->r)<<12));
}

#else
static uint32_t ce_4498dy()
{
  long double f;
  uint32_t r;
  if((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) != VT_CONST)
...
/* bug (intention?) in Linux FPU emulator
  ...
  if(vtop[-1].r == VT_CMP)
    ...
    if(r2&0x8)
      c1=vtop[-1].r;
    vtop[-1].r=get_reg_ex(RC_FLOAT,ce_amqbrz(vtop[-1].r,c1));
    c1=ce_b92u6i(vtop[-1].r);
  }
  vtop--;
  o(x|(r<<16)|(c1<<12)|r2);
}
...
ST_FUNC void gen_cvt_itof(int t)
{
  ...
  if(bt == VT_INT || bt == VT_SHORT || bt == VT_BYTE) {
    ...
    r=ce_fx9php(gv(RC_INT));
#ifdef TCC_ARM_VFP
    r2=vce_b92u6i(vtop->r=get_reg(RC_FLOAT));
...
#else
    r2=ce_b92u6i(vtop->r=get_reg(RC_FLOAT));
...
void gen_cvt_ftoi(int t)
{
  ...
  if(t==VT_INT) {
#ifdef TCC_ARM_VFP
    r=vce_b92u6i(gv(RC_FLOAT));
    u=u?0:0x10000;
    o(0xEEBC0AC0|(r<<12)|r|T2CPR(r2)|u); /* ftoXizY */
    r2=ce_fx9php(vtop->r=get_reg(RC_INT));
...
void gen_cvt_ftof(int t)
{
  ...
  if(((vtop->type.t & VT_BTYPE) == VT_FLOAT) != ((t & VT_BTYPE) == VT_FLOAT)) {
    uint32_t r = vce_b92u6i(gv(RC_FLOAT));
...
ST_FUNC void gen_vla_alloc(CType *type, int align) {
    ...
    o(ce_gmlmsf(0xE3C0D000|(r<<16), align - 1)); /* bic sp, r, #align-1 */
    vpop();
#if defined(CONFIG_TCC_BCHECK)
    if (tcc_state->do_bounds_check) {
        vpushi(0);
