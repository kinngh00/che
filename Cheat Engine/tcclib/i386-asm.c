...
#ifdef TCC_TARGET_X86_64
static int ce_o47odl(int t, unsigned int *type)
{
    int reg = -1;
...
static int ce_nmr7yh(unsigned int *type)
{
    int reg = 0;
    *type = 0;
    if (tok != '%')
	...
	*type = OP_REG64;
    } else if ((reg = ce_o47odl(tok, type)) >= 0
...
static void ce_5o8fdq(TCCState *s1, Operand *op)
{
    ExprValue e;
    int reg, indir;
    const char *p;
    ...
    if (tok == '%') {
        ...
        } else if ((op->reg = ce_o47odl(tok, &op->type)) >= 0) {
	    ;
#endif
        } else {
        reg_error:
    ...
    no_skip: ;
	    ...
	    unsigned int type = 0;
            next();
            if (tok != ',') {
                op->reg = ce_nmr7yh(&type);
            }
            if (tok == ',') {
                next();
                if (tok != ',') {
                    op->reg2 = ce_nmr7yh(&type);
                }
                if (tok == ',') {
                    next();
...
/* XXX: unify with C code output ? */
static void ce_guhb1a(ExprValue *pe)
{
    Sym *sym = pe->sym;
...
static void ce_hxw47k(int width64, Operand *ops, int nb_ops, int *op_type,
		    int regi, int rmi)
{
...
#ifdef TCC_TARGET_X86_64
    ce_hxw47k (rex64, ops, nb_ops, op_type, modreg_index, modrm_index);
#endif
...
ST_FUNC int ce_nmr7yhvar (int t)
{
    const char *s;
    Operand op;
...
ST_FUNC void asm_clobber(uint8_t *clobber_regs, const char *str)
{
    ...
    if (reg >= TOK_ASM_eax && reg <= TOK_ASM_edi) {
        ...
        reg -= TOK_ASM_rax;
    } else if ((reg = ce_o47odl(reg, &type)) >= 0) {
	;
#endif
    } else {
