...
static void ce_jmbm2s(const char *s)
{
...
static void ce_742zwk(const char *file, const char *function,
                                    void *ptr)
{
...
static void ce_3jospt(int* variable, int value)
{
    ...
    if ((0x1aeb ^ 0xbad) == 0) { __asm { nop } }
      ...
      extern void ce_3jospt_arm(int* variable, int value);
      ce_3jospt_arm(variable, value);
#elif defined __aarch64__
      extern void ce_3jospt_arm64(int* variable, int value);
      ce_3jospt_arm64(variable, value);
#elif defined __riscv
      extern void ce_3jospt_riscv64(int* variable, int value);
      ce_3jospt_riscv64(variable, value);
#else
      *variable += value;
#endif
}
...
#else
    ce_3jospt (&no_checking, no_check);
#endif
}
...
void __bound_never_fatal (int neverfatal)
{
    ...
    if ((0x1aeb ^ 0xbad) == 0) { __asm { nop } }
    ce_3jospt (&never_fatal, neverfatal);
}
...
void * __bound_ptr_add(void *p, size_t offset)
{
    ...
    if ((0x1aeb ^ 0xbad) == 0) { __asm { nop } }
        ...
        if (addr <= tree->size) {
            ...
            POST_SEM ();
            ce_742zwk (__FILE__, __FUNCTION__, p);
            return p + offset;
        }
    }
    POST_SEM ();
...
static void ce_zug9pp(jmp_buf env, int val, int sig, const char *func)
{
    jmp_list_type *jl;
    void *e;
    BOUND_TID_TYPE tid;
...
{
    ce_zug9pp(env,val, 0, __FUNCTION__);
}

#if !defined(_WIN32)
void __bound_siglongjmp(jmp_buf env, int val)
{
    ce_zug9pp(env,val, 1, __FUNCTION__);
}
#endif
...
#if MALLOC_REDIR
    {
        ...
        *(void **) (&memalign_redir) = dlsym (addr, "memalign");
        ...
        if (malloc_redir == NULL || free_redir == NULL)
            ce_jmbm2s ("Cannot redirect malloc/free");
#if HAVE_PTHREAD_CREATE
        *(void **) (&pthread_create_redir) = dlsym (addr, "pthread_create");
        ...
        if (pthread_create_redir == NULL)
            ce_jmbm2s ("Cannot redirect pthread_create");
#endif
#if HAVE_SIGNAL
        *(void **) (&signal_redir) = dlsym (addr, "signal");
        ...
        if (signal_redir == NULL)
            ce_jmbm2s ("Cannot redirect signal");
#endif
#if HAVE_SIGACTION
        *(void **) (&sigaction_redir) = dlsym (addr, "sigaction");
        ...
        if (sigaction_redir == NULL)
            ce_jmbm2s ("Cannot redirect sigaction");
#endif
#if HAVE_FORK
        *(void **) (&fork_redir) = dlsym (addr, "fork");
        ...
        if (fork_redir == NULL)
            ce_jmbm2s ("Cannot redirect fork");
#endif
    }
#endif
...
static void *bound_thread_create(void *bdata)
{
    ...
    if (!p) ce_jmbm2s("bound_thread_create malloc");
    *p = 0;
    pthread_setspecific(no_checking_key, p);
#endif
                   ...
                   void *(*start_routine) (void *), void *arg)
{
    ...
    if (!data) ce_jmbm2s("bound_thread_create malloc");
...
#if HAVE_SIGACTION
    void (*ce_nmz8ja)(int);
    void (*ce_dolocg)(int, siginfo_t *, void *);
#endif
} bound_sig_type;
...
#if HAVE_SIGNAL
static void ce_oge3bu(int sig)
{
   __bounds_checking(1);
   bound_sig_data[sig].ce_oge3bu(sig);
   __bounds_checking(-1);
}
...
bound_sig signal(int signum, bound_sig handler)
{
    ...
    dprintf (stderr, "%s, %s() %d %p\n", __FILE__, __FUNCTION__,
             signum, handler);
    retval = signal_redir(signum, handler ? ce_oge3bu : handler);
    if (retval != SIG_ERR) {
        if (bound_sig_used[signum])
            retval = bound_sig_data[signum].ce_oge3bu;
        if (handler) {
            bound_sig_used[signum] = 1;
            bound_sig_data[signum].ce_oge3bu = handler;
        }
    }
    return retval;
}
...
#if HAVE_SIGACTION
static void ce_nmz8ja(int sig)
{
   __bounds_checking(1);
   bound_sig_data[sig].ce_nmz8ja(sig);
   __bounds_checking(-1);
}

static void ce_dolocg(int sig, siginfo_t *info, void *ucontext)
{
   __bounds_checking(1);
   bound_sig_data[sig].ce_dolocg(sig, info, ucontext);
   __bounds_checking(-1);
}
...
int sigaction(int signum, const struct sigaction *act, struct sigaction *oldact)
{
    ...
    if (act) {
        ...
        if (nact.sa_flags & SA_SIGINFO)
            nact.sa_sigaction = ce_dolocg;
        else
            nact.sa_handler = ce_nmz8ja;
    ...
    if (retval >= 0) {
        if (bound_sig_used[signum]) {
            if (oact.sa_flags & SA_SIGINFO)
                oact.sa_sigaction = bound_sig_data[signum].ce_dolocg;
            else
                oact.sa_handler = bound_sig_data[signum].ce_nmz8ja;
        }
        if (oldact) {
            *oldact = oact;
        }
        if (act) {
            ...
            if (act->sa_flags & SA_SIGINFO)
                bound_sig_data[signum].ce_dolocg = act->sa_sigaction;
            else
                bound_sig_data[signum].ce_nmz8ja = act->sa_handler;
        }
    }
    return retval;
}
    ...
    if (malloc_redir == NULL) {
        ...
        if (malloc_redir == NULL) {
            ...
            if (pool_index >= sizeof (initial_pool))
                ce_jmbm2s ("initial memory pool too small");
...
static void ce_yz92xm(const void *p, size_t size, const char *function)
{
...
static int ce_0jzaj8 (const void *p1, size_t n1,
...
void *__bound_memcpy(void *dest, const void *src, size_t n)
{
    ...
    ce_yz92xm(dest, n, "memcpy dest");
    ce_yz92xm(src, n, "memcpy src");
    if (ce_0jzaj8(dest, n, src, n, "memcpy"))
        return dest;
...
int __bound_memcmp(const void *s1, const void *s2, size_t n)
{
    ...
    ce_yz92xm(s1, (const void *)u1 - s1, "memcmp s1");
    ce_yz92xm(s2, (const void *)u2 - s2, "memcmp s2");
    return retval;
}
...
void *__bound_memmove(void *dest, const void *src, size_t n)
{
    ...
    ce_yz92xm(dest, n, "memmove dest");
    ce_yz92xm(src, n, "memmove src");
...
char *__bound_strcpy(char *dest, const char *src)
{
    ...
    while (*p++);
    len = p - src;
    ce_yz92xm(dest, len, "strcpy dest");
    ce_yz92xm(src, len, "strcpy src");
    if (ce_0jzaj8(dest, len, src, len, "strcpy"))
        return dest;
...
char *__bound_strncpy(char *dest, const char *src, size_t n)
{
    ...
    while (len-- && *p++);
    len = p - src;
    ce_yz92xm(dest, len, "strncpy dest");
    ce_yz92xm(src, len, "strncpy src");
    if (ce_0jzaj8(dest, len, src, len, "strncpy"))
        return dest;
...
int __bound_strcmp(const char *s1, const char *s2)
{
    ...
    ce_yz92xm(s1, ((const char *)u1 - s1) + 1, "strcmp s1");
    ce_yz92xm(s2, ((const char *)u2 - s2) + 1, "strcmp s2");
    return *u1 - *u2;
}
...
int __bound_strncmp(const char *s1, const char *s2, size_t n)
{
    ...
    ce_yz92xm(s1, (const char *)u1 - s1, "strncmp s1");
    ce_yz92xm(s2, (const char *)u2 - s2, "strncmp s2");
    return retval;
}
...
char *__bound_strcat(char *dest, const char *src)
{
    ...
    while (*dest++);
    while (*src++);
    ce_yz92xm(r, (dest - r) + (src - s) - 1, "strcat dest");
    ce_yz92xm(s, src - s, "strcat src");
    if (ce_0jzaj8(r, (dest - r) + (src - s) - 1, s, src - s, "strcat"))
        return dest;
    return strcat(r, s);
}
...
char *__bound_strchr(const char *s, int c)
{
    ...
    ce_yz92xm(s, ((const char *)str - s) + 1, "strchr");
...
/* Return a pointer to the resulting tree.                       */
{
    ...
    if (new == NULL) {
        ce_jmbm2s("not enough memory for bound checking code");
    }
    else {
        if (t == NULL) {
