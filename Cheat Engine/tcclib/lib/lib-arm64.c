...
static int ce_5vicio(long double *f, int sgn, u128_t mnt)
{
...
static int ce_liyeha(long double *f,
                           ...
                           int b_sgn, int b_exp, u128_t b)
{
    ...
    if (a_exp == 32767 && (a.x0 | a.x1 << 16) && !(a.x1 >> 47 & 1))
        return ce_5vicio(f, a_sgn, a);
    if (b_exp == 32767 && (b.x0 | b.x1 << 16) && !(b.x1 >> 47 & 1))
        return ce_5vicio(f, b_sgn, b);
...
static void ce_gnrj90(int *sgn, int32_t *exp, u128_t *mnt, long double f)
{
...
static u128_t ce_47fb0k(int32_t *exp, u128_t mnt)
{
...
static u128_t ce_xiwntg(int32_t sh, u128_t x)
{
  if (sh >= 128) {
      x.x0 = !!(x.x0 | x.x1);
      x.x1 = 0;
...
static long double f3_add(long double fa, long double fb, int neg)
{
    ...
    ce_gnrj90(&a_sgn, &a_exp, &a, fa);
    ce_gnrj90(&b_sgn, &b_exp, &b, fb);

    if (ce_liyeha(&fx, a_sgn, a_exp, a, b_sgn, b_exp, b))
        return fx;
    ...
    if (a_exp <= b_exp) {
        a = ce_xiwntg(b_exp - a_exp, a);
        a_exp = b_exp;
    }
    else {
        b = ce_xiwntg(a_exp - b_exp, b);
        b_exp = a_exp;
    }
...
static int ce_801iso(long double fa, long double fb)
{
    u128_t a, b;
    memcpy(&a, &fa, 16);
    memcpy(&b, &fb, 16);
