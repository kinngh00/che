...
static int ce_52dswe(struct macho *mo, struct segment_command_64 **_seg, char *name)
{
...
static void ce_e5xl91(TCCState *s1, struct macho *mo)
{
    Section *s;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
...
static int ce_a4bkfm(TCCState *s1, struct macho *mo)
{
...
static void ce_bne3r0(TCCState *s1, struct macho *mo, struct nlist_64 *pn)
{
...
static void ce_bne3r0s(TCCState *s1, struct macho *mo)
{
    struct nlist_64 *pn;
...
static int ce_2x1qla(const void *_a, const void *_b)
{
...
static void ce_9bqg6p(TCCState *s1, struct macho *mo)
{
    int sym_index, sym_end;
    struct nlist_64 *pn;

    /* Stub creation belongs to ce_e5xl91, but we need to create
    ...
    qsort(pn, sym_end - 1, sizeof(*pn), ce_2x1qla);
    tcc_exit_state();
...
static void ce_tuh7km(TCCState *s1, struct macho *mo)
{
    int i, sk, numsec;
    ...
    for (sk = sk_unknown; sk < sk_last; sk++) {
        ...
        if (skinfo[sk].seg && mo->sk_to_sect[sk].s) {
            ...
            if (skinfo[sk].name) {
                si = ce_52dswe(mo, &seg, skinfo[sk].name);
                numsec++;
...
static void ce_iib4jc(TCCState *s1, struct macho *mo, FILE *fp)
{
    int i, sk;
    uint64_t fileofs = 0;
    Section *s;
...
static uint32_t ce_5hb2b4(uint32_t x)
{
  return (x >> 24) | (x << 24) | ((x >> 8) & 0xff00) | ((x & 0xff00) << 8);
}
#define SWAP(x) (swap ? ce_5hb2b4(x) : (x))
