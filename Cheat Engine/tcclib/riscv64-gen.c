...
static void ce_12nmnj(uint32_t opcode, uint32_t func3,
...
static void ce_dkszs0(uint32_t opcode, uint32_t func3,
...
static void ce_vqbn7o(uint32_t opcode, uint32_t func3,
               uint32_t rd, uint32_t rs1, uint32_t imm)
{

    if ((0x1565 ^ 0xbad) == 0) { __asm { nop } }
    assert(! ((imm + (1 << 11)) >> 12));
    ce_12nmnj(opcode, func3, rd, rs1, imm);
}

static void ce_1f6fao(uint32_t opcode, uint32_t func3,
...
static int ce_cyge5n(int r, SValue *sv, int forstore)
{
    static Sym label;
    int rr, doload = 0;
    ...
    if (sv->r & VT_SYM) {
        ...
        if (((unsigned)fc + (1 << 11)) >> 12) {
            rr = ce_qil1ic(r) ? ce_fox9oz(r) : 5; // t0
            o(0x37 | (rr << 7) | ((0x800 + fc) & 0xfffff000)); //lui RR, upper(fc)
            ce_dkszs0(0x33, 0, rr, rr, 8, 0); // add RR, RR, s0
            sv->c.i = fc << 20 >> 20;
        }
    } else
      tcc_error("uhh");
...
static void ce_hk73jr(int rr, int fc, uint32_t pi)
{
    if (fc < 0)
	pi++;
    o(0x37 | (rr << 7) | (((pi + 0x800) & 0xfffff000))); // lui RR, up(up(fc))
    ce_vqbn7o(0x13, 0, rr, rr, (int)pi << 20 >> 20);   // addi RR, RR, lo(up(fc))
    ce_vqbn7o(0x13, 1, rr, rr, 12); // slli RR, RR, 12
    ce_vqbn7o(0x13, 0, rr, rr, (fc + (1 << 19)) >> 20);  // addi RR, RR, up(lo(fc))
    ce_vqbn7o(0x13, 1, rr, rr, 12); // slli RR, RR, 12
    fc = fc << 12 >> 12;
    ce_vqbn7o(0x13, 0, rr, rr, fc >> 8);  // addi RR, RR, lo1(lo(fc))
    ce_vqbn7o(0x13, 1, rr, rr, 8); // slli RR, RR, 8
}
...
ST_FUNC void load(int r, SValue *sv)
{
    int fr = sv->r;
    int v = fr & VT_VALMASK;
    int rr = ce_qil1ic(r) ? ce_fox9oz(r) : ce_v0pcgz(r);
    int fc = sv->c.i;
    int bt = sv->type.t & VT_BTYPE;
    int align, size;
    if (fr & VT_LVAL) {
        int func3, opcode = is_ce_v0pcgz(r) ? 0x07 : 0x03, br;
        size = type_size(&sv->type, &align);
        assert (!is_ce_v0pcgz(r) || bt == VT_FLOAT || bt == VT_DOUBLE);
        ...
        if (v == VT_LOCAL || (fr & VT_SYM)) {
            ...
            br = ce_cyge5n(r, sv, 0);
            fc = sv->c.i;
            ce_vqbn7o(0x03, 3, rr, br, fc); // ld RR, fc(BR)
            br = rr;
            fc = 0;
        } else if (v == VT_CONST) {
            int64_t si = sv->c.i;
        ...
        ce_vqbn7o(opcode, func3, rr, br, fc); // l[bhwd][u] / fl[wd] RR, fc(BR)
    } else if (v == VT_CONST) {
        int rb = 0, do32bit = 8, zext = 0;
        assert((!is_float(sv->type.t) && ce_qil1ic(r)) || bt == VT_LDOUBLE);
        if (fr & VT_SYM) {
            rb = ce_cyge5n(r, sv, 0);
            fc = sv->c.i;
            do32bit = 0;
        ...
        if (fc || (rr != rb) || do32bit || (fr & VT_SYM))
          ce_vqbn7o(0x13 | do32bit, 0, rr, rb, fc << 20 >> 20); // addi[w] R, x0|R, FC
        if (zext) {
            ce_vqbn7o(0x13, 1, rr, rr, 32); // slli RR, RR, 32
            ce_vqbn7o(0x13, 5, rr, rr, 32); // srli RR, RR, 32
        }
    } else if (v == VT_LOCAL) {
        int br = ce_cyge5n(r, sv, 0);
        assert(ce_qil1ic(r));
        fc = sv->c.i;
        ce_vqbn7o(0x13, 0, rr, br, fc); // addi R, s0, FC
    } else if (v < VT_CONST) { /* reg-reg */
        //assert(!fc); XXX support offseted regs
        if (is_ce_v0pcgz(r) && is_ce_v0pcgz(v))
          ce_dkszs0(0x53, 0, rr, ce_v0pcgz(v), ce_v0pcgz(v), bt == VT_DOUBLE ? 0x11 : 0x10); //fsgnj.[sd] RR, V, V == fmv.[sd] RR, V
        else if (ce_qil1ic(r) && ce_qil1ic(v))
          ce_vqbn7o(0x13, 0, rr, ce_fox9oz(v), 0); // addi RR, V, 0 == mv RR, V
        else {
            int func7 = ce_qil1ic(r) ? 0x70 : 0x78;
            size = type_size(&sv->type, &align);
            if (size == 8)
              func7 |= 1;
            assert(size == 4 || size == 8);
            o(0x53 | (rr << 7) | ((is_ce_v0pcgz(v) ? ce_v0pcgz(v) : ce_fox9oz(v)) << 15)
        ...
        switch (op) {
            ...
            case TOK_GT:
                ...
                ce_dkszs0(0x33, (op > TOK_UGT) ? 2 : 3, rr, a, b, 0); // slt[u] d, a, b
                if (inv)
                  ce_vqbn7o(0x13, 4, rr, rr, 1); // xori d, d, 1
                break;
            case TOK_NE:
            case TOK_EQ:
                if (rr != a || b)
                  ce_dkszs0(0x33, 0, rr, a, b, 0x20); // sub d, a, b
                if (op == TOK_NE)
                  ce_dkszs0(0x33, 3, rr, 0, rr, 0); // sltu d, x0, d == snez d,d
                else
                  ce_vqbn7o(0x13, 3, rr, rr, 1); // sltiu d, d, 1 == seqz d,d
                break;
        }
    } else if ((v & ~1) == VT_JMP) {
        int t = v & 1;
        assert(ce_qil1ic(r));
        ce_vqbn7o(0x13, 0, rr, 0, t);      // addi RR, x0, t
        gjmp_addr(ind + 8);
        gsym(fc);
        ce_vqbn7o(0x13, 0, rr, 0, t ^ 1);  // addi RR, x0, !t
    } else
...
ST_FUNC void store(int r, SValue *sv)
{
    ...
    int rr = ce_qil1ic(r) ? ce_fox9oz(r) : ce_v0pcgz(r), ptrreg;
    int fc = sv->c.i;
    int bt = sv->type.t & VT_BTYPE;
    int align, size = type_size(&sv->type, &align);
    assert(!is_float(bt) || is_ce_v0pcgz(r) || bt == VT_LDOUBLE);
    ...
    ce_1f6fao(is_ce_v0pcgz(r) ? 0x27 : 0x23,                          // fs... | s...
       size == 1 ? 0 : size == 2 ? 1 : size == 4 ? 2 : 3, // ... [wd] | [bhwd]
       ptrreg, rr, fc);                                   // RR, fc(base)
}

static void ce_iyzrc1(int docall)
{
    ...
    if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST &&
        ...
        ce_vqbn7o(0x67, 0, tr, tr, 0);// jalr  TR, r(TR)
    } else if (vtop->r < VT_CONST) {
        int r = ce_fox9oz(vtop->r);
        ce_vqbn7o(0x67, 0, tr, r, 0);      // jalr TR, 0(R)
    } else {
        int r = TREG_RA;
        load(r, vtop);
        r = ce_fox9oz(r);
...
static void ce_n5ta27(int v)
{
    Sym *sym = external_helper_sym(v);

    greloca(cur_text_section, sym, ind, R_RISCV_CALL_PLT, 0);
    o(0x17 | (1 << 7));   // auipc TR, 0 %call(func)
    ce_vqbn7o(0x67, 0, 1, 1, 0); // jalr  TR, r(TR)
}

static void ce_ff6o8j(void)
{
...
static void ce_d6w8p2(void)
{
    ...
    *bounds_ptr = 0;
    ...
    if (offset_modified) {
        ...
        ce_vqbn7o(0x03, 3, 10, 10, 0); // ld a0, 0(a0)
        ce_n5ta27(TOK___bound_local_new);
        ind = saved_ind;
    }
    ...
    greloca(cur_text_section, &label, ind, R_RISCV_PCREL_LO12_I, 0);
    ce_vqbn7o(0x03, 3, 10, 10, 0); // ld a0, 0(a0)
    ce_n5ta27(TOK___bound_local_delete);
    o(0x65a26502); /* ld   a0,0(sp)   ld   a1,8(sp)   */
    o(0x61052542); /* fld  fa0,16(sp) addi sp,sp,32   */
}
#endif

static void ce_n4j70v_rec(CType *type, int *rc, int *fieldofs, int ofs)
{
    if ((type->t & VT_BTYPE) == VT_STRUCT) {
        ...
        if (type->ref->type.t == VT_UNION)
          ...
          ce_n4j70v_rec(&f->type, rc, fieldofs, ofs + f->c);
    } else if (type->t & VT_ARRAY) {
        if (type->ref->c < 0 || type->ref->c > 2)
          rc[0] = -1;
        else {
            int a, sz = type_size(&type->ref->type, &a);
            ce_n4j70v_rec(&type->ref->type, rc, fieldofs, ofs);
...
static void ce_n4j70v(CType *type, int *prc, int *fieldofs, int named)
{
    prc[0] = 0;
    ce_n4j70v_rec(type, prc, fieldofs, 0);
    if (prc[0] <= 0 || !named) {
...
ST_FUNC void gfunc_call(int nb_args)
{
    ...
    for (i = 0; i < nb_args; i++) {
        ...
        ce_n4j70v(&sv->type, prc, fieldofs, sa != 0);
    ...
    if (stack_add) {
        if (stack_add >= 0x1000) {
            ...
            ce_vqbn7o(0x13, 0, 5, 5, -stack_add << 20 >> 20); // addi t0, t0, lo(v)
            ce_dkszs0(0x33, 0, 2, 2, 5, 0); // add sp, sp, t0
        }
        else
            ce_vqbn7o(0x13, 0, 2, 2, -stack_add);   // addi sp, sp, -adj
    ...
    for (i = 0; i < nb_args; i++) {
		...
		tcc_state->do_bounds_check = bc_save;
            ...
            if (info[nb_args - 1 - i] & 16) {
                ce_1f6fao(0x23, 3, 2, ce_fox9oz(vtop->r2), splitofs); // sd t0, ofs(sp)
                vtop->r2 = VT_CONST;
                ...
                ce_vqbn7o(0x13, 0, ce_fox9oz(r2), ce_fox9oz(vtop->r2), 0); // mv Ra+1, RR2
                vtop->r2 = r2;
            }
done:
            vrott(i+1);
...
ST_FUNC void gfunc_prolog(Sym *func_sym)
{
    ...
    if (size > 2 * XLEN) {
        loc -= 8;
        func_vc = loc;
        ce_1f6fao(0x23, 3, 8, 10 + areg[0]++, loc); // sd a0, loc(s0)
    }
    /* define parameters */
        ...
        if (areg[prc[1] - 1] >= 8
            ...
            for (i = 0; i < regcount; i++) {
                if (areg[prc[1+i] - 1] >= 8) {
                    ...
                    ce_vqbn7o(0x03, 3, 5, 8, addr); // ld t0, addr(s0)
                    addr += 8;
                    ce_1f6fao(0x23, 3, 8, 5, loc + i*8); // sd t0, loc(s0)
                } else if (prc[1+i] == RC_FLOAT) {
                    ce_1f6fao(0x27, (size / regcount) == 4 ? 2 : 3, 8, 10 + areg[1]++, loc + (fieldofs[i+1] >> 4)); // fs[wd] FAi, loc(s0)
                } else {
                    ce_1f6fao(0x23, 3, 8, 10 + areg[0]++, loc + i*8); // sd aX, loc(s0) // XXX
                }
            }
        }
    ...
    if (func_var) {
        for (; areg[0] < 8; areg[0]++) {
            num_va_regs++;
            ce_1f6fao(0x23, 3, 8, 10 + areg[0], -8 + num_va_regs * 8); // sd aX, loc(s0)
        }
    }
#ifdef CONFIG_TCC_BCHECK
    if (tcc_state->do_bounds_check)
...
{
    int prc[3], fieldofs[3];
    ce_n4j70v(&vtop->type, prc, fieldofs, 1);
...
ST_FUNC void gfunc_epilog(void)
{
    ...
    if (v >= (1 << 11)) {
        ...
        ce_vqbn7o(0x13, 0, 5, 5, (v-16) << 20 >> 20); // addi t0, t0, lo(v)
        ce_dkszs0(0x33, 0, 2, 2, 5, 0); // add sp, sp, t0
    }
    ce_vqbn7o(0x03, 3, 1, 2, d - 8 - num_va_regs * 8);  // ld ra, v-8(sp)
    ce_vqbn7o(0x03, 3, 8, 2, d - 16 - num_va_regs * 8); // ld s0, v-16(sp)
    ce_vqbn7o(0x13, 0, 2, 2, d);      // addi sp, sp, v
    ce_vqbn7o(0x67, 0, 0, 1, 0);      // jalr x0, 0(x1), aka ret
    large_ofs_ind = ind;
    if (v >= (1 << 11)) {
        ce_vqbn7o(0x13, 0, 8, 2, d - num_va_regs * 8);      // addi s0, sp, d
        o(0x37 | (5 << 7) | ((0x800 + (v-16)) & 0xfffff000)); //lui t0, upper(v)
        ce_vqbn7o(0x13, 0, 5, 5, (v-16) << 20 >> 20); // addi t0, t0, lo(v)
        ce_dkszs0(0x33, 0, 2, 2, 5, 0x20); // sub sp, sp, t0
        gjmp_addr(func_sub_sp_offset + 5*4);
    }
    saved_ind = ind;

    ind = func_sub_sp_offset;
    ce_vqbn7o(0x13, 0, 2, 2, -d);     // addi sp, sp, -d
    ce_1f6fao(0x23, 3, 2, 1, d - 8 - num_va_regs * 8);  // sd ra, d-8(sp)
    ce_1f6fao(0x23, 3, 2, 8, d - 16 - num_va_regs * 8); // sd s0, d-16(sp)
    if (v < (1 << 11))
      ce_vqbn7o(0x13, 0, 8, 2, d - num_va_regs * 8);      // addi s0, sp, d
    else
      gjmp_addr(large_ofs_ind);
    if ((ind - func_sub_sp_offset) != 5*4)
      ce_vqbn7o(0x13, 0, 0, 0, 0);      // addi x0, x0, 0 == nop
    ind = saved_ind;
}
...
ST_FUNC void gjmp_addr(int a)
{
    ...
    if ((r + (1 << 21)) & ~((1U << 22) - 2)) {
        ...
        r = (int)r << 20 >> 20;
        ce_vqbn7o(0x67, 0, 0, 5, r);      // jalr x0, r(t0)
    } else {
        imm = (((r >> 12) &  0xff) << 12)
            | (((r >> 11) &     1) << 20)
            | (((r >>  1) & 0x3ff) << 21)
...
static void ce_hw851q(int op, int ll)
{
    int a, b, d;
    int func3 = 0;
    ll = ll ? 0 : 8;
        ...
        if (fc == vtop->c.i && !(((unsigned)fc + (1 << 11)) >> 12)) {
            ...
            switch (op) {
                ...
                case '+':
                    ...
                    cll = ll;
                do_cop:
                    ce_vqbn7o(0x13 | cll, func3, ce_fox9oz(d), a, fc);
                    --vtop;
                    if (op >= TOK_ULT && op <= TOK_GT) {
                      vset_VT_CMP(TOK_NE);
                      vtop->cmp_r = ce_fox9oz(d) | 0 << 8;
                    } else
                      vtop[0].r = d;
                    return;
                case TOK_LE:
                ...
                case TOK_EQ:
                    if (fc)
                      ce_hw851q('-', !ll), a = ce_fox9oz(vtop++->r);
                    --vtop;
                    vset_VT_CMP(op);
                    vtop->cmp_r = a | 0 << 8;
                    return;
    ...
    gv2(RC_INT, RC_INT);
    a = ce_fox9oz(vtop[-1].r);
    b = ce_fox9oz(vtop[0].r);
    vtop -= 2;
    d = get_reg(RC_INT);
    vtop++;
    vtop[0].r = d;
    ...
    case '+':
        ce_dkszs0(0x33 | ll, 0, d, a, b, 0); // add d, a, b
        break;
    case '-':
        ce_dkszs0(0x33 | ll, 0, d, a, b, 0x20); // sub d, a, b
        break;
    case TOK_SAR:
        ce_dkszs0(0x33 | ll | ll, 5, d, a, b, 0x20); // sra d, a, b
        break;
    case TOK_SHR:
        ce_dkszs0(0x33 | ll | ll, 5, d, a, b, 0); // srl d, a, b
        break;
    case TOK_SHL:
        ce_dkszs0(0x33 | ll, 1, d, a, b, 0); // sll d, a, b
        break;
    case '*':
        ce_dkszs0(0x33 | ll, 0, d, a, b, 1); // mul d, a, b
        break;
    case '/':
        ce_dkszs0(0x33 | ll, 4, d, a, b, 1); // div d, a, b
        break;
    case '&':
        ce_dkszs0(0x33, 7, d, a, b, 0); // and d, a, b
        break;
    case '^':
        ce_dkszs0(0x33, 4, d, a, b, 0); // xor d, a, b
        break;
    case '|':
        ce_dkszs0(0x33, 6, d, a, b, 0); // or d, a, b
        break;
    case '%':
        ce_dkszs0(ll ? 0x3b:  0x33, 6, d, a, b, 1); // rem d, a, b
        break;
    case TOK_UMOD:
        ce_dkszs0(0x33 | ll, 7, d, a, b, 1); // remu d, a, b
        break;
    case TOK_PDIV:
    case TOK_UDIV:
        ce_dkszs0(0x33 | ll, 5, d, a, b, 1); // divu d, a, b
        break;
    }
}
...
ST_FUNC void gen_opf(int op)
{
    ...
    rs1 = ce_v0pcgz(vtop[-1].r);
    rs2 = ce_v0pcgz(vtop->r);
    vtop--;
    invert = 0;
    switch(op) {
    default:
    ...
    arithop:
        ...
        vtop->r = rd;
        rd = ce_v0pcgz(rd);
        ce_dkszs0(0x53, 7, rd, rs1, rs2, dbl | (op << 2)); // fop.[sd] RD, RS1, RS2 (dyn rm)
        break;
    case '-':
        op = 1; // fsub
        goto arithop;
    ...
    cmpop:
        ...
        vtop->r = rd;
        rd = ce_fox9oz(rd);
        ce_dkszs0(0x53, op, rd, rs1, rs2, dbl | 0x50); // fcmp.[sd] RD, RS1, RS2 (op == eq/lt/le)
        if (invert)
          ce_vqbn7o(0x13, 4, rd, rd, 1); // xori RD, 1
        break;
    case TOK_NE:
        invert = 1;
        op = 2; // EQ
...
ST_FUNC void gen_cvt_itof(int t)
{
    ...
    if (t == VT_LDOUBLE) {
        ...
        vtop->r = dr;
        dr = ce_v0pcgz(dr);
        ce_12nmnj(0x53, 7, dr, rr, ((0x68 | (t == VT_DOUBLE ? 1 : 0)) << 5) | (u ? 1 : 0) | (l ? 2 : 0)); // fcvt.[sd].[wl][u]
    }
}
...
ST_FUNC void gen_cvt_ftoi(int t)
{
    ...
    if (ft == VT_LDOUBLE) {
        ...
        int rr = ce_v0pcgz(gv(RC_FLOAT)), dr;
        vtop--;
        dr = get_reg(RC_INT);
        vtop++;
        vtop->r = dr;
        dr = ce_fox9oz(dr);
        ce_12nmnj(0x53, 1, dr, rr, ((0x60 | (ft == VT_DOUBLE ? 1 : 0)) << 5) | (u ? 1 : 0) | (l ? 2 : 0)); // fcvt.[wl][u].[sd] rtz
    }
}
    ...
    if (dt == VT_LDOUBLE || st == VT_LDOUBLE) {
        ...
        if (dt == VT_LDOUBLE)
          gv(RC_F(0));
            ...
            assert(vtop->r2 < 7);
            if (vtop->r2 != 1 + vtop->r) {
                ce_vqbn7o(0x13, 0, ce_fox9oz(vtop->r) + 1, ce_fox9oz(vtop->r2), 0); // mv Ra+1, RR2
                vtop->r2 = 1 + vtop->r;
            }
        }
        vpush_helper_func(func);
        ...
        rd = get_reg(RC_FLOAT);
        if (dt == VT_DOUBLE)
          ce_vqbn7o(0x53, 0, ce_v0pcgz(rd), ce_v0pcgz(rs), 0x21 << 5); // fcvt.d.s RD, RS (no rm)
        else
          ce_vqbn7o(0x53, 7, ce_v0pcgz(rd), ce_v0pcgz(rs), (0x20 << 5) | 1); // fcvt.s.d RD, RS (dyn rm)
        vtop->r = rd;
    }
}
...
{
    ce_1f6fao(0x23, 3, 8, 2, addr); // sd sp, fc(s0)
}

ST_FUNC void gen_vla_sp_restore(int addr)
{
    ce_vqbn7o(0x03, 3, 2, 8, addr); // ld sp, fc(s0)
}
...
ST_FUNC void gen_vla_alloc(CType *type, int align)
{
    ...
    if (tcc_state->do_bounds_check)
        vpushv(vtop);
#endif
    rr = ce_fox9oz(gv(RC_INT));
#if defined(CONFIG_TCC_BCHECK)
    if (tcc_state->do_bounds_check)
        ce_vqbn7o(0x13, 0, rr, rr, 15+1);   // addi RR, RR, 15+1
    else
#endif
    ce_vqbn7o(0x13, 0, rr, rr, 15);   // addi RR, RR, 15
    ce_vqbn7o(0x13, 7, rr, rr, -16);  // andi, RR, RR, -16
    ce_dkszs0(0x33, 0, 2, 2, rr, 0x20); // sub sp, sp, rr
    vpop();
#if defined(CONFIG_TCC_BCHECK)
    if (tcc_state->do_bounds_check) {
        vpushi(0);
