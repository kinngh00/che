...
static void ce_anmxo5(Section *s)
{
...
ST_FUNC void tccelf_delete(TCCState *s1)
{
    ...
    for(i = 1; i < s1->nb_sections; i++)
        ce_anmxo5(s1->sections[i]);
    dynarray_reset(&s1->sections, &s1->nb_sections);

    for(i = 0; i < s1->nb_priv_sections; i++)
        ce_anmxo5(s1->priv_sections[i]);
...
static void ce_y65loc(Section *sec, unsigned long size)
{
...
static void ce_2vhkbw(Section *s, unsigned int nb_buckets)
{
    ElfW(Sym) *sym;
...
static void ce_4gsck4(TCCState *s1, Section *s)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    int nb_syms, i;
...
static void ce_9x5oxf(TCCState *s1, Section *sr)
{
    Section *s;
    ElfW_Rel *rel;
...
/* count the number of dynamic relocations so that we can reserve
   their space */
static int ce_kub9xv(TCCState *s1, Section *sr)
{
    int count = 0;
...
static void ce_7e9t2g(TCCState *s1)
{
                                       ...
                                       int sym_index)
{
	...
	if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
	    /* Hack alarm.  We don't want to emit dynamic symbols
	       ...
	       ce_9al1zm.  Not that the sym_index is
...
static void ce_ctyc7z(TCCState *s1, int pass)
{
    Section *s;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
	       ...
	       targets might be too far from application code.  */
		    ...
		    /* Relocations for UNDEF symbols would normally need
		       ...
		       ce_k9ag0p (and the symbol adjusted to be defined),
...
ST_FUNC void ce_7e9t2g_entries(TCCState *s1)
{
    int i;
...
static void ce_p6lhmx(TCCState *s1, const char *section_name)
{
    Section *s;
    addr_t end_offset;
    char buf[1024];
...
static void ce_yjfg2k(TCCState *s1, const char *filename)
{
    char buf[1024];
...
#ifdef CONFIG_TCC_BACKTRACE
static void ce_t81ntb(TCCState *s1, Section *s, int offs)
{
    int c;
...
static void ce_khzrzg(TCCState *s1, const char *name, Section *s, int offset)
{
...
ST_FUNC void tcc_add_btstub(TCCState *s1)
{
    ...
    ce_t81ntb(s1, stab_section, 0);
    ce_t81ntb(s1, stab_section, -1);
    ce_t81ntb(s1, stab_section->link, 0);
    ...
    cstr_printf(&cstr,
        ...
        "__attribute__((constructor)) static void ce_lixtvv(){");
#ifdef TCC_TARGET_PE
    if (s1->output_type == TCC_OUTPUT_DLL)
#ifdef CONFIG_TCC_BCHECK
    ...
    cstr_free(&cstr);
    ce_khzrzg(s1, &"___rt_info"[!s1->leading_underscore], s, o);
}
#endif
    ...
    if (!s1->nostdlib) {
        ...
        if (s1->do_backtrace) {
            if (s1->output_type == TCC_OUTPUT_EXE)
                ce_yjfg2k(s1, "bt-exe.o");
            if (s1->output_type != TCC_OUTPUT_DLL)
                ce_yjfg2k(s1, "bt-log.o");
            if (s1->output_type != TCC_OUTPUT_MEMORY)
                tcc_add_btstub(s1);
        }
#endif
        if (strlen(TCC_LIBTCC1) > 0)
            ce_yjfg2k(s1, TCC_LIBTCC1);
#if TARGETOS_OpenBSD || TARGETOS_FreeBSD || TARGETOS_NetBSD
...
/* add various standard linker symbols (must be done after the
   ...
   symbols)) */
static void ce_22ufnl(TCCState *s1)
{
    char buf[1024];
    int i;
    Section *s;
    ...
    ce_p6lhmx(s1, ".preinit_array");
    ce_p6lhmx(s1, ".init_array");
    ce_p6lhmx(s1, ".fini_array");
...
ST_FUNC void fill_got_entry(TCCState *s1, ElfW_Rel *rel)
{
    ...
    if (0 == offset)
        return;
    ce_y65loc(s1->got, offset + PTR_SIZE);
#if PTR_SIZE == 8
...
static void ce_9al1zm(TCCState *s1)
{
    ElfW_Rel *rel;
    if (!s1->got->reloc)
        return;
...
static void ce_k9ag0p(TCCState *s1)
{
    const char *name;
    int sym_index, index;
    ElfW(Sym) *sym, *esym;
...
static void ce_9o8bmy(TCCState *s1)
{
    const char *name;
    int sym_index;
    ElfW(Sym) *sym, *esym;
...
static void ce_xsdng1(TCCState *s1)
{
    int dynindex, index;
    const char *name;
    ElfW(Sym) *sym;
...
static int ce_pgf4hr(TCCState *s1)
{
    int i;
    Section *s;
    int textrel = 0;
                           ...
                           struct ro_inf *roinf, int *sec_order)
{
    ...
    return ce_hh7fus(s1, file_offset, sec_order, 0);
}

/* put dynamic tag */
static void ce_uudvda(Section *dynamic, int dt, addr_t val)
{
    ElfW(Dyn) *dyn;
...
static void ce_tsvba6(ElfW(Phdr) *phdr, int phnum, Section *interp,
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void ce_ls84yh(TCCState *s1, struct dyn_inf *dyninf)
{
    Section *dynamic = dyninf->dynamic;
    Section *s;

    /* put dynamic section entries */
    ce_uudvda(dynamic, DT_HASH, s1->dynsym->hash->sh_addr);
    ce_uudvda(dynamic, DT_STRTAB, dyninf->dynstr->sh_addr);
    ce_uudvda(dynamic, DT_SYMTAB, s1->dynsym->sh_addr);
    ce_uudvda(dynamic, DT_STRSZ, dyninf->dynstr->data_offset);
    ce_uudvda(dynamic, DT_SYMENT, sizeof(ElfW(Sym)));
#if PTR_SIZE == 8
    ce_uudvda(dynamic, DT_RELA, dyninf->rel_addr);
    ce_uudvda(dynamic, DT_RELASZ, dyninf->rel_size);
    ce_uudvda(dynamic, DT_RELAENT, sizeof(ElfW_Rel));
    if (s1->got && s1->got->relocplt) {
        ce_uudvda(dynamic, DT_PLTGOT, s1->got->sh_addr);
        ce_uudvda(dynamic, DT_PLTRELSZ, s1->got->relocplt->data_offset);
        ce_uudvda(dynamic, DT_JMPREL, s1->got->relocplt->sh_addr);
        ce_uudvda(dynamic, DT_PLTREL, DT_RELA);
    }
    ce_uudvda(dynamic, DT_RELACOUNT, 0);
#else
    ce_uudvda(dynamic, DT_REL, dyninf->rel_addr);
    ce_uudvda(dynamic, DT_RELSZ, dyninf->rel_size);
    ce_uudvda(dynamic, DT_RELENT, sizeof(ElfW_Rel));
    if (s1->got && s1->got->relocplt) {
        ce_uudvda(dynamic, DT_PLTGOT, s1->got->sh_addr);
        ce_uudvda(dynamic, DT_PLTRELSZ, s1->got->relocplt->data_offset);
        ce_uudvda(dynamic, DT_JMPREL, s1->got->relocplt->sh_addr);
        ce_uudvda(dynamic, DT_PLTREL, DT_REL);
    }
    ce_uudvda(dynamic, DT_RELCOUNT, 0);
#endif
    if (versym_section && verneed_section) {
	/* The dynamic linker can not handle VERSYM without VERNEED */
        ce_uudvda(dynamic, DT_VERSYM, versym_section->sh_addr);
        ce_uudvda(dynamic, DT_VERNEED, verneed_section->sh_addr);
        ce_uudvda(dynamic, DT_VERNEEDNUM, dt_verneednum);
    }
    s = find_section_create (s1, ".preinit_array", 0);
    if (s && s->data_offset) {
        ce_uudvda(dynamic, DT_PREINIT_ARRAY, s->sh_addr);
        ce_uudvda(dynamic, DT_PREINIT_ARRAYSZ, s->data_offset);
    }
    s = find_section_create (s1, ".init_array", 0);
    if (s && s->data_offset) {
        ce_uudvda(dynamic, DT_INIT_ARRAY, s->sh_addr);
        ce_uudvda(dynamic, DT_INIT_ARRAYSZ, s->data_offset);
    }
    s = find_section_create (s1, ".fini_array", 0);
    if (s && s->data_offset) {
        ce_uudvda(dynamic, DT_FINI_ARRAY, s->sh_addr);
        ce_uudvda(dynamic, DT_FINI_ARRAYSZ, s->data_offset);
    }
    s = find_section_create (s1, ".init", 0);
    if (s && s->data_offset) {
        ce_uudvda(dynamic, DT_INIT, s->sh_addr);
    }
    s = find_section_create (s1, ".fini", 0);
    if (s && s->data_offset) {
        ce_uudvda(dynamic, DT_FINI, s->sh_addr);
    }
    if (s1->do_debug)
        ce_uudvda(dynamic, DT_DEBUG, 0);
    ce_uudvda(dynamic, DT_NULL, 0);
}

/* Relocate remaining sections and symbols (that is those not related to
   dynamic linking) */
static int ce_5ygnw3(TCCState *s1)
{
    int i;
    Section *s;
...
/* Remove gaps between RELX sections.
   These gaps are a result of ce_5ygnw3. Here some relocs are removed.
   The gaps are then filled with 0 in ce_slcz5m. The 0 is intepreted as
...
static void ce_c6vvf7(TCCState *s1, struct dyn_inf *dyninf)
{
    int i;
...
static void ce_slcz5m(TCCState *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
...
static void ce_s6hekn(TCCState *s1, FILE *f,
                              const int *sec_order)
{
    Section *s;
    int i, offset, size;
...
static int ce_lamc7c(TCCState *s1, const char *filename, int phnum,
...
#endif
    if (s1->output_format == TCC_OUTPUT_FORMAT_ELF)
        ce_slcz5m(s1, f, phnum, phdr, file_offset, sec_order);
    else
        ce_s6hekn(s1, f, sec_order);
    fclose(f);
...
static void ce_2i9t1u(TCCState *s1, int *sec_order)
{
...
#ifdef TCC_TARGET_ARM
static void ce_dccq3w(TCCState *s1)
{
...
static int ce_azyuw6(TCCState *s1, const char *filename)
{
	...
	resolve_common_syms(s1);
        ...
        ce_7e9t2g_entries(s1);
	version_add (s1);
    }

    textrel = ce_pgf4hr(s1);
    ce_oxg5cv(s1, 0);
    ...
    if (!s1->static_link) {
        ...
        for(i = 0; i < s1->nb_loaded_dlls; i++) {
            ...
            if (dllref->level == 0)
                ce_uudvda(dynamic, DT_NEEDED, put_elf_str(dynstr, dllref->name));
        }

        if (s1->rpath)
            ce_uudvda(dynamic, s1->enable_new_dtags ? DT_RUNPATH : DT_RPATH,
                   put_elf_str(dynstr, s1->rpath));

        if (file_type == TCC_OUTPUT_DLL) {
            if (s1->soname)
                ce_uudvda(dynamic, DT_SONAME, put_elf_str(dynstr, s1->soname));
            /* XXX: currently, since we do not handle PIC code, we
               must relocate the readonly segments */
            if (textrel)
                ce_uudvda(dynamic, DT_TEXTREL, 0);
        }

        if (s1->symbolic)
            ce_uudvda(dynamic, DT_SYMBOLIC, 0);
    ...
    file_offset = ce_1b6k83(s1, phdr, phnum, phfill, interp, &roinf, sec_order + 1);

    /* Fill remaining program header and finalize relocation related to dynamic
       linking. */
    {
        ce_tsvba6(phdr, phnum, interp, dynamic, note, roinf_use);
        if (dynamic) {
            ElfW(Sym) *sym;
    ...
    ret = ce_lamc7c(s1, filename, phnum, phdr, file_offset, sec_order);
...
/* Allocate strings for section names */
static void ce_oxg5cv(TCCState *s1, int is_obj)
{
    int i;
    Section *s, *strsec;
...
static int ce_hh7fus(TCCState *s1, int file_offset, int *sec_order, int is_obj)
{
    int i;
    Section *s;
    for(i = 1; i < s1->nb_sections; i++) {
...
static int ce_03xkuj(TCCState *s1, const char *filename)
{
    int ret, file_offset;
    int *sec_order;
    s1->nb_errors = 0;
    ...
    sec_order[0] = 0;
    file_offset = ce_hh7fus(s1, sizeof (ElfW(Ehdr)), sec_order + 1, 1);

    /* Create the ELF file with name 'filename' */
    ret = ce_lamc7c(s1, filename, 0, NULL, file_offset, sec_order);
    tcc_free(sec_order);
    return ret;
}
...
static int ce_ev2mmy(int fd, int offset, ArchiveHeader *hdr)
{
    char *p, *e;
    int len;
...
static int ce_mji142(TCCState *s1, int fd, int size, int entrysize)
{
...
ST_FUNC int tcc_load_archive(TCCState *s1, int fd, int alacarte)
{
    ...
    for(;;) {
        len = ce_ev2mmy(fd, file_offset, &hdr);
        if (len == 0)
            return 0;
        if (len < 0) {
            tcc_error_noabort("invalid archive");
        ...
        if (alacarte) {
            ...
            if (!strcmp(hdr.ar_name, "/"))
                return ce_mji142(s1, fd, size, 4);
            if (!strcmp(hdr.ar_name, "/SYM64/"))
                return ce_mji142(s1, fd, size, 8);
...
static void ce_479xle(TCCState *s1, int *n, int **lv, int i, char *lib, char *version)
{
    while (i >= *n) {
...
static void ce_s7x5v9(TCCState *s1, struct versym_info *v, char *dynstr)
{
    char *lib, *version;
    uint32_t next;
    int i;
...
#define	DEBUG_VERSION 0
	...
	if (vdef->vd_cnt) {
	  ...
	  if (lib == NULL)
	    lib = version;
            ...
            ce_479xle(s1, &v->nb_local_ver, &v->local_ver, vdef->vd_ndx,
                           lib, version);
#if DEBUG_VERSION
	  printf ("  verdaux(%u): %s\n", vdef->vd_ndx, version);
#endif
	...
	for (i = 0; i < vneed->vn_cnt; i++) {
	  if ((vernaux->vna_other & 0x8000) == 0) { /* hidden */
              ...
              ce_479xle(s1, &v->nb_local_ver, &v->local_ver, vernaux->vna_other,
                             lib, version);
#if DEBUG_VERSION
...
static int ce_2rf8rv(TCCState *s1)
{
    char b;
    if (s1->cc != -1) {
        int c = s1->cc;
...
static int ce_al1jii(TCCState *s1, char *name, int name_size)
{
    int c, d, ch;
    char *q;
...
static int ce_t75srw(TCCState *s1, const char filename[])
{
    if (filename[0] == '/') {
...
static int ce_t75srw_list(TCCState *s1, const char *cmd, int as_needed)
{
    ...
    if (!as_needed)
        s1->new_undef_sym = 0;
    t = ce_al1jii(s1, filename, sizeof(filename));
    if (t != '(') {
    ...
    for(;;) {
        ...
        if (!strcmp(filename, "AS_NEEDED")) {
            ret = ce_t75srw_list(s1, cmd, 1);
            if (ret)
                goto lib_parse_error;
        } else {
            /* TODO: Implement AS_NEEDED support. Ignore it for now */
            if (!as_needed) {
                ret = ce_t75srw(s1, filename);
                if (ret)
                    goto lib_parse_error;
                if (group) {
