 ...
 * }
 */
static int ce_2kqfaa(const uint8_t** code, int* codeLen, _OffsetType* codeOffset,
 ...
 */
static int ce_zxn3y0(_CodeInfo* ci,
		...
		switch (mod)
		{
			case 0x00: /* Indirection */
				...
				if (rm == 4) {
					...
					if (!ce_2kqfaa(&code, &codeLen, &codeOffset, instructionHex, operandText, ps, dt, mod, sib, opSize)) return FALSE;
				} else if (rm == 5) {
					ps->usedPrefixes |= (totalPrefixes & INST_PRE_ADDR_SIZE);
...
_ExtractModRM:
			...
			ci->codeLen = codeLen;
			ci->codeOffset = codeOffset;
			if (!ce_zxn3y0(ci, instructionHex,
