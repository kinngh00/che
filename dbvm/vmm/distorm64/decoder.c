...
static int ce_kh34sa(const uint8_t* code, int codeLen, _OffsetType codeOffset,
	...
	*usedInstructionsCount = 0;
	...
	while (codeLen > 0) {
		...
		if (prefixSize > 0) {
			...
			if (ce_kh34sa(code, codeLen, codeOffset, &ps, dt, pdi)) {
			...
			pdi = &result[nextPos++];
			ce_kh34sa(code, codeLen, codeOffset, &ps, dt, pdi);
			pdi->offset = startCodeOffset;
